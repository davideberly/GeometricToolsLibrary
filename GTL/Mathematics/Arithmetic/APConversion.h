...
namespace gtl
{
    ...
    public:
        ...
        APConversion(std::size_t precision, std::size_t maxIterations)
            :
            mPrecision(precision),
            mMaxIterations(maxIterations),
            mThreshold(std::ldexp(C_<Rational>(1), -static_cast<std::int32_t>(mPrecision)))
        {
            GTL_ARGUMENT_ASSERT(
                mPrecision > 0,
        ...
        inline std::size_t GetPrecision() const
        {
            return mPrecision;
        }
        ...
        std::size_t EstimateSqrt(Rational const& aSqr, Rational& aMin, Rational& aMax)
        {
            ...
            for (iterate = 1; iterate <= mMaxIterations; ++iterate)
            {
                ...
                aMax.Convert(2 * mPrecision, APRoundingMode::UPWARD, aMax);
                aMin = aSqr / aMax;
            }
            return iterate;
        }
        ...
        // interval.
        std::size_t EstimateSqrt(Rational const& aSqr, Rational& a)
        {
            // Compute a bounding interval aMin <= a <= aMax.
            Rational aMin{}, aMax{};
            std::size_t numIterates = EstimateSqrt(aSqr, aMin, aMax);
            // Use the average of the interval endpoints as the estimate.
            a = std::ldexp(aMin + aMax, -1);
            return numIterates;
        }

        std::size_t EstimateApB(Rational const& aSqr, Rational const& bSqr,
            Rational& tMin, Rational& tMax)
        {
            ...
            for (iterate = 1; iterate <= mMaxIterations; ++iterate)
            {
                ...
                tMax.Convert(2 * mPrecision, APRoundingMode::UPWARD, tMax);
                tMaxSqr = tMax * tMax;
                tMin = (a2pb2 * tMaxSqr - a2mb2Sqr) / (tMax * (tMaxSqr - a2pb2));
            }
            return iterate;
        ...
        std::size_t EstimateAmB(Rational const& aSqr, Rational const& bSqr,
            Rational& tMin, Rational& tMax)
        {
                ...
                if (signSecDer < C_<Rational>(0))
                {
                    ...
                    for (iterate = 1; iterate <= mMaxIterations; ++iterate)
                    {
                        ...
                        else
                        {
                            ...
                            // quadratic growth in the number of bits.
                            tMin = tMid;
                            tMin.Convert(2 * mPrecision, APRoundingMode::DOWNWARD, tMin);
                        }
                    }
                }
        ...
        std::size_t Estimate(QFN1 const& q, Rational& qMin, Rational& qMax)
        {
            Rational const& x = q.x[0];
            Rational const& y = q.x[1];
            Rational const& d = q.d;

            std::size_t numIterates{};
        ...
        // Compute an estimate of the root when you do not need a bounding
        // interval.
        std::size_t Estimate(QFN1 const& q, Rational& qEstimate)
        {
            // Compute a bounding interval qMin <= q <= qMax.
            Rational qMin{}, qMax{};
            std::size_t numIterates = Estimate(q, qMin, qMax);
            // Use the average of the interval endpoints as the estimate.
    ...
    private:
        void PreprocessSqr(Rational const& aSqr, Rational& rSqr, std::int32_t& exponentA)
        {
            // Factor a^2 = r^2 * 2^e, where r^2 in [1/2,1).
            std::int32_t exponentASqr{};
        ...
        Rational GetMinOfSqrt(Rational const& rSqr, std::int32_t exponent)
        {
            // Compute a lower bound on the square root of r^2.
            double lowerRSqr = 0.0;
            rSqr.Convert(APRoundingMode::DOWNWARD, lowerRSqr);
        ...
        Rational GetMaxOfSqrt(Rational const& rSqr, std::int32_t exponent)
        {
            // Compute an upper bound on the square root of r^2.
            double upperRSqr = 0.0;
            rSqr.Convert(APRoundingMode::UPWARD, upperRSqr);
