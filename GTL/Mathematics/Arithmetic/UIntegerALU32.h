...
//     // i-th block, where 'i' is of type std::size_t. UIntegerAP32 has Container
//     // = std::vector<std::uint32_t> and UIntegerFP32<N> has Container =
...
//     // need for 'void SetNumBlocks(std::size_t)' because the number of blocks
//     // is modified by 'void SetNumBits(std::size_t)'.
...
namespace gtl
{
    ...
    public:
        ...
        {
            std::size_t const numBits0 = n0.GetNumBits();
            std::size_t const numBits1 = n1.GetNumBits();
            ...
            if (numBits0 > 0)
            {
                std::size_t const numBlocks0 = n0.GetNumBlocks();
                auto const& bits0 = n0.GetBits();
                auto const& bits1 = n1.GetBits();
                for (std::size_t j = 0, i = numBlocks0 - 1; j < numBlocks0; ++j, --i)
                {
                    if (bits0[i] != bits1[i])
                    {
                        return false;
        ...
        static bool LessThan(UInteger const& n0, UInteger const& n1)
        {
            ...
            if (numBits0 > 0 && numBits1 > 0)
            {
                ...
                auto const& bits0 = n0.GetBits();
                auto const& bits1 = n1.GetBits();
                std::size_t const bitIndex0 = numBits0 - 1;
                std::size_t const bitIndex1 = numBits1 - 1;
                std::uint32_t const numBlockBits0 = static_cast<std::uint32_t>(1 + (bitIndex0 % 32));
                std::uint32_t const numBlockBits1 = static_cast<std::uint32_t>(1 + (bitIndex1 % 32));
                std::int64_t blockIndex0 = static_cast<std::int64_t>(bitIndex0 / 32);
                std::int64_t blockIndex1 = static_cast<std::int64_t>(bitIndex1 / 32);

                // The type of block0 and block1 is std::uint64_t because it is
                ...
                // compiling with MSVS 2019 (16.5.1), if u is a std::uint32_t, the
                // result of (u >> 32) is u, not 0.
                std::uint64_t block0 = static_cast<std::uint64_t>(bits0[blockIndex0]);
                std::uint64_t block1 = static_cast<std::uint64_t>(bits1[blockIndex1]);
                while (blockIndex0 >= 0 && blockIndex1 >= 0)
                {
                    std::uint32_t const lshift0 = static_cast<std::uint32_t>(32 - numBlockBits0);
                    std::uint32_t const lshift1 = static_cast<std::uint32_t>(32 - numBlockBits1);
                    std::uint32_t value0 = static_cast<std::uint32_t>((block0 << lshift0) & 0x00000000FFFFFFFFull);
                    std::uint32_t value1 = static_cast<std::uint32_t>((block1 << lshift1) & 0x00000000FFFFFFFFull);

                    // Shift bits in the next block (if any) to fill the
                    // current block.
                    if (--blockIndex0 >= 0)
                    {
                        block0 = static_cast<std::uint64_t>(bits0[blockIndex0]);
                        value0 |= static_cast<std::uint32_t>((block0 >> numBlockBits0) & 0x00000000FFFFFFFFull);
                    }
                    if (--blockIndex1 >= 0)
                    {
                        block1 = static_cast<std::uint64_t>(bits1[blockIndex1]);
                        value1 |= static_cast<std::uint32_t>((block1 >> numBlockBits1) & 0x00000000FFFFFFFFull);
                    }
                    if (value0 < value1)
                    {
                        return true;
        ...
        {
            std::size_t const n0NumBits = n0.GetNumBits();
            std::size_t const n1NumBits = n1.GetNumBits();

            // Add the numbers considered as positive integers. Set the last
            // block to zero in case no carry-out occurs.
            std::size_t const numBits = std::max(n0NumBits, n1NumBits) + 1;
            result.SetNumBits(numBits);
            result.SetBack(0);

            // Get the input array sizes.
            std::size_t const numBlocks0 = n0.GetNumBlocks();
            std::size_t const numBlocks1 = n1.GetNumBlocks();
            ...
            {
                std::uint64_t sum = u0[i] + (u1[i] + carry);
                bits[i] = static_cast<std::uint32_t>(sum & 0x00000000FFFFFFFFull);
                carry = (sum >> 32);
            }
            ...
            if (carry > 0)
            {
                ...
                {
                    std::uint64_t sum = u0[i] + carry;
                    bits[i] = static_cast<std::uint32_t>(sum & 0x00000000FFFFFFFFull);
                    carry = (sum >> 32);
                }
                if (carry > 0)
                {
                    bits[i] = static_cast<std::uint32_t>(carry & 0x00000000FFFFFFFFull);
                }
            }
            else
            {
            ...
            std::uint32_t const firstBitIndex = static_cast<std::uint32_t>(numBitsM1 % 32);
            std::uint32_t const mask = (1 << firstBitIndex);
        ...
        static void Sub(UInteger const& n0, UInteger const& n1, UInteger& result)
        {
            ...
            {
                std::uint64_t sum = bits2[i] + carry;
                bits2[i] = static_cast<std::uint32_t>(sum & 0x00000000FFFFFFFFull);
                carry = (sum >> 32);
            }
            ...
            {
                std::uint64_t sum = bits2[i] + (bits0[i] + carry);
                bits[i] = static_cast<std::uint32_t>(sum & 0x00000000FFFFFFFFull);
                carry = (sum >> 32);
            }
            ...
            std::size_t block = numBlocks0 - 1;
            for (std::size_t j = 0; j < numBlocks0; ++j, --block)
            {
                if (bits[block] > 0)
                {
            ...
            {
                std::size_t constexpr bitsPerBlock = 32;
                std::size_t const leading = static_cast<std::size_t>(BitHacks::GetLeadingBit(bits[block]));
                result.SetNumBits(bitsPerBlock * block + leading + 1);
            }
            else
            {
                // TODO: This block originally did not exist in GTL, only the
        ...
        static void Mul(UInteger const& n0, UInteger const& n1, UInteger& result)
        {
            ...
            std::size_t const numBits = numBits0 + numBits1;
            result.SetNumBits(numBits);
            auto& bits = result.GetBits();
            ...
            std::size_t const numBlocks0 = n0.GetNumBlocks();
            std::size_t const numBlocks1 = n1.GetNumBlocks();
            std::size_t const numBlocks = result.GetNumBlocks();
            ...
            std::uint64_t block0 = bits0[0];
            std::uint64_t carry = 0;
            for (i1 = 0; i1 < numBlocks1; ++i1)
            {
                std::uint64_t term = block0 * bits1[i1] + carry;
                bits[i1] = static_cast<std::uint32_t>(term & 0x00000000FFFFFFFFull);
                carry = (term >> 32);
            }
            if (i1 < numBlocks)
            {
                bits[i1] = static_cast<std::uint32_t>(carry & 0x00000000FFFFFFFFull);
            }
            ...
            for (i0 = 1; i0 < numBlocks0; ++i0)
            {
                ...
                {
                    std::uint64_t term = block0 * bits1[i1] + carry;
                    pBits[i2] = static_cast<std::uint32_t>(term & 0x00000000FFFFFFFFull);
                    carry = (term >> 32);
                }
                if (i2 < numBlocks)
                {
                    pBits[i2] = static_cast<std::uint32_t>(carry & 0x00000000FFFFFFFFull);
                }
                ...
                for (i1 = 0, i2 = i0; i1 < numBlocks1; ++i1, ++i2)
                {
                    std::uint64_t sum = pBits[i2] + (bits[i2] + carry);
                    bits[i2] = static_cast<std::uint32_t>(sum & 0x00000000FFFFFFFFull);
                    carry = (sum >> 32);
                }
                if (i2 < numBlocks)
                {
                    std::uint64_t sum = pBits[i2] + carry;
                    bits[i2] = static_cast<std::uint32_t>(sum & 0x00000000FFFFFFFFull);
                }
            }
        ...
        static void ShiftLeft(UInteger const& n0, std::int32_t inShift, UInteger& result)
        {
            GTL_ARGUMENT_ASSERT(
                inShift > 0,
                "Shift must be positive.");

            std::size_t shift = static_cast<std::size_t>(inShift);

            std::size_t numBits0 = n0.GetNumBits();
            auto const& bits0 = n0.GetBits();
            ...
            auto& bits = result.GetBits();
            std::size_t const shiftBlock = static_cast<std::size_t>(shift / 32);
            std::size_t i{};
            ...
            std::size_t const numBlocks0 = n0.GetNumBlocks();
            std::uint32_t const lshift = shift % 32;
            std::size_t j{};
            if (lshift > 0)
            {
                ...
                std::uint32_t const rshift = static_cast<std::uint32_t>(32 - lshift);
                std::uint32_t prev = 0;
                for (i = shiftBlock, j = 0; j < numBlocks0; ++i, ++j)
                {
                    std::uint32_t const curr = bits0[j];
        ...
        // shifted.
        static std::int32_t ShiftRightToOdd(UInteger const& n0, UInteger& result)
        {
            auto const& bits0 = n0.GetBits();

            // Get the leading 1-bit.
            std::size_t const numBlocks0 = n0.GetNumBlocks();
            std::size_t const numBlocks0M1 = numBlocks0 - 1;
            std::size_t const leading = static_cast<std::size_t>(BitHacks::GetLeadingBit(bits0[numBlocks0M1]));
            std::size_t constexpr bitsPerBlock = 32;
            std::size_t const firstBitIndex = bitsPerBlock * numBlocks0M1 + leading;

            // Get the trailing 1-bit.
            std::size_t lastBitIndex = std::numeric_limits<std::size_t>::max();
            for (std::size_t block = 0; block < numBlocks0; ++block)
            {
                std::uint32_t value = bits0[block];
                if (value > 0)
                {
                    std::size_t trailing = static_cast<std::size_t>(BitHacks::GetTrailingBit(value));
                    lastBitIndex = bitsPerBlock * block + trailing;
                    break;
                }
            }
            // As long as the preconditions n0 even and positive are satisfied,
            // lastBitIndex will not be std::numeric_limits<std::size_t>::max().

            // The right-shifted result.
            std::size_t const numBits = firstBitIndex - lastBitIndex + 1;
            result.SetNumBits(numBits);
            auto& bits = result.GetBits();
            std::size_t const numBlocks = result.GetNumBlocks();

            // Get the location of the low-order 1-bit within the result.
            std::size_t const shiftBlock = lastBitIndex / 32;
            std::uint32_t const rshift = static_cast<std::uint32_t>(lastBitIndex % 32);
            if (rshift > 0)
            {
                std::uint32_t const lshift = static_cast<std::uint32_t>(32 - rshift);
                std::size_t i{}, j = shiftBlock;
                std::uint32_t curr = bits0[j++];
                for (i = 0; j < numBlocks0; ++i, ++j)
                {
                    std::uint32_t const next = bits0[j];
            ...
            {
                for (std::size_t i = 0, j = shiftBlock; i < numBlocks; ++i, ++j)
                {
                    bits[i] = bits0[j];
                }
            }

            std::size_t fullRShift = bitsPerBlock * shiftBlock + static_cast<std::size_t>(rshift);
            return static_cast<std::int32_t>(fullRShift);
        }
        ...
        static std::int32_t RoundUp(UInteger& n0)
        {
            UInteger rounded{};
