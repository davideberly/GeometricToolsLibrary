...
namespace gtl
{
    template <typename Float, typename UInt, std::int32_t NumBits, std::int32_t Precision>
    class IEEEBinary
    {
    public:
        ...
        // Special constants.
        static std::int32_t constexpr NUM_ENCODING_BITS = NumBits;
        static std::int32_t constexpr NUM_EXPONENT_BITS = NumBits - Precision;
        static std::int32_t constexpr NUM_SIGNIFICAND_BITS = Precision;
        static std::int32_t constexpr NUM_TRAILING_BITS = Precision - 1;
        static std::int32_t constexpr EXPONENT_BIAS = (1 << (NUM_EXPONENT_BITS - 1)) - 1;
        static std::int32_t constexpr MAX_BIASED_EXPONENT = (1 << NUM_EXPONENT_BITS) - 1;
        static std::int32_t constexpr MIN_SUB_EXPONENT = 1 - EXPONENT_BIAS;
        static std::int32_t constexpr MIN_EXPONENT = MIN_SUB_EXPONENT - NUM_TRAILING_BITS;
        static std::int32_t constexpr SIGN_SHIFT = NumBits - 1;

        static UInt constexpr SIGN_MASK = (UInt(1) << (NumBits - 1));
        static UInt constexpr NOT_SIGN_MASK = UInt(~SIGN_MASK);
        static UInt constexpr TRAILING_MASK = (UInt(1) << NUM_TRAILING_BITS) - 1;
        static UInt constexpr EXPONENT_MASK = NOT_SIGN_MASK & ~TRAILING_MASK;
        static UInt constexpr NAN_QUIET_MASK = (UInt(1) << (NUM_TRAILING_BITS - 1));
        static UInt constexpr NAN_PAYLOAD_MASK = (TRAILING_MASK >> 1);
        static UInt constexpr MAX_TRAILING = TRAILING_MASK;
        static UInt constexpr SUP_TRAILING = (UInt(1) << NUM_TRAILING_BITS);
        static UInt constexpr POS_ZERO = UInt(0);
        static UInt constexpr NEG_ZERO = SIGN_MASK;
        static UInt constexpr MIN_SUBNORMAL = UInt(1);
        static UInt constexpr MAX_SUBNORMAL = TRAILING_MASK;
        static UInt constexpr MIN_NORMAL = SUP_TRAILING;
        static UInt constexpr MAX_NORMAL = NOT_SIGN_MASK & ~SUP_TRAILING;
        static UInt constexpr POS_INFINITY = EXPONENT_MASK;
        static UInt constexpr NEG_INFINITY = SIGN_MASK | EXPONENT_MASK;
    ...
    private:
        friend class UnitTestIEEEBinary;
    };

    using IEEEBinary32 = IEEEBinary<float, std::uint32_t, 32, 24>;
    ...
    using IEEEBinary64 = IEEEBinary<double, std::uint64_t, 64, 53>;
