...
namespace gtl
{
    class IEEEBinary16 : public IEEEBinary<std::int16_t, std::uint16_t, 16, 11>
    {
    public:
        ...
        IEEEBinary16()
            :
            IEEEBinary<std::int16_t, std::uint16_t, 16, 11>()
        {
            // uninitialized
        }
        ...
        IEEEBinary16(IEEEBinary16 const& object)
            :
            IEEEBinary<std::int16_t, std::uint16_t, 16, 11>(object)
        {
        }
        ...
        IEEEBinary16(float inNumber)
            ...
            IEEEBinary<std::int16_t, std::uint16_t, 16, 11>()
        {
            union { float n; std::uint32_t e; } temp = { inNumber };
        ...
        IEEEBinary16(double inNumber)
            ...
            IEEEBinary<std::int16_t, std::uint16_t, 16, 11>()
        {
            union { float n; std::uint32_t e; } temp = { static_cast<float>(inNumber) };
            temp.n = static_cast<float>(inNumber);
            encoding = Convert32To16(temp.e);
        }

        IEEEBinary16(std::uint16_t inEncoding)
            :
            IEEEBinary<std::int16_t, std::uint16_t, 16, 11>(inEncoding)
        {
        }
        ...
        {
            union { std::uint32_t e; float n; } temp = { Convert16To32(encoding) };
            return temp.n;
        }
        ...
        {
            IEEEBinary<std::int16_t, std::uint16_t, 16, 11>::operator=(object);
            return *this;
        }
    ...
    private:
        // Members from the base class IEEEBinary<std::int16_t, std::uint16_t, 16, 11>.
        //
        ...
        static std::uint32_t constexpr F16_AVR_MIN_SUB_ZER = 0x33000000;  // 2^{-25}
        static std::uint32_t constexpr F16_MIN_SUB = 0x33800000;          // 2^{-24}
        static std::uint32_t constexpr F16_MIN_NOR = 0x38800000;          // 2^{-14}
        static std::uint32_t constexpr F16_MAX_NOR = 0x477FE000;          // 2^{16)*(1-2^{-11})
        static std::uint32_t constexpr F16_AVR_MAX_NOR_INF = 0x477FF000;  // 2^{16)*(1-2^{-12})

        // The amount to shift when converting between signs of 16-bit and
        // 32-bit numbers.
        static std::uint32_t constexpr CONVERSION_SIGN_SHIFT
        ...
        static std::uint32_t constexpr CONVERSION_TRAILING_SHIFT
        ...
        static std::uint32_t constexpr FRACTION_HALF = F32::SIGN_MASK;

        static std::uint16_t Convert32To16(std::uint32_t inEncoding)
        {
            ...
            std::uint32_t sign32 = (inEncoding & F32::SIGN_MASK);
            std::uint32_t biased32 = ((inEncoding & F32::EXPONENT_MASK) >> F32::NUM_TRAILING_BITS);
            std::uint32_t trailing32 = (inEncoding & F32::TRAILING_MASK);
            std::uint32_t nonneg32 = (inEncoding & F32::NOT_SIGN_MASK);

            // Generate the channels for y.
            std::uint16_t sign16 = static_cast<std::uint16_t>(sign32 >> CONVERSION_SIGN_SHIFT);
            ...
            if (biased32 < F32::MAX_BIASED_EXPONENT)
            {
                ...
                if (nonneg32 < F16_MIN_NOR)
                {
                    ...
                    // 0.s9 ... s0 * 2^{-14}.
                    std::int32_t p = static_cast<std::int32_t>(biased32) - F32::EXPONENT_BIAS
                        + F16::EXPONENT_BIAS;
                    ...
                    std::uint32_t rshift = static_cast<std::uint32_t>(
                        -F16::MIN_SUB_EXPONENT - p);
                    std::uint16_t trailing16 = static_cast<std::uint16_t>(trailing32 >> rshift);

                    // Get the fractional part.
                    std::uint32_t lshift = static_cast<std::uint32_t>(
                        F32::NUM_ENCODING_BITS + F16::MIN_SUB_EXPONENT + p);
                    std::uint32_t frcpart = (trailing32 << lshift);
                ...
                if (nonneg32 <= F16_MAX_NOR)
                {
                    ...
                    std::int32_t e = static_cast<std::int32_t>(biased32) - F32::EXPONENT_BIAS;
                    std::uint16_t biased16 = static_cast<std::uint16_t>(e + F16::EXPONENT_BIAS);
                    ...
                    // Get the integer part.
                    std::uint16_t trailing16 = static_cast<std::uint16_t>(
            ...
            std::uint16_t maskPayload = static_cast<std::uint16_t>(
                trailing32 >> CONVERSION_TRAILING_SHIFT);
            return sign16 | F16::EXPONENT_MASK | maskPayload;
        }

        static std::uint32_t Convert16To32(std::uint16_t inEncoding)
        {
            // Extract the channels for the IEEEBinary16 number.
            std::uint16_t sign16 = (inEncoding & F16::SIGN_MASK);
            std::uint16_t biased16 = ((inEncoding & F16::EXPONENT_MASK) >> F16::NUM_TRAILING_BITS);
            std::uint16_t trailing16 = (inEncoding & F16::TRAILING_MASK);

            // Generate the channels for the binary32 number.
            std::uint32_t sign32 = static_cast<std::uint32_t>(sign16 << CONVERSION_SIGN_SHIFT);
            ...
            if (biased16 == 0)
            {
                ...
                else
                {
                    ...
                    std::uint32_t trailing32 = static_cast<std::uint32_t>(trailing16);
                    std::int32_t leading = BitHacks::GetLeadingBit(trailing32);
                    std::int32_t shift = 23 - leading;
                    std::uint32_t biased32 = static_cast<std::uint32_t>(F32::EXPONENT_BIAS - 1 - shift);
            ...
            if (biased16 < F16::MAX_BIASED_EXPONENT)
            {
                ...
                std::uint32_t biased32 = static_cast<std::uint32_t>(biased16 - F16::EXPONENT_BIAS + F32::EXPONENT_BIAS);
                std::uint32_t trailing32 = (static_cast<std::uint32_t>(trailing16) << CONVERSION_TRAILING_SHIFT);
                return sign32 | (biased32 << F32::NUM_TRAILING_BITS) | trailing32;
            }
            ...
            std::uint32_t maskPayload = (static_cast<std::uint32_t>(trailing16) << CONVERSION_TRAILING_SHIFT);
    ...
    {
        std::uint16_t result = static_cast<std::uint16_t>(x) ^ IEEEBinary16::SIGN_MASK;
        return result;
    }
...
namespace std
{
    ...
    inline gtl::IEEEBinary16 frexp(gtl::IEEEBinary16 x, std::int32_t* exponent)
    {
        return static_cast<gtl::IEEEBinary16>(std::frexp(static_cast<float>(x), exponent));
    }

    inline gtl::IEEEBinary16 ldexp(gtl::IEEEBinary16 x, std::int32_t exponent)
    {
...
namespace gtl
{
    ...
    inline std::int32_t isign(IEEEBinary16 x)
    {
