...
namespace gtl
{
    ...
    public:
        ...
        // binary sequence.
        inline static std::uint32_t PopulationCount(std::uint32_t value)
        {
        ...
        inline static std::uint32_t PopulationCount(std::uint64_t value)
        {
        ...
        inline static std::uint32_t GetNumLeadingZeroBits(std::uint32_t value)
        {
            value = value | (value >> 1);
            value = value | (value >> 2);
            value = value | (value >> 4);
        ...
        inline static std::uint32_t GetNumLeadingZeroBits(std::uint64_t value)
        {
            value = value | (value >> 1);
            value = value | (value >> 2);
            value = value | (value >> 4);
        ...
        inline static std::uint32_t GetNumTrailingZeroBits(std::uint32_t value)
        {
            return static_cast<std::uint32_t>(32 - GetNumLeadingZeroBits(~value & (value - 1)));
        }

        inline static std::uint32_t GetNumTrailingZeroBits(std::uint64_t value)
        {
            return static_cast<std::uint32_t>(64 - GetNumLeadingZeroBits(~value & (value - 1)));
        }
        ...
        // the result.
        inline static std::uint32_t GetLeadingBit(std::uint32_t value)
        {
            ...
            if (value > 0)
            {
                return static_cast<std::uint32_t>(31 - GetNumLeadingZeroBits(value));
            }
            else
            {
        ...
        inline static std::uint32_t GetLeadingBit(std::uint64_t value)
        {
#if defined(GTL_THROW_ON_BIT_HACKS_ERROR)
            GTL_DOMAIN_ASSERT(
                value > 0,
            ...
            if (value > 0)
            {
                return static_cast<std::uint32_t>(63 - GetNumLeadingZeroBits(value));
            }
            else
            {
                return std::numeric_limits<std::uint32_t>::max();
        ...
        // runtime, then a test of the return value is required to validate
        // the result.
        inline static std::uint32_t GetTrailingBit(std::uint32_t value)
        {
#if defined(GTL_THROW_ON_BIT_HACKS_ERROR)
            GTL_DOMAIN_ASSERT(
                value > 0,
        ...
        inline static std::uint32_t GetTrailingBit(std::uint64_t value)
        {
#if defined(GTL_THROW_ON_BIT_HACKS_ERROR)
            GTL_DOMAIN_ASSERT(
                value > 0,
        ...
        inline static bool IsPowerOfTwo(std::uint32_t value)
        {
            return (value > 0) && ((value & (value - 1)) == 0);
        }

        inline static bool IsPowerOfTwo(std::uint64_t value)
        {
            return (value > 0) && ((value & (value - 1)) == 0);
        }
        ...
        inline static std::uint32_t Log2OfPowerOfTwo(std::uint32_t value)
        {
            ...
            if (isPowerOfTwo)
            {
                std::uint32_t log2 = (value & 0xAAAAAAAAu) != 0;
        ...
        inline static std::uint32_t Log2OfPowerOfTwo(std::uint64_t value)
        {
            bool isPowerOfTwo = IsPowerOfTwo(value);
#if defined(GTL_THROW_ON_BIT_HACKS_ERROR)
            GTL_DOMAIN_ASSERT(
            ...
            if (isPowerOfTwo)
            {
                std::uint64_t log2 = (value & 0xAAAAAAAAAAAAAAAAull) != 0;
                log2 |= static_cast<std::uint64_t>((value & 0xFFFFFFFF00000000ull) != 0) << 5;
                log2 |= static_cast<std::uint64_t>((value & 0xFFFF0000FFFF0000ull) != 0) << 4;
                log2 |= static_cast<std::uint64_t>((value & 0xFF00FF00FF00FF00ull) != 0) << 3;
                log2 |= static_cast<std::uint64_t>((value & 0xF0F0F0F0F0F0F0F0ull) != 0) << 2;
                log2 |= static_cast<std::uint64_t>((value & 0xCCCCCCCCCCCCCCCCull) != 0) << 1;
                return static_cast<std::uint32_t>(log2);
            }
            else
            {
                return std::numeric_limits<std::uint32_t>::max();
        ...
        // std::uint32_t input or n = 64 for std::uint64_t input.  The possible outcomes
        // for r are
        //   v = 0, r = {1, 0} (if exceptions are disabled)
        //   v = 2^p, r = {2^p, 0}
        //   1 < 2^{p-1} < v < 2^p <= 2^{n-1}, r = {2^p, 0}
        //   2^{n-1} < v < 2^n, r = {0, 1}
        inline static std::array<std::uint32_t, 2> RoundUpToPowerOfTwo(std::uint32_t value)
        {
#if defined(GTL_THROW_ON_BIT_HACKS_ERROR)
            GTL_DOMAIN_ASSERT(
                value > 0,
            ...
            if (value > 0)
            {
                std::uint32_t leading = GetLeadingBit(value);
                std::uint32_t mask = (1 << leading);
                if ((value & ~mask) == 0)
                {
        ...
        inline static std::array<std::uint64_t, 2> RoundUpToPowerOfTwo(std::uint64_t value)
        {
#if defined(GTL_THROW_ON_BIT_HACKS_ERROR)
            GTL_DOMAIN_ASSERT(
                value > 0,
            ...
            {
                std::uint32_t leading = GetLeadingBit(value);
                std::uint64_t mask = (1ull << leading);
                if ((value & ~mask) == 0)
                {
                    // The value is a power of two.
                    result[0] = value;
        ...
        inline static std::uint32_t RoundDownToPowerOfTwo(std::uint32_t value)
        {
#if defined(GTL_THROW_ON_BIT_HACKS_ERROR)
            GTL_DOMAIN_ASSERT(
                value > 0,
        ...
        inline static std::uint64_t RoundDownToPowerOfTwo(std::uint64_t value)
        {
#if defined(GTL_THROW_ON_BIT_HACKS_ERROR)
            GTL_DOMAIN_ASSERT(
                value > 0,
