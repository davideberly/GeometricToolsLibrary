...
namespace gtl
{
    ...
    private:
        friend class UnitTestPolygonTree;
    };
    ...
    public:
        ...
        public:
            ...
            std::vector<std::size_t> polygon;
            std::int64_t chirality;
            std::vector<std::array<std::size_t, 3>> triangulation;
            std::size_t self, parent, minChild, supChild;
        };
        ...
        // clipping.
        std::vector<std::array<std::size_t, 3>> interiorTriangles;
        std::vector<std::size_t> interiorNodeIndices;

        // The triangles in the polygon tree that cover each region bounded
        // by an inner polygon and its contained outer polygons.
        std::vector<std::array<std::size_t, 3>> exteriorTriangles;
        std::vector<std::size_t> exteriorNodeIndices;

        // The triangles inside the polygon tree.
        //   insideTriangles = interiorTriangle + exteriorTriangles
        std::vector<std::array<std::size_t, 3>> insideTriangles;
        std::vector<std::size_t> insideNodeIndices;
        ...
        // any 'nodes[]' element.
        std::vector<std::array<std::size_t, 3>> outsideTriangles;

        // All the triangles:
        //   allTriangles = insideTriangles + outsideTriangles.
        std::vector<std::array<std::size_t, 3>> allTriangles;
    ...
    public:
        ...
        // smax = std::numeric_limits<std::size_t>::max(). The function is
        ...
        template <typename T>
        std::pair<std::size_t, std::size_t> GetContainingTriangle(Vector2<T> const& test,
            Vector2<T> const* points)
        {
            std::size_t constexpr smax = std::numeric_limits<std::size_t>::max();
            std::pair<std::size_t, std::size_t> result = std::make_pair(smax, smax);
            ...
            while (stack.size() > 0 && result.first == smax)
            {
                ...
                for (std::size_t tIndex = 0; tIndex < node.triangulation.size(); ++tIndex)
                {
        ...
        // that the triangle is nodes[nIndex].triangulation[tIndex]. If there
        // is no such triangle, the returned pair is (smax,smax) where
        // smax = std::numeric_limits<std::size_t>::max(). The function uses a
        ...
        {
            GTL_ARGUMENT_ASSERT(
                triangles.size() == nodeIndices.size(),
                "Mismatched sizes.");
            ...
            {
                std::size_t const nIndex = nodeIndices[tIndex];
                auto const& node = nodes[nIndex];
        ...
        //   std::vector<Vector2<T>> points = <some vector of points>;
        //   Vector2<T> test = <some point>;
        //   std::size_t resultInt, resultExt, resultOut;
        ...
        template <typename T>
        std::size_t GetContainingTriangle(Vector2<T> const& test,
            std::vector<std::array<std::size_t, 3>> const& triangles, std::int64_t chirality,
            Vector2<T> const* points)
        {
    ...
    private:
        ...
        template <typename T>
        static bool PointInTriangle(Vector2<T> const& test, std::int64_t chirality,
            std::array<std::size_t, 3> const& triangle, Vector2<T> const* points)
        {
            T const sign = static_cast<T>(chirality);
            for (std::size_t i1 = 0, i0 = 2; i1 < 3; i0 = i1++)
            {
    ...
    private:
        // TODO: Split up the unit tests for these two classes.
        friend class UnitTestPolgonTreeEx;
