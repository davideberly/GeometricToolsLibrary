...
// Axis-Aligned Bounding Boxes for such an algorithm.  The Execute() function
...
// ordered so that element[0] < element[1]. The T type must be chosen to
// guarantee exact computation of edge-edge intersections.
...
namespace gtl
{
    ...
    public:
        IsPlanarGraph()
            :
            mDuplicatedPositions{},
            mDuplicatedEdges{},
            mDegenerateEdges{},
            mEdgesWithInvalidVertices{},
            mInvalidIntersections{}
        {
        }

        // Flags for classification of the query.
        static std::uint32_t constexpr IPG_IS_PLANAR_GRAPH              = 0x00000000;
        static std::uint32_t constexpr IPG_INVALID_INPUT_SIZES          = 0x00000001;
        static std::uint32_t constexpr IPG_DUPLICATED_POSITIONS         = 0x00000002;
        static std::uint32_t constexpr IPG_DUPLICATED_EDGES             = 0x00000004;
        static std::uint32_t constexpr IPG_DEGENERATE_EDGES             = 0x00000008;
        static std::uint32_t constexpr IPG_EDGES_WITH_INVALID_VERTICES  = 0x00000010;
        static std::uint32_t constexpr IPG_INVALID_INTERSECTIONS        = 0x00000020;

        // The function returns a combination of the IPG_* flags. A combined
        // value of 0 indicates the input forms a planar graph. If the
        // combined value is not zero, examine the flags for the failure
        // conditions and use the Get* member accessors to obtain specific
        // information about the failure. If the positions.size() < 2 or
        // edges.size() == 0, the IPG_INVALID_INPUT_SIZES flag is set.

        std::uint32_t Execute(std::vector<std::array<T, 2>> const& positions,
            std::vector<std::array<std::size_t, 2>> const& edges)
        {
            ...
            std::uint32_t flags = IsValidTopology(positions, edges);
            if (flags == IPG_INVALID_INPUT_SIZES)
            {
                return flags;
            }

            std::set<EdgeKey<false>> overlappingRectangles{};
            ComputeOverlappingRectangles(positions, edges, overlappingRectangles);
            for (auto const& key : overlappingRectangles)
            {
                ...
                // intersect.
                std::array<std::size_t, 2> e0 = edges[key[0]];
                std::array<std::size_t, 2> e1 = edges[key[1]];
                std::array<T, 2> const& p0 = positions[e0[0]];
                std::array<T, 2> const& p1 = positions[e0[1]];
                std::array<T, 2> const& q0 = positions[e1[0]];
                std::array<T, 2> const& q1 = positions[e1[1]];
        ...
        inline std::vector<std::vector<std::size_t>> const& GetDuplicatedPositions() const
        {
            return mDuplicatedPositions;
        }

        inline std::vector<std::vector<std::size_t>> const& GetDuplicatedEdges() const
        {
            return mDuplicatedEdges;
        }

        inline std::vector<std::size_t> const& GetDegenerateEdges() const
        {
            return mDegenerateEdges;
        }

        inline std::vector<std::size_t> const& GetEdgesWithInvalidVertices() const
        {
            return mEdgesWithInvalidVertices;
        }

        inline std::vector<EdgeKey<false>> const& GetInvalidIntersections() const
        {
    ...
    private:
        ...
        public:
            ...
            std::size_t type;   // '0' if interval min, '1' if interval max.
        ...
        std::uint32_t IsValidTopology(std::vector<std::array<T, 2>> const& positions,
            std::vector<std::array<std::size_t, 2>> const& edges)
        {
            std::size_t const numPositions = positions.size();
            std::size_t const numEdges = edges.size();
            ...
            std::uint32_t flags = IPG_IS_PLANAR_GRAPH;
            std::map<std::array<T, 2>, std::vector<std::size_t>> uniquePositions;
            for (std::size_t i = 0; i < numPositions; ++i)
            {
                std::array<T, 2> p = positions[i];
            ...
            std::map<EdgeKey<false>, std::vector<std::size_t>> uniqueEdges;
            for (std::size_t i = 0; i < numEdges; ++i)
            {
                EdgeKey<false> key(edges[i][0], edges[i][1]);
            ...
            for (std::size_t i = 0; i < numEdges; ++i)
            {
                std::array<std::size_t, 2> e = edges[i];
                if (e[0] == e[1])
                {
        ...
        void ComputeOverlappingRectangles(std::vector<std::array<T, 2>> const& positions,
            std::vector<std::array<std::size_t, 2>> const& edges,
            std::set<EdgeKey<false>>& overlappingRectangles) const
        {
            // Compute axis-aligned bounding rectangles for the edges.
            std::size_t const numEdges = edges.size();
            std::vector<std::array<T, 2>> emin(numEdges);
            std::vector<std::array<T, 2>> emax(numEdges);
            for (std::size_t i = 0; i < numEdges; ++i)
            {
                std::array<std::size_t, 2> e = edges[i];
                std::array<T, 2> const& p0 = positions[e[0]];
                std::array<T, 2> const& p1 = positions[e[1]];
            ...
            for (std::size_t i = 0; i < numEndpoints; ++i)
            {
                ...
                if (endpoint.type == 0)  // an interval 'begin' value
                {
                    ...
                    for (auto activeIndex : active)
                    {
                        ...
                        std::array<T, 2> const& r0min = emin[activeIndex];
                        std::array<T, 2> const& r0max = emax[activeIndex];
                        std::array<T, 2> const& r1min = emin[index];
                        std::array<T, 2> const& r1max = emax[index];
                        if (r0max[1] >= r1min[1] && r0min[1] <= r1max[1])
                        {
                            if (activeIndex < index)
                            {
                                overlappingRectangles.insert(EdgeKey<false>(activeIndex, index));
                            }
                            else
                            {
                                overlappingRectangles.insert(EdgeKey<false>(index, activeIndex));
                            }
                        }
                    }
                    active.insert(index);
        ...
        bool InvalidSegmentIntersection(
            std::array<T, 2> const& p0, std::array<T, 2> const& p1,
            std::array<T, 2> const& q0, std::array<T, 2> const& q1) const
        {
            ...
            std::array<T, 2> p1mp0{}, q1mq0{}, q0mp0{};
            for (std::size_t j = 0; j < 2; ++j)
            {
                p1mp0[j] = p1[j] - p0[j];
                q1mq0[j] = q1[j] - q0[j];
            ...
            T denom = p1mp0[0] * q1mq0[1] - p1mp0[1] * q1mq0[0];
            T numer0 = q0mp0[0] * q1mq0[1] - q0mp0[1] * q1mq0[0];
            T numer1 = q0mp0[0] * p1mp0[1] - q0mp0[1] * p1mp0[0];
            ...
            if (denom != zero)
            {
                ...
                if (denom > zero)
                {
                    if (zero <= numer0 && numer0 <= denom && zero <= numer1 && numer1 <= denom)
                    {
                        // The segments intersect.
                        return (numer0 != zero && numer0 != denom) || (numer1 != zero && numer1 != denom);
                    }
                    else
                    {
                        return false;
            ...
            else
            {
                ...
                else
                {
                    ...
                    T sqrLenP1mP0 = p1mp0[0] * p1mp0[0] + p1mp0[1] * p1mp0[1];
                    T value0 = q0mp0[0] * p1mp0[0] + q0mp0[1] * p1mp0[1];
                    T value1 = q1mp0[0] * p1mp0[0] + q1mp0[1] * p1mp0[1];
                    if ((value0 >= sqrLenP1mP0 && value1 >= sqrLenP1mP0) ||
                        (value0 <= zero && value1 <= zero))
                    {
        ...
        std::vector<std::vector<std::size_t>> mDuplicatedPositions;
        std::vector<std::vector<std::size_t>> mDuplicatedEdges;
        std::vector<std::size_t> mDegenerateEdges;
        std::vector<std::size_t> mEdgesWithInvalidVertices;
        std::vector<EdgeKey<false>> mInvalidIntersections;

    private:
        friend class UnitTestIsPlanarGraph;
    };
}
