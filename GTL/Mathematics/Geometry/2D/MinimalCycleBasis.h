...
namespace gtl
{
    ...
    public:
        ...
        using Filament = std::vector<std::size_t>;
        ...
        {
            static_assert(
                std::is_floating_point<T>::value && !is_arbitrary_precision<T>::value,
        ...
        inline std::vector<std::size_t> const& GetIsolatedVertices() const
        {
    ...
    private:
        ...
        static std::int32_t GetSignDet(RPosition const& rInput0, RPosition const& rInput1)
        {
        ...
        struct Vertex
        {
            Vertex(std::size_t inIndex = 0, Position const* inPosition = nullptr)
                :
                index(inIndex),
                position(inPosition),
                adjacents{},
            ...
            mutable std::uint32_t rPositionComputed;
        ...
        std::shared_ptr<Tree> ExtractCycleFromClosedWalk(std::vector<Vertex*>& closedWalk)
        {
            ...
            std::map<Vertex*, std::size_t> duplicates{};
            std::set<std::size_t> detachments{};
            std::size_t numClosedWalk = closedWalk.size();
            for (std::size_t i = 1; i < numClosedWalk - 1; ++i)
            {
                ...
                // closedWalk[i].
                std::size_t iMin = diter->second, iMax = i;
                detachments.insert(iMin);
                for (std::size_t j = iMin + 1; j < iMax; ++j)
                {
            ...
            if (numClosedWalk > 3)
            {
                ...
                for (auto i : detachments)
                {
                    ...
                    for (auto vertex : orgVertex->adjacents)
                    {
                        ...
                        std::int32_t signDet0 = GetSignDet(rDVer, rDMin);
                        std::int32_t signDet1 = GetSignDet(rDVer, rDMax);
                        bool containsVertex{};
                        if (isConvex)
                        {
        ...
        std::vector<std::size_t> ExtractCycle(std::vector<Vertex*>& closedWalk)
        {
            ...
            std::vector<std::size_t> cycle(closedWalk.size());
            for (std::size_t i = 0; i < closedWalk.size(); ++i)
            {
        ...
        Vertex* GetClockwiseMost(Vertex* vPrev, Vertex* vCurr) const
        {
            ...
            for (auto vAdj : vCurr->adjacents)
            {
                ...
                std::int32_t signDet0 = GetSignDet(rDCurr, rDAdj);
                std::int32_t signDet1 = GetSignDet(rDNext, rDAdj);
                if (vCurrConvex)
                {
        ...
        std::vector<std::size_t> mIsolatedVertices;
        std::vector<Filament> mFilaments;
        Forest mForest;

    private:
        friend class UnitTestMinimalCycleBasis;
    };
}
