...
namespace gtl
{
    ...
    public:
        ...
        static std::size_t constexpr NumWords = std::is_same<InputType, float>::value ? 342 : 2561;
        ...
        struct Candidate
        {
            Candidate()
                ...
                N{},
                M{},
                f00(C_<ComputeType>(0)),
                f10(C_<ComputeType>(0)),
                f01(C_<ComputeType>(0)),
                f11(C_<ComputeType>(0)),
                levelCurveProcessorIndex(invalidIndex),
                axis{
                    VCompute3{ C_<ComputeType>(1), C_<ComputeType>(0), C_<ComputeType>(0) },
                    VCompute3{ C_<ComputeType>(0), C_<ComputeType>(1), C_<ComputeType>(0) },
                    VCompute3{ C_<ComputeType>(0), C_<ComputeType>(0), C_<ComputeType>(1) }
                },
                minSupportIndex{ invalidIndex, invalidIndex, invalidIndex },
                maxSupportIndex{ invalidIndex, invalidIndex, invalidIndex },
                volume(C_<ComputeType>(0))
            {
            }
    ...
    public:
        ...
        MinimumVolumeBox3(std::size_t numThreads = 0)
            ...
            mDomainIndex{},
            mZero(C_<ComputeType>(0)),
            mOne(C_<ComputeType>(1)),
            mHalf(C_<ComputeType>(1, 2)),
            mNumVertices(0),
            mNumTriangles(0),
            mAdjacentPool{},
            mVertexAdjacent{},
        ...
        // box is determined by the dimension of the hull. The returned
        // std::size_t is the dimension. The interpretation is
        //   0: The hull is a single point. The box has center at that
        //      point, the axes are the standard Euclidean basis, and the
        //      extents are all 0.
        //   1: The hull is a line segment. The box has center at the
        ...
        //   2: The hull is a planar polygon. The box is the minimum-area
        ...
        //   3: The hull is a convex polyhedron. The other operator()(*)
        ...
        {
            GTL_ARGUMENT_ASSERT(
                numPoints > 0 && points != nullptr && lgMaxSample >= 2,
                "Invalid argument.");

            InputType const zero = static_cast<InputType>(0);
            InputType const one = static_cast<InputType>(1);
            InputType const half = static_cast<InputType>(1, 2);

            mConvexHull3(numPoints, points, 0);
            std::size_t dimension = mConvexHull3.GetDimension();
            ...
            if (dimension == 1)
            {
                ...
                ComputeOrthonormalBasis(1, box.axis[0], box.axis[1], box.axis[2]);
                volume = zero;
                return 1;
            }
            ...
            if (dimension == 2)
            {
                ...
                std::array<Vector3<InputType>, 3> basis{};
                basis[0] = normal;
            ...
            for (std::size_t i = 0; i < numPoints; ++i)
            {
                inVertices[i] = points[i];
            }
            auto const& triangles = mConvexHull3.GetHull();
            std::vector<std::size_t> inIndices(triangles.size());
            std::size_t current = 0;
            for (auto index : triangles)
            {
                inIndices[current++] = index;
            }

            UniqueVerticesSimplices<Vector3<InputType>, std::size_t, 3> uvt{};
            ...
            operator()(outVertices.size(), outVertices.data(),
                outIndices.size(), outIndices.data(), lgMaxSample, box, volume);
            ...
            InputType& volume)
        {
            GTL_ARGUMENT_ASSERT(
                ...
                {
                    GTL_ARGUMENT_ASSERT(0 <= inIndices[i] && inIndices[i] < numVertices,
                        "Invalid index.");
                }
                ...
                std::vector<std::size_t> indices(numIndices);
        ...
        // Returns the convex hull that was computed for computing the
        // minimum-volume box. Note that convex hull is not computed when
        // using operator()(...std::size_t const* inIndices...) because that
        // overload uses the provided convex hull.
    ...
    protected:
            ...
            std::vector<std::size_t> const& indices)
        {
            ...
            std::size_t const* current = indices.data();
            for (std::size_t t = 0; t < mNumTriangles; ++t)
            {
            ...
            std::map<std::size_t, DynamicVETManifoldMesh::Vertex*> sortedVMap{};
            ...
            std::map<DynamicETManifoldMesh::Edge*, std::size_t> edgeIndexMap{};
            std::size_t index = 0;
            for (auto const& element : emap)
            {
                ...
                {
                    mEdges[index].V[j] = (std::size_t)element.second->V[j];
                }
                ++index;
            }

            std::map<DynamicETManifoldMesh::Triangle*, std::size_t> triangleIndexMap{};
            index = 0;
            for (auto const& element : tmap)
            {
                ...
                {
                    mTriangles[index].V[j] = (std::size_t)element.second->V[j];
                }
                ++index;
            }
            ...
            std::size_t const numEdges = mEdges.size();
            mEdgeIndices.reserve(mEdges.size() * mEdges.size());
            for (std::size_t e0 = 0; e0 < numEdges; ++e0)
            {
                for (std::size_t e1 = e0 + 1; e1 < numEdges; ++e1)
                {
        ...
        void PrepareVerticesAndNormals(std::vector<Vector3<InputType>> const& vertices)
        {
            ...
            for (std::size_t j = 0; j < 3; ++j)
            {
                ...
                mVertices[0][j] = C_<ComputeType>(0);
            }
            for (std::size_t i = 1; i < mNumVertices; ++i)
            {
                for (std::size_t j = 0; j < 3; ++j)
                {
        ...
        std::size_t GetExtreme(VCompute3 const& direction, ComputeType& dMax)
        {
        ...
        void ProcessEdgePair(std::array<std::size_t, 2> const& edgeIndex, Candidate& mvCandidate)
        {
            ...
            std::size_t bits00 = (candidate.f00 > mZero ? 1 : (candidate.f00 < mZero ? 2 : 0));
            std::size_t bits10 = (candidate.f10 > mZero ? 1 : (candidate.f10 < mZero ? 2 : 0));
            std::size_t bits01 = (candidate.f01 > mZero ? 1 : (candidate.f01 < mZero ? 2 : 0));
            std::size_t bits11 = (candidate.f11 > mZero ? 1 : (candidate.f11 < mZero ? 2 : 0));
            std::size_t index = bits00 | (bits10 << 2) | (bits01 << 4) | (bits11 << 6);
        ...
        void GetMinimumVolumeBox(OrientedBox3<InputType>& box, InputType& volume)
        {
            ...
            Vector3<RationalType> average = C_<RationalType>(1, 2) * (pmax + pmin);
            for (std::size_t i = 0; i < 3; ++i)
            {
                for (std::size_t j = 0; j < 3; ++j)
                {
            ...
            mRBox.scaledExtent = C_<RationalType>(1, 2) * difference;
        ...
        std::size_t mNumVertices, mNumTriangles;
        std::vector<std::size_t> mAdjacentPool;
        std::vector<std::size_t> mVertexAdjacent;
        ...
        // Maintain the convex hull as a member so that it can be accessed by
        // a user of MinimumVolumeBox3.
    ...
    private:
        friend class UnitTestMinimumVolumeBox3;
    };
}
