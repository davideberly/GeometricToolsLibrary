...
namespace gtl
{
    ...
    public:
        ...
        bool operator()(std::vector<Vector3<InputType>> const& points, Sphere3<InputType>& minimal)
        {
            GTL_ARGUMENT_ASSERT(
                points.size() >= 1,
                "Invalid input.");
            ...
            std::array<std::function<UpdateResult(std::size_t)>, 5> update{};
            update[0] = [this](std::size_t) { return std::make_pair(Sphere3<ComputeType>{}, false); };
            ...
            update[4] = [this](std::size_t i) { return UpdateSupport4(i); };
            ...
            for (std::size_t i = 0; i < numPoints; ++i)
            {
                ...
                {
                    mComputePoints[i][j] = static_cast<ComputeType>(points[permuted[i]][j]);
                }
            }
            ...
            // the new sphere (or near the sphere boundary) because they were
            // enclosed by the previous sphere. The chances are better that
                ...
                if (!SupportContains(i))
                {
                    if (!Contains(i, ctMinimal))
                    {
                        ...
                        else
                        {
                            ...
                            // occurred. Returning non-minimal sphere. TODO:
                            // Should we throw an exception?
                            ContSphere3<InputType>::GetContainer(points, minimal);
                            mNumSupport = 0;
                            mSupport.fill(0);
                            return false;
                        }
        ...
        inline std::array<std::size_t, 4> const& GetSupport() const
        {
            return mSupport;
        }
    ...
    private:
        ...
        // squared radius.
        bool Contains(std::size_t i, Sphere3<ComputeType> const& sphere) const
        {
        ...
        Sphere3<ComputeType> ExactSphere1(std::size_t i0) const
        {
        ...
        Sphere3<ComputeType> ExactSphere2(std::size_t i0, std::size_t i1) const
        {
        ...
        Sphere3<ComputeType> ExactSphere3(std::size_t i0, std::size_t i1, std::size_t i2) const
        {
        ...
        Sphere3<ComputeType> ExactSphere4(std::size_t i0, std::size_t i1, std::size_t i2, std::size_t i3) const
        {
            ...
            Vector3<ComputeType> B{
                C_<ComputeType>(1, 2) * A(0, 0),
                C_<ComputeType>(1, 2) * A(1, 1),
                C_<ComputeType>(1, 2) * A(2, 2) };

            Sphere3<ComputeType> minimal{};
            Vector3<ComputeType> X{};
            if (LinearSystem<ComputeType>::Solve(A, B, X))
            {
                ComputeType x3 = C_<ComputeType>(1) - X[0] - X[1] - X[2];
        ...
        UpdateResult UpdateSupport2(std::size_t i)
        {
            ...
            std::size_t iSphere = 0, iMinRSqr = std::numeric_limits<std::size_t>::max();
            std::size_t k0{}, k1{};

            // Permutations of type 2.
            for (std::size_t j = 0; j < numType2; ++j, ++iSphere)
            {
            ...
            default:
                ...
                // a simple bounding sphere for the result and terminate the
        ...
        UpdateResult UpdateSupport3(std::size_t i)
        {
            ...
            std::size_t constexpr numType4 = 1;  // {0, 1, 2, 3}
            ...
            // Permutations of type 3.
            for (std::size_t j = 0; j < numType3; ++j, ++iSphere)
            {
        ...
        UpdateResult UpdateSupport4(std::size_t i)
        {
            // Permutations of type 2, used for calling ExactSphere2(...).
            std::size_t constexpr numType2 = 4;
            std::array<std::array<std::size_t, 4>, numType2> type2 =
            { {
                { 0, /*4*/ 1, 2, 3 },
                { 1, /*4*/ 0, 2, 3 },
                { 2, /*4*/ 0, 1, 3 },
            ...
            // Permutations of type 3, used for calling ExactSphere3(...).
            std::size_t constexpr numType3 = 6;
            std::array<std::array<std::size_t, 4>, numType3> type3 =
            { {
                { 0, 1, /*4*/ 2, 3 },
                { 0, 2, /*4*/ 1, 3 },
                { 0, 3, /*4*/ 1, 2 },
            ...
            // Permutations of type 4, used for calling ExactSphere4(...).
            std::size_t constexpr numType4 = 4;
            std::array<std::array<std::size_t, 4>, numType4> type4 =
            { {
                { 0, 1, 2, /*4*/ 3 },
                { 0, 1, 3, /*4*/ 2 },
                { 0, 2, 3, /*4*/ 1 },
            ...
            // Permutations of type 4.
            for (std::size_t j = 0; j < numType4; ++j, ++iSphere)
            {
        ...
        std::array<std::size_t, 4> mSupport;
    ...
    private:
        friend class UnitTestMinimumVolumeSphere3;
    };
}
