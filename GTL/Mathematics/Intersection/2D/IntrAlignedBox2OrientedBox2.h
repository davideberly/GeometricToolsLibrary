...
// but only one is reported. The value is 0 when box0.axis[0] separates, 1
// when box0.axis[1] separates, 2 when box1.axis[0] separates, or 3 when
...
namespace gtl
{
    ...
    public:
        struct Output
        {
            ...
            bool intersect;
            std::int32_t separating;
        };

        Output operator()(AlignedBox2<T> const& box0, OrientedBox2<T> const& box1)
        {
            Output output{};
    ...
    private:
        friend class UnitTestIntrAlignedBox2OrientedBox2;
    };
}
