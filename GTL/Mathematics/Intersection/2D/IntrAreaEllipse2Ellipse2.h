...
namespace gtl
{
    ...
    public:
        ...
        struct Output
        {
            ...
            typename EIQuery::Output findOutput;
        ...
        Output operator()(Ellipse2<T> const& ellipse0, Ellipse2<T> const& ellipse1)
        {
            ...
            Output ar{};
            ar.configuration = Output::Configuration::INVALID;
            ar.findOutput = EIQuery{}(ellipse0, ellipse1);
            ar.area = C_<T>(0);
    ...
    private:
        ...
        void FinishEllipseInfo(EllipseInfo& E)
        {
            ...
            E.halfAB = C_<T>(1, 2) * E.AB;
            E.BpA = E.extent[1] + E.extent[0];
            E.BmA = E.extent[1] - E.extent[0];
        }

        void AreaDispatch(EllipseInfo const& E0, EllipseInfo const& E1, Output& ar)
        {
            if (ar.findOutput.intersect)
            {
                if (ar.findOutput.numPoints == 1)
                {
                    // Containment or separation.
                    AreaCS(E0, E1, ar);
                }
                else if (ar.findOutput.numPoints == 2)
                {
                    if (ar.findOutput.isTransverse[0])
                    {
                ...
                else if (ar.findOutput.numPoints == 3)
                {
                    ...
                    else if (!ar.findOutput.isTransverse[1])
                    {
                        Area2(E0, E1, 2, 0, ar);
                    }
                    else  // ar.findOutput.isTransverse[2] == false
                    {
                        Area2(E0, E1, 0, 1, ar);
                    }
                }
                else  // ar.findOutput.numPoints == 4
                {
                    Area4(E0, E1, ar);
                }
            }
        ...
        void AreaCS(EllipseInfo const& E0, EllipseInfo const& E1, Output& ar)
        {
            if (ar.findOutput.numPoints <= 1)
            {
                ...
                if (qform0 > C_<T>(1) && qform1 > C_<T>(1))
                {
                    ...
                    ar.configuration = Output::Configuration::ELLIPSES_ARE_SEPARATED;
                    ar.area = C_<T>(0);
                }
                else
                {
                    ...
                    if (E0.AB < E1.AB)
                    {
                        ar.configuration = Output::Configuration::E1_CONTAINS_E0;
                        ar.area = C_PI<T> * E0.AB;
                    }
                    else
                    {
                        ar.configuration = Output::Configuration::E0_CONTAINS_E1;
                        ar.area = C_PI<T> * E1.AB;
                    }
                }
            }
            else
            {
                ar.configuration = Output::Configuration::ELLIPSES_ARE_EQUAL;
                ar.area = C_PI<T> * E0.AB;
            }
        }

        void Area2(EllipseInfo const& E0, EllipseInfo const& E1, std::size_t i0, std::size_t i1, Output& ar)
        {
            ar.configuration = Output::Configuration::ONE_CHORD_REGION;

            // The endpoints of the chord.
            Vector2<T> const& P0 = ar.findOutput.points[i0];
            Vector2<T> const& P1 = ar.findOutput.points[i1];
        ...
        void Area4(EllipseInfo const& E0, EllipseInfo const& E1, Output& ar)
        {
            ar.configuration = Output::Configuration::FOUR_CHORD_REGION;
            ...
            // SpecialIntersection.]
            std::multimap<T, std::size_t> ordering{};
            std::size_t i{};
            for (i = 0; i < 4; ++i)
            {
                Vector2<T> PmC = ar.findOutput.points[i] - E0.center;
            ...
            std::array<std::size_t, 4> permute{ 0, 0, 0, 0 };
            i = 0;
            ...
            Vector2<T> diag20 =
                ar.findOutput.points[permute[2]] - ar.findOutput.points[permute[0]];
            Vector2<T> diag31 =
                ar.findOutput.points[permute[3]] - ar.findOutput.points[permute[1]];
            ar.area = C_<T>(1, 2) * std::fabs(DotPerp(diag20, diag31));
            ...
            for (std::size_t i0 = 3, i1 = 0; i1 < 4; i0 = i1++)
            {
                // Get a pair of consecutive points.
                Vector2<T> const& P0 = ar.findOutput.points[permute[i0]];
                Vector2<T> const& P1 = ar.findOutput.points[permute[i1]];
        ...
        T ComputeAreaChordRegion(EllipseInfo const& E, Vector2<T> const& P0mC, Vector2<T> const& P1mC)
        {
            ...
            T triArea = C_<T>(1, 2) * std::fabs(DotPerp(P0mC, P1mC));
            ...
            else
            {
                ...
                theta0 += C_TWO_PI<T>;  // ensure theta0 > theta1
                F0 = ComputeIntegral(E, theta0);
                F1 = ComputeIntegral(E, theta1);
                sectorArea = F0 - F1;
                return C_PI<T> * E.AB - (sectorArea - triArea);
            }
        }

        T ComputeIntegral(EllipseInfo const& E, T const& theta)
        {
            T twoTheta = C_<T>(2) * theta;
