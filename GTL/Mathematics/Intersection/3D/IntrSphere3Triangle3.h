...
namespace gtl
{
    ...
    public:
        ...
        typename std::enable_if<!is_arbitrary_precision<Dummy>::value, Output>::type
            ...
            Triangle3<T> const& triangle, Vector3<T> const& triangleVelocity)
        {
            ...
            if (sqrLenV == C_<T>(0))
            {
                ...
                // The 'output' is already set to the correct state for this
                // case.
                return output;
            }
            ...
            if (dotUDelta0 >= sphere.radius)
            {
                ...
                if (dotUV >= C_<T>(0))
                {
                    ...
                    // plane of the triangle. The 'output' is already set to
                ...
                if (foundContact)
                {
                    ...
                    output.contactTime = tbar;
                    output.contactPoint = sphere.center + tbar * sphereVelocity;
                    return output;
                }
            }
            ...
            for (std::size_t im1 = 2, i = 0; i < 3; im1 = i++)
            {
                ...
                if (sqrLenHatV > C_<T>(0))
                {
                    ...
                    if (alpha >= C_<T>(0))
                    {
                        ...
                        if (beta >= C_<T>(0))
                        {
                            ...
                            if (mu + omega * tbar >= C_<T>(0))
                            {
                                if (del[i] + nu[i] * tbar >= C_<T>(0))
                                {
                                    if (delp[i] + nu[i] * tbar <= C_<T>(0))
                                    {
            ...
            for (std::size_t im1 = 2, i = 0; i < 3; im1 = i++)
            {
                ...
                if (alpha >= C_<T>(0))
                {
                    ...
                    if (beta >= C_<T>(0))
                    {
                        ...
                        if (delp[im1] + nu[im1] * tbar >= C_<T>(0))
                        {
                            if (del[i] + nu[i] * tbar <= C_<T>(0))
                            {
            ...
            // and triangle do not come into contact. The 'output' is already
        ...
        struct ExactResult
        {
            ...
            //   Output::Convert(output.contactTime, contactTime);
            //   Output::Convert(output.contactPoint, contactPoint);
            ...
            Triangle3<T> const& triangle, Vector3<T> const& triangleVelocity)
        {
            ...
            // Determine where the sphere center is located relative to the
            // planes of the triangle offset faces of the sphere-swept volume.
            QFN1 const qfzero(C_<T>(0), C_<T>(0), sqrLenU);
            ...
            if (element >= qfzero)
            {
                ...
                {
                    // The sphere is moving away from, or parallel to, the
                    // plane of the triangle. The 'output' is already set
                ...
                {
                    output.intersectionType = 1;
                    output.contactTime.x = -element.x / dotUV;
                    output.contactTime.y = -element.y / dotUV;
                    for (std::size_t j = 0; j < 3; ++j)
                    {
                        output.contactPoint[j].x = sphere.center[j] + output.contactTime.x * sphereVelocity[j];
                        output.contactPoint[j].y = output.contactTime.y * sphereVelocity[j];
                    }
                    return output;
                }
            }
            ...
            for (std::size_t i = 2, ip1 = 0; ip1 < 3; i = ip1++)
            {
                ...
                if (sqrLenHatV > C_<T>(0))
                {
                    ...
                    if (alpha >= C_<T>(0))
                    {
                        ...
                        if (beta >= C_<T>(0))
                        {
                            QFN1 const qfzero(C_<T>(0), C_<T>(0), beta);
                            T mu = Dot(ExU[i], Delta[i]);
                            T omega = Dot(ExU[i], hatV);
                            ...
                            if (arg0 >= qfzero)
                            {
                                ...
                                if (arg1 >= qfzero)
                                {
                                    ...
                                    {
                                        output.intersectionType = 1;
                                        output.contactTime.x = alpha / sqrLenHatV;
                                        output.contactTime.y = (T)-1 / sqrLenHatV;
                                        for (std::size_t j = 0; j < 3; ++j)
                                        {
                                            output.contactPoint[j].x = sphere.center[j] + output.contactTime.x * sphereVelocity[j];
                                            output.contactPoint[j].y = output.contactTime.y * sphereVelocity[j];
                ...
                if (alpha >= C_<T>(0))
                {
                    ...
                    if (beta >= C_<T>(0))
                    {
                        ...
                        if (arg0 >= qfzero)
                        {
                            ...
                            {
                                output.intersectionType = 1;
                                output.contactTime.x = alpha / sqrLenV;
                                output.contactTime.y = (T)-1 / sqrLenV;
                                for (std::size_t j = 0; j < 3; ++j)
                                {
                                    output.contactPoint[j].x = sphere.center[j] + output.contactTime.x * sphereVelocity[j];
                                    output.contactPoint[j].y = output.contactTime.y * sphereVelocity[j];
    ...
    private:
        friend class UnitTestIntrSphere3Triangle3;
    };
}
