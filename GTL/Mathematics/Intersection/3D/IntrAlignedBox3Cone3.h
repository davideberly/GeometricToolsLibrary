...
namespace gtl
{
    ...
    public:
            ...
            mConfiguration{}
        {
            ...
            // Clear the edges.
            for (std::size_t r = 0; r < MAX_CANDIDATE_EDGES; ++r)
            {
            ...
            for (std::size_t r = 0; r < MAX_VERTICES; ++r)
            {
                for (std::size_t c = 0; c < MAX_VERTICES; ++c)
                {
        ...
        Output operator()(AlignedBox3<T> const& box, Cone3<T> const& cone)
        {
            Output output{};
            ...
            // computed.
            T boxMinHeight = C_<T>(0);
            T boxMaxHeight = C_<T>(0);
            ...
            {
                output.intersect = CandidatesHavePointInsideCone(cone);
                return output;
            }
    ...
    protected:
        ...
        static std::size_t constexpr NUM_BOX_VERTICES = 8;
        static std::size_t constexpr NUM_BOX_EDGES = 12;
        static std::size_t constexpr NUM_BOX_FACES = 6;
        static std::size_t constexpr MAX_VERTICES = 32;
        static std::size_t constexpr VERTEX_MIN_BASE = 8;
        static std::size_t constexpr VERTEX_MAX_BASE = 20;
        static std::size_t constexpr MAX_CANDIDATE_EDGES = 496;
        static std::size_t constexpr NUM_CONFIGURATIONS = 81;
        ...
        // with v0 < v1.
        std::array<std::array<std::size_t, 2>, NUM_BOX_EDGES> mEdges;
        ...
        std::size_t mNumCandidateEdges;
        std::array<std::array<std::size_t, 2>, MAX_CANDIDATE_EDGES> mCandidateEdges;
        ...
        std::array<std::array<std::size_t, MAX_VERTICES>, MAX_VERTICES> mAdjacencyMatrix;

        using ConfigurationFunction = void (TIQuery::*)(std::size_t, Face const&);
            ...
            T& boxMinHeight, T& boxMaxHeight)
        {
            ...
            Vector3<T> const& V = cone.vertex;
            Vector3<T> const& U = cone.direction;
            Vector3<T> CmV = C - V;
            T DdCmV = Dot(U, CmV);
        ...
        static bool ConeAxisIntersectsBox(AlignedBox3<T> const& box, Cone3<T> const& cone)
        {
            ...
            {
                Segment3<T> segment{};
                segment.p[0] = cone.vertex + cone.GetMinHeight() * cone.direction;
                segment.p[1] = cone.vertex + cone.GetMaxHeight() * cone.direction;
                auto sbOutput = TIQuery<T, Segment3<T>, AlignedBox3<T>>()(segment, box);
                if (sbOutput.intersect)
                {
                    return true;
                }
            }
            ...
            {
                Ray3<T> ray{};
                ray.origin = cone.vertex + cone.GetMinHeight() * cone.direction;
                ray.direction = cone.direction;
                auto rbOutput = TIQuery<T, Ray3<T>, AlignedBox3<T>>()(ray, box);
        ...
        bool BoxFullyInConeSlab(AlignedBox3<T> const& box, T boxMinHeight, T boxMaxHeight, Cone3<T> const& cone)
        {
            ...
            for (std::size_t i = 0; i < NUM_BOX_VERTICES; ++i)
            {
                mVertices[i] -= cone.vertex;
            }
        ...
        {
            for (std::size_t i = 0; i < mNumCandidateEdges; ++i)
            {
        ...
        void ComputeCandidatesOnBoxEdges(Cone3<T> const& cone)
        {
            for (std::size_t i = 0; i < NUM_BOX_VERTICES; ++i)
            {
                T h = Dot(cone.direction, mVertices[i]);
                T coneMaxHeight = (cone.IsFinite() ? cone.GetMaxHeight() : std::numeric_limits<T>::max());
                mProjectionMin[i] = cone.GetMinHeight() - h;
                mProjectionMax[i] = h - coneMaxHeight;
            }

            std::size_t v0 = VERTEX_MIN_BASE, v1 = VERTEX_MAX_BASE;
            for (std::size_t i = 0; i < NUM_BOX_EDGES; ++i, ++v0, ++v1)
            {
                ...
                bool clipMin =
                    (p0Min < C_<T>(0) && p1Min > C_<T>(0)) ||
                    (p0Min > C_<T>(0) && p1Min < C_<T>(0));
                if (clipMin)
                {
                    mVertices[v0] = (p1Min * mVertices[edge[0]] - p0Min * mVertices[edge[1]])
                        / (p1Min - p0Min);
                }
                ...
                bool clipMax =
                    (p0Max < C_<T>(0) && p1Max > C_<T>(0)) || (p0Max > C_<T>(0) && p1Max < C_<T>(0));
                if (clipMax)
                {
                    mVertices[v1] = (p1Max * mVertices[edge[0]] - p0Max * mVertices[edge[1]])
                        / (p1Max - p0Max);
                }
                ...
                else
                {
                    ...
                    if (p0Min <= C_<T>(0) && p1Min <= C_<T>(0) &&
                        p0Max <= C_<T>(0) && p1Max <= C_<T>(0))
                    {
        ...
        {
            T p0{}, p1{}, p2{}, p3{};
            std::size_t b0{}, b1{}, b2{}, b3{}, index{};
            for (std::size_t i = 0; i < NUM_BOX_FACES; ++i)
            {
                ...
                b0 = (p0 < C_<T>(0) ? 0 : (p0 > C_<T>(0) ? 2 : 1));
                b1 = (p1 < C_<T>(0) ? 0 : (p1 > C_<T>(0) ? 2 : 1));
                b2 = (p2 < C_<T>(0) ? 0 : (p2 > C_<T>(0) ? 2 : 1));
                b3 = (p3 < C_<T>(0) ? 0 : (p3 > C_<T>(0) ? 2 : 1));
        ...
        void InsertEdge(std::size_t v0, std::size_t v1)
        {
        ...
        // so on.
        void NNNN_0(std::size_t, Face const&)
        {
        }

        void NNNZ_1(std::size_t, Face const&)
        {
        }

        void NNNP_2(std::size_t base, Face const& face)
        {
            InsertEdge(base + face.e[2], base + face.e[3]);
        }

        void NNZN_3(std::size_t, Face const&)
        {
        }

        void NNZZ_4(std::size_t, Face const&)
        {
        }

        void NNZP_5(std::size_t base, Face const& face)
        {
            InsertEdge(face.v[2], base + face.e[3]);
        }

        void NNPN_6(std::size_t base, Face const& face)
        {
            InsertEdge(base + face.e[1], base + face.e[2]);
        }

        void NNPZ_7(std::size_t base, Face const& face)
        {
            InsertEdge(base + face.e[1], face.v[3]);
        }

        void NNPP_8(std::size_t base, Face const& face)
        {
            InsertEdge(base + face.e[1], base + face.e[3]);
        }

        void NZNN_9(std::size_t, Face const&)
        {
        }

        void NZNZ_10(std::size_t, Face const&)
        {
        }

        void NZNP_11(std::size_t base, Face const& face)
        {
            InsertEdge(base + face.e[2], face.v[3]);
            InsertEdge(base + face.e[3], face.v[3]);
        }

        void NZZN_12(std::size_t, Face const&)
        {
        }

        void NZZZ_13(std::size_t, Face const&)
        {
        }

        void NZZP_14(std::size_t base, Face const& face)
        {
            InsertEdge(face.v[2], face.v[3]);
            InsertEdge(base + face.e[3], face.v[3]);
        }

        void NZPN_15(std::size_t base, Face const& face)
        {
            InsertEdge(base + face.e[2], face.v[1]);
        }

        void NZPZ_16(std::size_t, Face const& face)
        {
            InsertEdge(face.v[1], face.v[3]);
        }

        void NZPP_17(std::size_t base, Face const& face)
        {
            InsertEdge(base + face.e[3], face.v[1]);
        }

        void NPNN_18(std::size_t base, Face const& face)
        {
            InsertEdge(base + face.e[0], base + face.e[1]);
        }

        void NPNZ_19(std::size_t base, Face const& face)
        {
            InsertEdge(base + face.e[0], face.v[1]);
            InsertEdge(base + face.e[1], face.v[1]);
        }

        void NPNP_20(std::size_t base, Face const& face)
        {
            InsertEdge(base + face.e[0], face.v[1]);
            InsertEdge(base + face.e[1], face.v[1]);
            InsertEdge(base + face.e[2], face.v[3]);
        ...
        void NPZN_21(std::size_t base, Face const& face)
        {
            InsertEdge(base + face.e[0], face.v[2]);
        }

        void NPZZ_22(std::size_t base, Face const& face)
        {
            InsertEdge(base + face.e[0], face.v[1]);
            InsertEdge(face.v[1], face.v[2]);
        }

        void NPZP_23(std::size_t base, Face const& face)
        {
            InsertEdge(base + face.e[0], face.v[1]);
            InsertEdge(face.v[1], face.v[2]);
        ...
        void NPPN_24(std::size_t base, Face const& face)
        {
            InsertEdge(base + face.e[0], base + face.e[2]);
        }

        void NPPZ_25(std::size_t base, Face const& face)
        {
            InsertEdge(base + face.e[0], face.v[3]);
        }

        void NPPP_26(std::size_t base, Face const& face)
        {
            InsertEdge(base + face.e[0], base + face.e[3]);
        }

        void ZNNN_27(std::size_t, Face const&)
        {
        }

        void ZNNZ_28(std::size_t, Face const&)
        {
        }

        void ZNNP_29(std::size_t base, Face const& face)
        {
            InsertEdge(base + face.e[2], face.v[0]);
        }

        void ZNZN_30(std::size_t, Face const&)
        {
        }

        void ZNZZ_31(std::size_t, Face const&)
        {
        }

        void ZNZP_32(std::size_t, Face const& face)
        {
            InsertEdge(face.v[0], face.v[2]);
        }

        void ZNPN_33(std::size_t base, Face const& face)
        {
            InsertEdge(base + face.e[1], face.v[2]);
            InsertEdge(base + face.e[2], face.v[2]);
        }

        void ZNPZ_34(std::size_t base, Face const& face)
        {
            InsertEdge(base + face.e[1], face.v[2]);
            InsertEdge(face.v[2], face.v[3]);
        }

        void ZNPP_35(std::size_t base, Face const& face)
        {
            InsertEdge(face.v[0], base + face.e[1]);
        }

        void ZZNN_36(std::size_t, Face const&)
        {
        }

        void ZZNZ_37(std::size_t, Face const&)
        {
        }

        void ZZNP_38(std::size_t base, Face const& face)
        {
            InsertEdge(face.v[0], face.v[3]);
            InsertEdge(face.v[3], base + face.e[2]);
        }

        void ZZZN_39(std::size_t, Face const&)
        {
        }

        void ZZZZ_40(std::size_t, Face const&)
        {
        }

        void ZZZP_41(std::size_t, Face const& face)
        {
            InsertEdge(face.v[0], face.v[3]);
            InsertEdge(face.v[3], face.v[2]);
        }

        void ZZPN_42(std::size_t base, Face const& face)
        {
            InsertEdge(face.v[1], face.v[2]);
            InsertEdge(face.v[2], base + face.e[2]);
        }

        void ZZPZ_43(std::size_t, Face const& face)
        {
            InsertEdge(face.v[1], face.v[2]);
            InsertEdge(face.v[2], face.v[3]);
        }

        void ZZPP_44(std::size_t, Face const&)
        {
        }

        void ZPNN_45(std::size_t base, Face const& face)
        {
            InsertEdge(face.v[0], base + face.e[1]);
        }

        void ZPNZ_46(std::size_t base, Face const& face)
        {
            InsertEdge(face.v[0], face.v[1]);
            InsertEdge(face.v[1], base + face.e[1]);
        }

        void ZPNP_47(std::size_t base, Face const& face)
        {
            InsertEdge(face.v[0], face.v[1]);
            InsertEdge(face.v[1], base + face.e[1]);
            InsertEdge(base + face.e[2], face.v[3]);
        ...
        void ZPZN_48(std::size_t, Face const& face)
        {
            InsertEdge(face.v[0], face.v[2]);
        }

        void ZPZZ_49(std::size_t, Face const& face)
        {
            InsertEdge(face.v[0], face.v[1]);
            InsertEdge(face.v[1], face.v[2]);
        }

        void ZPZP_50(std::size_t, Face const&)
        {
        }

        void ZPPN_51(std::size_t base, Face const& face)
        {
            InsertEdge(face.v[0], base + face.e[2]);
        }

        void ZPPZ_52(std::size_t, Face const&)
        {
        }

        void ZPPP_53(std::size_t, Face const&)
        {
        }

        void PNNN_54(std::size_t base, Face const& face)
        {
            InsertEdge(base + face.e[3], base + face.e[0]);
        }

        void PNNZ_55(std::size_t base, Face const& face)
        {
            InsertEdge(face.v[3], base + face.e[0]);
        }

        void PNNP_56(std::size_t base, Face const& face)
        {
            InsertEdge(base + face.e[2], base + face.e[0]);
        }

        void PNZN_57(std::size_t base, Face const& face)
        {
            InsertEdge(base + face.e[3], face.v[0]);
            InsertEdge(face.v[0], base + face.e[0]);
        }

        void PNZZ_58(std::size_t base, Face const& face)
        {
            InsertEdge(face.v[3], face.v[0]);
            InsertEdge(face.v[0], base + face.e[0]);
        }

        void PNZP_59(std::size_t base, Face const& face)
        {
            InsertEdge(face.v[2], base + face.e[0]);
        }

        void PNPN_60(std::size_t base, Face const& face)
        {
            InsertEdge(base + face.e[3], face.v[0]);
            InsertEdge(face.v[0], base + face.e[0]);
            InsertEdge(base + face.e[1], face.v[2]);
        ...
        void PNPZ_61(std::size_t base, Face const& face)
        {
            InsertEdge(face.v[3], face.v[0]);
            InsertEdge(face.v[0], base + face.e[0]);
            InsertEdge(base + face.e[1], face.v[2]);
        ...
        void PNPP_62(std::size_t base, Face const& face)
        {
            InsertEdge(base + face.e[0], base + face.e[1]);
        }

        void PZNN_63(std::size_t base, Face const& face)
        {
            InsertEdge(base + face.e[3], face.v[1]);
        }

        void PZNZ_64(std::size_t, Face const& face)
        {
            InsertEdge(face.v[3], face.v[1]);
        }

        void PZNP_65(std::size_t base, Face const& face)
        {
            InsertEdge(base + face.e[2], face.v[1]);
        }

        void PZZN_66(std::size_t base, Face const& face)
        {
            InsertEdge(base + face.e[3], face.v[0]);
            InsertEdge(face.v[0], face.v[1]);
        }

        void PZZZ_67(std::size_t, Face const&)
        {
        }

        void PZZP_68(std::size_t, Face const&)
        {
        }

        void PZPN_69(std::size_t base, Face const& face)
        {
            InsertEdge(base + face.e[3], face.v[0]);
            InsertEdge(face.v[0], face.v[1]);
            InsertEdge(face.v[1], face.v[2]);
        ...
        void PZPZ_70(std::size_t, Face const&)
        {
        }

        void PZPP_71(std::size_t, Face const&)
        {
        }

        void PPNN_72(std::size_t base, Face const& face)
        {
            InsertEdge(base + face.e[3], base + face.e[1]);
        }

        void PPNZ_73(std::size_t base, Face const& face)
        {
            InsertEdge(face.v[3], base + face.e[1]);
        }

        void PPNP_74(std::size_t base, Face const& face)
        {
            InsertEdge(base + face.e[2], base + face.e[1]);
        }

        void PPZN_75(std::size_t base, Face const& face)
        {
            InsertEdge(base + face.e[2], face.v[2]);
        }

        void PPZZ_76(std::size_t, Face const&)
        {
        }

        void PPZP_77(std::size_t, Face const&)
        {
        }

        void PPPN_78(std::size_t base, Face const& face)
        {
            InsertEdge(base + face.e[3], base + face.e[2]);
        }

        void PPPZ_79(std::size_t, Face const&)
        {
        }

        void PPPP_80(std::size_t, Face const&)
        {
        }

    private:
        friend class UnitTestIntrAlignedBox3Cone3;
    };
}
