...
// The queries consider the box to be a solid. The test-intersection query
// uses the method of separating axes.
...
namespace gtl
{
    ...
    public:
        struct Output
        {
           Output()
            ...
            bool intersect;
            std::array<std::int32_t, 2> separating;
        };
            ...
            T epsilon = C_<T>(0))
        {
            ...
            epsilon = std::max(epsilon, C_<T>(0));
            T const cutoff = C_<T>(1) - epsilon;
            ...
            if (r > r01)
            {
                ...
                output.separating[0] = 0;
                output.separating[1] = -1;
                return output;
            }
            ...
            {
                // The output.separating[] values are invalid because there is
                // no separation.
            ...
            // The output.separating[] values are invalid because there is no
            // separation.
            output.intersect = true;
            return output;
        }
    ...
    private:
        friend class UnitTestIntrAlignedBox3OrientedBox3;
    };
}
