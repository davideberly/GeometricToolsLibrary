...
// returned, where smax = std::numeric_limits<std::size_t>::max(). If the axis
// is a face normal N[i1], then (smax,i1) is returned. If the axis is a cross
...
namespace gtl
{
    ...
    public:
        struct Output
        {
            Output()
                :
                intersect(false),
                separating{ std::numeric_limits<std::size_t>::max(), std::numeric_limits<std::size_t>::max() }
            {
            }

            bool intersect;
            std::array<std::size_t, 2> separating;
        };
            ...
            T epsilon = C_<T>(0))
        {
            ...
            for (std::size_t i = 0; i < 4; ++i)
            {
                ...
                if (WhichSide(tetra1, P, N) > 0)
                {
                    ...
                    output.separating[0] = i;
                    output.separating[1] = std::numeric_limits<std::size_t>::max();
                    return output;
                }
            }
            ...
            for (std::size_t i = 0; i < 4; ++i)
            {
                ...
                if (WhichSide(tetra0, P, N) > 0)
                {
                    ...
                    // normal line P + t * N.
                    output.intersect = false;
                    output.separating[0] = std::numeric_limits<std::size_t>::max();
                    output.separating[1] = i;
                    return output;
                }
            }
            ...
            // each tetrahedron.
            T const cutoff = std::min(std::max(C_<T>(1) - epsilon, C_<T>(0)), C_<T>(1));
            for (std::size_t i0 = 0; i0 < 6; ++i0)
            {
                ...
                for (std::size_t i1 = 0; i1 < 6; ++i1)
                {
                    ...
                    if (std::fabs(Dot(E0, E1)) < cutoff)
                    {
                        ...
                        std::int32_t side0 = WhichSide(tetra0, P0, N);
                        if (side0 == 0)
                        {
                            continue;
                        }

                        std::int32_t side1 = WhichSide(tetra1, P0, N);
                        if (side1 == 0)
                        {
                            continue;
                        }
                        ...
                        if (side0 * side1 < 0)
                        {
                            ...
                            // projection of P.
                            output.intersect = false;
                            output.separating[0] = i0;
                            output.separating[1] = i1;
                            return output;
                        }
                    }
                }
    ...
    private:
        static std::int32_t WhichSide(Tetrahedron3<T> const& tetra,
    ...
    private:
        friend class UnitTestIntrTetrahedron3Tetrahedron3;
    };
}
