...
{
    template <typename T>
    class TIQuery<T, OrientedBox3<T>, Frustum3<T>>
    {
    public:
        struct Output
        {
        ...
        Output operator()(OrientedBox3<T> const& box, Frustum3<T> const& frustum)
        {
            Output output{};

            // Convenience variables.
            Vector3<T> const* axis = &box.axis[0];
            Vector3<T> const& extent = box.extent;

            Vector3<T> diff = box.center - frustum.origin;  // C-E

            std::array<T, 3> A;      // Dot(R,A[i])
            std::array<T, 3> B;      // Dot(U,A[i])
            std::array<T, 3> C;      // Dot(D,A[i])
            std::array<T, 3> D;      // (Dot(R,C-E),Dot(U,C-E),Dot(D,C-E))
            std::array<T, 3> NA;     // dmin*Dot(R,A[i])
            std::array<T, 3> NB;     // dmin*Dot(U,A[i])
            std::array<T, 3> NC;     // dmin*Dot(D,A[i])
            std::array<T, 3> ND;     // dmin*(Dot(R,C-E),Dot(U,C-E),?)
            std::array<T, 3> RC;     // rmax*Dot(D,A[i])
            std::array<T, 3> RD;     // rmax*(?,?,Dot(D,C-E))
            std::array<T, 3> UC;     // umax*Dot(D,A[i])
            std::array<T, 3> UD;     // umax*(?,?,Dot(D,C-E))
            std::array<T, 3> NApRC;  // dmin*Dot(R,A[i]) + rmax*Dot(D,A[i])
            std::array<T, 3> NAmRC;  // dmin*Dot(R,A[i]) - rmax*Dot(D,A[i])
            std::array<T, 3> NBpUC;  // dmin*Dot(U,A[i]) + umax*Dot(D,A[i])
            std::array<T, 3> NBmUC;  // dmin*Dot(U,A[i]) - umax*Dot(D,A[i])
            std::array<T, 3> RBpUA;  // rmax*Dot(U,A[i]) + umax*Dot(R,A[i])
            std::array<T, 3> RBmUA;  // rmax*Dot(U,A[i]) - umax*Dot(R,A[i])
            ...
            for (i = 0; i < 3; ++i)
            {
                T fRB = frustum.rBound * B[i];
                T fUA = frustum.uBound * A[i];
                RBpUA[i] = fRB + fUA;
                RBmUA[i] = fRB - fUA;
            }
            for (i = 0; i < 3; ++i)
    ...
    private:
        friend class UnitTestIntrOrientedBox3Frustum3;
    };
}
