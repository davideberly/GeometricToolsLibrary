...
namespace gtl
{
    ...
    public:
        ...
        Output operator()(CanonicalBox3<T> const& box, Cylinder3<T> const& cylinder)
        {
            GTL_ARGUMENT_ASSERT(
                cylinder.IsFinite(),
                "Infinite cylinders are not yet supported.");

            // The output.intersect is initially false.
            Output output{};
            ...
            Vector3<T> C = cylinder.center;
            Vector3<T> D = cylinder.direction;
            T r = cylinder.radius;
            T hDiv2 = C_<T>(1, 2) * cylinder.height;
            ...
            if (D[0] > C_<T>(0))
            {
                if (D[1] > C_<T>(0))
                {
                    if (D[2] > C_<T>(0))  // (+,+,+)
                    {
                        output.intersect = DoQueryNoZeros(C, D, r, hDiv2, E);
                    }
                    else  // (+,+,0)
                    {
                        output.intersect = DoQueryOneZero({ 0, 1, 2 }, C, D, r, hDiv2, E);
                    }
                }
                else
                {
                    if (D[2] > C_<T>(0))  // (+,0,+)
                    {
                        output.intersect = DoQueryOneZero({ 2, 0, 1 }, C, D, r, hDiv2, E);
                    }
                    else  // (+,0,0)
                    {
                        output.intersect = DoQueryTwoZeros({ 0, 1, 2 }, C, r, E);
                    }
                }
            }
            else
            {
                if (D[1] > C_<T>(0))
                {
                    if (D[2] > C_<T>(0))  // (0,+,+)
                    {
                        output.intersect = DoQueryOneZero({ 1, 2, 0 }, C, D, r, hDiv2, E);
                    }
                    else  // (0,+,0)
                    {
                        output.intersect = DoQueryTwoZeros({ 1, 2, 0 }, C, r, E);
                    }
                }
                else
                {
                    if (D[2] > C_<T>(0))  // (0,0,+)
                    {
                        output.intersect = DoQueryTwoZeros({ 2, 0, 1 }, C, r, E);
                    }
                    else  // (0,0,0)
                    {
                        GTL_RUNTIME_ERROR("The cylinder direction cannot be (0,0,0).");
                    }
                }
            }
    ...
    private:
            ...
            Cylinder3<T> const& cylinder)
        {
            ...
            Vector3<T> const& C = cylinder.center;
            Vector3<T> const& D = cylinder.direction;
        ...
        static bool DoQueryTwoZeros(std::array<std::size_t, 3> const& i,
        ...
        static bool DoQueryOneZero(std::array<std::size_t, 3> const& i,
            ...
            Vector3<T> const& E)
        {
            ...
            T sMin = C_<T>(0), tHat = d1 * e1mc1 - d0 * e0pc0;
            if (-hDiv2 <= tHat)
            {
                if (tHat <= hDiv2)
                {
            ...
            T sMax = C_<T>(0), tBar = d0 * e0mc0 - d1 * e1pc1;
            if (-hDiv2 <= tBar)
            {
                if (tBar <= hDiv2)
                {
            ...
            T const& r, T const& hDiv2, Vector3<T> const& E)
        {
            ...
            // (U0, -U1)
            lower = (s1p >= C_<T>(0) ? -E[2] : -E[2] - s1p / D[2]);
            upper = (s5n <= C_<T>(0) ? +E[2] : +E[2] - s5n / D[2]);
            if (lower <= upper)
            {
                P0 = { +E[0], -E[1], lower };
                P1 = { +E[0], -E[1], upper };
            ...
            // (U1, -U0)
            lower = (s2p >= C_<T>(0) ? -E[2] : -E[2] - s2p / D[2]);
            upper = (s6n <= C_<T>(0) ? +E[2] : +E[2] - s6n / D[2]);
            if (lower <= upper)
            {
                P0 = { -E[0], +E[1], lower };
                P1 = { -E[0], +E[1], upper };
            ...
            // (U0, -U2)
            lower = (s1p >= C_<T>(0) ? -E[1] : -E[1] - s1p / D[1]);
            upper = (s3n <= C_<T>(0) ? +E[1] : +E[1] - s3n / D[1]);
            if (lower <= upper)
            {
                P0 = { +E[0], lower, -E[2] };
                P1 = { +E[0], upper, -E[2] };
            ...
            // (U2, -U0)
            lower = (s4p >= C_<T>(0) ? -E[1] : -E[1] - s4p / D[1]);
            upper = (s6n <= C_<T>(0) ? +E[1] : +E[1] - s6n / D[1]);
            if (lower <= upper)
            {
                P0 = { -E[0], lower, +E[2] };
                P1 = { -E[0], upper, +E[2] };
            ...
            // (U1, -U2)
            lower = (s2p >= C_<T>(0) ? -E[0] : -E[0] - s2p / D[0]);
            upper = (s3n <= C_<T>(0) ? +E[0] : +E[0] - s3n / D[0]);
            if (lower <= upper)
            {
                P0 = { lower, +E[1], -E[2] };
                P1 = { upper, +E[1], -E[2] };
            ...
            // (U2, -U1)
            lower = (s4p >= C_<T>(0) ? -E[0] : -E[0] - s4p / D[0]);
            upper = (s5n <= C_<T>(0) ? +E[0] : +E[0] - s5n / D[0]);
            if (lower <= upper)
            {
                P0 = { lower, -E[1], +E[2] };
                P1 = { upper, -E[1], +E[2] };
            ...
            // (U0, -D)
            lower = (s3p >= C_<T>(0) ? -E[2] : -E[2] - s3p / D[2]);
            upper = (s5p <= C_<T>(0) ? +E[2] : +E[2] - s5p / D[2]);
            if (lower <= upper)
            {
                if (s3p >= C_<T>(0))
                {
            ...
            // (D, -U0)
            lower = (s2n >= C_<T>(0) ? -E[2] : -E[2] - s2n / D[2]);
            upper = (s4n <= C_<T>(0) ? +E[2] : +E[2] - s4n / D[2]);
            if (lower <= upper)
            {
                if (s2n >= C_<T>(0))
                {
            ...
            // (U1, -D)
            lower = (s6p >= C_<T>(0) ? -E[0] : -E[0] - s6p / D[0]);
            upper = (s3p <= C_<T>(0) ? +E[0] : +E[0] - s3p / D[0]);
            if (lower <= upper)
            {
                if (s6p >= C_<T>(0))
                {
            ...
            // (D, -U1)
            lower = (s4n >= C_<T>(0) ? -E[0] : -E[0] - s4n / D[0]);
            upper = (s1n <= C_<T>(0) ? +E[0] : +E[0] - s1n / D[0]);
            if (lower <= upper)
            {
                if (s4n >= C_<T>(0))
                {
            ...
            // (U2, -D)
            lower = (s5p >= C_<T>(0) ? -E[1] : -E[1] - s5p / D[2]);
            upper = (s6p <= C_<T>(0) ? +E[1] : +E[1] - s6p / D[2]);
            if (lower <= upper)
            {
                if (s5p >= C_<T>(0))
                {
            ...
            // (D, -U2)
            lower = (s1n >= C_<T>(0) ? -E[1] : -E[1] - s1n / D[2]);
            upper = (s2n <= C_<T>(0) ? +E[1] : +E[1] - s2n / D[2]);
            if (lower <= upper)
            {
                if (s1n >= C_<T>(0))
                {
    ...
    private:
        friend class UnitTestIntrCanonicalBox3Cylinder3;
    };
}
