...
namespace gtl
{
    ...
    {
    public:
        bool ExecuteUseGeometry(Triangle3<T> const& triangle, Cylinder3<T> const& cylinder)
        {
            // Get a right-handed orthonormal basis from the cylinder axis
            // direction. The returned value from ComputeOrthonormalBasis is
            // unused, so it is discarded.
            Vector3<T> U0{}, U1{}, U2 = cylinder.direction;
            (void)ComputeOrthonormalBasis(1, U2, U0, U1);
            ...
            {
                Vector3<T> delta = triangle.v[i] - cylinder.center;
                P[i][0] = Dot(U0, delta);  // x[i]
                P[i][1] = Dot(U1, delta);  // y[i]
                P[i][2] = Dot(U2, delta);  // z[i]
            }
            ...
            T const hhalf = C_<T>(1, 2) * cylinder.height;
            if (z[2] < -hhalf)
            {
            ...
            if (-hhalf <= z[0] && z[2] <= hhalf)
            {
                ...
                // Figure 1 of the PDF.
                return DiskOverlapsPolygon(3, Q.data(), radius);
            }
            ...
            if (z[0] < -hhalf)
            {
                if (z[2] > hhalf)
                {
                    if (z[1] >= hhalf)
                    {
                        ...
                        return DiskOverlapsPolygon(4, polygon.data(), radius);
                    }
                    else if (z[1] <= -hhalf)
                    {
                    ...
                    else  // -hhalf < z[1] < hhalf
                    {
                        ...
                        return DiskOverlapsPolygon(5, polygon.data(), radius);
                    }
                }
                else if (z[2] > -hhalf)
                {
                    if (z[1] <= -hhalf)
                    {
                        ...
                        return DiskOverlapsPolygon(3, polygon.data(), radius);
                    }
                    else // z[1] > -hhalf
                    {
    ...
    private:
        ...
        bool DiskOverlapsPolygon(std::size_t numVertices, Vector2<T> const* Q,
            T const& radius) const
        {
    ...
    private:
        friend class UnitTestIntrTriangle3Cylinder3;
    };
}
