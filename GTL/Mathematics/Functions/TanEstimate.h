...
namespace gtl
{
    ...
    inline T TanEstimate(T x)
    {
        ...
        std::size_t constexpr select = ((Degree - 3) / 2);
        auto constexpr& coeff = C_TAN_EST_COEFF[select];
        std::size_t constexpr last = ((Degree - 1) / 2);
        T xsqr = x * x;
    ...
    inline T TanEstimateRR(T x)
    {
        ...
        if (std::fabs(y) <= C_PI_DIV_4<T>)
        {
            return TanEstimate<T, Degree>(y);
        }
        else if (y > C_PI_DIV_4<T>)
        {
            T poly = TanEstimate<T, Degree>(y - C_PI_DIV_4<T>);
            return (C_<T>(1) + poly) / (C_<T>(1) - poly);
        }
        else
        {
            T poly = TanEstimate<T, Degree>(y + C_PI_DIV_4<T>);
