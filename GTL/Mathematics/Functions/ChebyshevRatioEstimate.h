...
namespace gtl
{
    ...
    T constexpr C_CHBRAT_ACOEFF(std::size_t i)
    {
        return (Degree != (i + 1) ? C_<T>(1) : static_cast<T>(C_CHBRAT_EST_U[i]))
            / (static_cast<T>(i + 1) * static_cast<T>(2 * (i + 1) + 1));
    }

    template <typename T, std::size_t Degree>
    T constexpr C_CHBRAT_BCOEFF(std::size_t i)
    {
        return (Degree != (i + 1) ? C_<T>(1) : static_cast<T>(C_CHBRAT_EST_U[i]))
            * static_cast<T>(i + 1) / static_cast<T>(2 * (i + 1) + 1);
    }
    ...
    template <typename T, std::size_t Degree>
    T constexpr C_CHBRAT_ACOEFF_R(std::size_t i)
    {
        return (Degree != (i + 1) ? C_<T>(1) : static_cast<T>(C_CHBRAT_ESTR_U[i]))
            / (static_cast<T>(i + 1)* static_cast<T>(2 * (i + 1) + 1));
    }

    template <typename T, std::size_t Degree>
    T constexpr C_CHBRAT_BCOEFF_R(std::size_t i)
    {
        return (Degree != (i + 1) ? C_<T>(1) : static_cast<T>(C_CHBRAT_ESTR_U[i]))
            * static_cast<T>(i + 1) / static_cast<T>(2 * (i + 1) + 1);
    }
