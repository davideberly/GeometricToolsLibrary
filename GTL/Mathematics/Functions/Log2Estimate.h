...
namespace gtl
{
    ...
    inline T Log2Estimate(T x)
    {
        ...
        std::size_t constexpr select = Degree - 1;
        auto constexpr& coeff = C_LOG2_EST_COEFF[select];
        std::size_t constexpr last = Degree - 1;
        T t = x - C_<T>(1);  // t in [0,1]
        T poly = static_cast<T>(coeff[last]);
#if defined(GTL_USE_MSWINDOWS)
        // For Degree = 1, Visual Studio generates
        //   warning C6294: Ill-defined loop: initial condition does not
        //   satisfy test. Loop body not executed.'
        // By design of the template function, the loop must be skipped
        // because p(t) = coeff[0]*t, in which case 'last' is 0. The value
        // 'index = last - 1' wraps around to the maximum std::size_t number, but
        // this is irrelevant because 'i' is initialized to 0 and 'i < last'
        // is '0 < 0', which is false and the loop is skipped. The optimizing
        // compiler should not generate any code at all for the loop.
#pragma warning(disable: 6294)
#endif
        for (std::size_t i = 0, index = last - 1; i < last; ++i, --index)
        {
