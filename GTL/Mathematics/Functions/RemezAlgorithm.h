...
namespace gtl
{
    ...
    public:
        ...
        std::size_t Execute(Function const& F, Function const& FDer, T const& xMin,
            T const& xMax, std::size_t degree, std::size_t maxRemezIterations,
            std::size_t maxBisectionIterations, std::size_t maxBracketIterations)
        {
            GTL_ARGUMENT_ASSERT(
                xMin < xMax &&
                degree > 0 &&
    ...
    private:
        void ComputeInitialXNodes()
        {
            ...
            std::size_t const numNodes = mXNodes.size();
            T const halfPiDivDegree = C_PI_DIV_2<T> / static_cast<T>(mDegree);
            std::vector<T> cosAngles(numNodes);
            cosAngles.front() = -C_<T>(1);
            for (std::size_t i = 1, j = 2 * mDegree - 1; i <= mDegree; ++i, j -= 2)
            {
                T angle = static_cast<T>(j) * halfPiDivDegree;
                cosAngles[i] = std::cos(angle);
            }
            cosAngles.back() = C_<T>(1);
            if (numNodes & 1)
            {
                // Avoid the rounding errors when the angle is pi/2, where
                // cos(pi/2) is theoretically zero.
                cosAngles[numNodes / 2] = C_<T>(0);
            }

            // Transform the nodes to the interval [xMin, xMax].
            T const center = C_<T>(1, 2) * (mXMax + mXMin);
            T const radius = C_<T>(1, 2) * (mXMax - mXMin);
            mXNodes.front() = mXMin;
            for (std::size_t i = 1; i <= mDegree; ++i)
            {
        ...
        {
            for (std::size_t i = 0; i < mXNodes.size(); ++i)
            {
        ...
        {
            for (std::size_t i = 0; i < mUCoefficients.size(); ++i)
            {
        ...
        {
            T sign = C_<T>(1);
            for (std::size_t i = 0; i < mVCoefficients.size(); ++i)
            {
        ...
        {
            T const powNegOne = ((mDegree & 1) ? C_<T>(-1) : C_<T>(1));
        ...
        void ComputePCoefficients()
        {
            ...
            std::size_t index = mUCoefficients.size() - 1;
            Polynomial1<T> poly{ constant[index--] };
            for (std::size_t i = 1; i < mUCoefficients.size(); ++i, --index)
            {
                Polynomial1<T> linear{ -mXNodes[index], C_<T>(1)};
                poly = constant[index] + linear * poly;
            }

            for (std::size_t i = 0; i < mPCoefficients.size(); ++i)
            {
                mPCoefficients[i] = poly[i];
            }
        }
        ...
        bool IsOscillatory()
        {
            ...
            for (std::size_t i0 = 0, i1 = 1; i1 < mXNodes.size(); i0 = i1++)
            {
                if ((mErrors[i0] > C_<T>(0) && mErrors[i1] > C_<T>(0)) ||
                    (mErrors[i0] < C_<T>(0) && mErrors[i1] < C_<T>(0)))
                {
        ...
        void ComputePartition()
        {
            ...
            for (std::size_t i0 = 0, i1 = 1; i1 < mXNodes.size(); i0 = i1++)
            {
                T x0 = mXNodes[i0], x1 = mXNodes[i1], xMid = C_<T>(0), eMid = C_<T>(0);
                std::int32_t sign0 = (mErrors[i0] > C_<T>(0) ? 1 : -1);
                std::int32_t sign1 = (mErrors[i1] > C_<T>(0) ? 1 : -1);
                std::int32_t signMid{};
                ...
                for (iteration = 0; iteration < mMaxBisectionIterations; ++iteration)
                {
                    ...
                    signMid = (eMid > C_<T>(0) ? 1 : (eMid < C_<T>(0) ? -1 : 0));
                    if (signMid == sign0)
                    {
                        x0 = xMid;
                    }
        ...
        void ComputeXExtremes()
        {
            ...
            nextXNodes[0] = mXMin;
            for (std::size_t i0 = 1, i1 = 2; i0 < mDegree + 1; i0 = i1++)
            {
        ...
        T GetXExtreme(T x0, T x1)
        {
            ...
            std::int32_t signEDer0 = (eder0 > C_<T>(0) ? 1 : (eder0 < C_<T>(0) ? -1 : 0));
            std::int32_t signEDer1 = (eder1 > C_<T>(0) ? 1 : (eder1 < C_<T>(0) ? -1 : 0));
            GTL_ARGUMENT_ASSERT(
                signEDer0 * signEDer1 == -1,
                "The interval [x0,x1] does not bound a root.");

            T xmid = C_<T>(0), ederMid = C_<T>(0);
            std::int32_t signEMid = 0;
            for (std::size_t i = 0; i < mMaxBisectionIterations; ++i)
            {
                xmid = C_<T>(1, 2) * (x0 + x1);
                if (xmid == x0 || xmid == x1)
                {
                ...
                signEMid = (ederMid > C_<T>(0) ? 1 : (ederMid < C_<T>(0) ? -1 : 0));
        ...
        {
            std::size_t index = mVCoefficients.size() - 1;
            T result = mVCoefficients[index--];
            for (std::size_t i = 1; i < mVCoefficients.size(); ++i, --index)
            {
        ...
        {
            std::size_t index = mPCoefficients.size() - 1;
            T result = mPCoefficients[index--];
            for (std::size_t i = 1; i < mPCoefficients.size(); ++i, --index)
            {
        ...
        T EvaluatePDer(T const& x)
        {
            ...
            --index;
            for (std::size_t i = 2; i < mPCoefficients.size(); ++i, --index)
            {
        ...
        T mXMax;
        std::size_t mDegree;
        std::size_t mMaxRemezIterations;
        std::size_t mMaxBisectionIterations;
        std::size_t mMaxBracketIterations;
    ...
    private:
        friend class UnitTestRemezAlgorithm;
    };
}
