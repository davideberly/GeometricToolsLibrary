...
// StaticVETManifoldMesh represents a vertex-edge-triangle manifold mesh for
// which triangles are provided as a single batch and no mesh modification
// operations are going to be performed on the mesh. StaticVETManifoldMesh
// significantly outperforms DynamicVETManifoldMesh, a class that allows,
// triangle insertions and removals at any time. The underlying C++ container
// classes lead to significant memory allocation and deallocation costs and
// are also expensive for find operations. StaticVETManifoldMesh minimizes the
// memory management costs. Moreover, it allows for multithreading which is
// useful when the numbers of vertices and triangles are large.
...
namespace gtl
{
    ...
    public:
        ...
        // block of memory has 12 * numTriangles elements of type std::size_t.
        class Vertex
        {
        public:
            Vertex()
            ...
            inline std::size_t GetNumAdjacents() const
            {
                return mNumAdjacents;
            }

            inline std::array<std::size_t, 4> const* GetAdjacents() const
            {
                return mAdjacents;
            }

        private:
            // Only StaticVETManifoldMesh may write the members of this class.
            friend class StaticVETManifoldMesh;

            void Initialize(std::size_t numAdjacents, std::size_t*& storage)
            {
                mNumAdjacents = 0;
                mAdjacents = reinterpret_cast<std::array<std::size_t, 4>*>(storage);
                storage += 4 * numAdjacents;
            }

            void Insert(std::size_t v1, std::size_t v2, std::size_t t, std::size_t location)
            {
                mAdjacents[mNumAdjacents++] = { v1, v2, t, location };
            }

            // The number of triangles sharing v0. The value starts at zero
            // and is incremented during the StaticVETManifoldMesh::Populate
            // execution.
            std::size_t mNumAdjacents;
            ...
            std::array<std::size_t, 4>* mAdjacents;
        };
        ...
        StaticVETManifoldMesh(
            ...
            mMaxTrianglesAtVertex(0)
        {
            Create(numVertices, triangles, numThreads);
        }

        ~StaticVETManifoldMesh() = default;
        ...
        {
            GTL_ARGUMENT_ASSERT(
                numVertices >= 3 && triangles.size() > 0,
                "Invalid input.");

            mVertices.resize(numVertices);
            mStorage.resize(12 * triangles.size());
            std::fill(mStorage.begin(), mStorage.end(), invalid);
            mTriangles = triangles,
            mAdjacents.resize(triangles.size());
            std::fill(mAdjacents.begin(), mAdjacents.end(),
                std::array<std::size_t, 3>{ invalid, invalid, invalid });
            mMinTrianglesAtVertex = 0;
            mMaxTrianglesAtVertex = 0;

            std::vector<std::size_t> numTrianglesAtVertex(numVertices, 0);
        ...
        inline std::size_t GetMinNumTrianglesAtVertex() const
        {
            return mMinTrianglesAtVertex;
        }

        inline std::size_t GetMaxNumTrianglesAtVertex() const
        {
            return mMaxTrianglesAtVertex;
        }

        // Determine whether or not the undirected edge (v0,v1) exists.
        bool EdgeExists(std::size_t v0, std::size_t v1) const
        {
        ...
        bool GetAdjacentTriangles(std::size_t v0, std::size_t v1, std::size_t& adj0, std::size_t& adj1) const
        {
            if (v0 < mVertices.size() && v1 < mVertices.size() && v0 != v1)
            {
        ...
        //       <process component[i]>;
        //   }
        void GetComponents(std::vector<std::vector<std::size_t>>& components) const
        {
            components.clear();

            // The values are 0 (unvisited), 1 (discovered), 2 (finished).
            std::vector<std::uint32_t> visited(mTriangles.size(), 0);

            // Share a stack for the depth-first search. This avoids
            // allocating and deallocating a stack for each call to
            // DepthFirstSearch.
            std::vector<std::size_t> sharedStack(mTriangles.size());
            ...
            // caused by push_back.
            std::vector<std::size_t> sharedComponents(mTriangles.size());

            for (std::size_t t = 0; t < mTriangles.size(); ++t)
            {
                if (visited[t] == 0)
                {
                    std::size_t numInserted = DepthFirstSearch(t, visited,
                        sharedStack, sharedComponents.data());

                    std::vector<std::size_t> component(numInserted);
                    std::copy(
            ...
            bool duplicateEndpoints) const
        {
            ...
            for (std::size_t t = 0; t < mTriangles.size(); ++t)
            {
                ...
                for (std::size_t a = 0; a < 3; ++a)
                {
                    ...
                    {
                        std::array<std::size_t, 2> directed{ tri[(a + 1) % 3], tri[(a + 2) % 3] };
    ...
    private:
        struct BoundaryEdge
        {
            ...
            BoundaryEdge(std::size_t inT, std::size_t inA, bool inVisited)
                :
                t(inT),
                a(inA),
                visited(inVisited)
        ...
        // std::size_t, so the adjacent storage requires 12 * numTriangles values
        // of type std::size_t.
        void GetNumTrianglesAtVertex(std::vector<std::size_t>& counts)
        {
        ...
        void InitializeStorage(std::vector<std::size_t> const& numTrianglesAtVertex)
        {
        ...
        // 12 * numTrianglesAtVertex[v] elements of type std::size_t. For the
        // entire mesh, we need 12 * numTriangles elements of type std::size_t.
        void Populate()
        {
            for (std::size_t t = 0; t < mTriangles.size(); ++t)
            {
        ...
        // two triangles.
        void UpdateAdjacencyForSharedEdges(std::size_t numThreads)
        {
            if (numThreads <= 1)
            {
        ...
        void UpdateAdjacencyForSharedEdgesMultithreaded(std::size_t numThreads)
        {
            std::size_t const numVertices = mVertices.size();
            std::size_t const numVerticesPerThread = numVertices / numThreads;
        ...
        void UpdateAdjacencyForEdge(std::size_t v0)
        {
            auto& vertex0 = mVertices[v0];
            auto* adjacents0 = vertex0.mAdjacents;
            for (std::size_t j = 0; j < vertex0.mNumAdjacents; ++j, ++adjacents0)
            {
                std::size_t v1 = (*adjacents0)[0];
                std::size_t v2 = (*adjacents0)[1];
                ...
                if (adjacents1)
                {
                    ...
                    // writes to.
                    std::size_t tri0 = (*adjacents0)[2];
                    std::size_t loc0 = (*adjacents0)[3];
                    std::size_t adj1 = (*adjacents1)[2];
                    (*adjacents0)[3] = adj1;
        ...
        std::array<std::size_t, 4>* GetDirectedEdge(std::size_t v0, std::size_t v1) const
        {
            auto& vertex0 = mVertices[v0];
            auto* adjacents0 = vertex0.mAdjacents;
            for (std::size_t j = 0; j < vertex0.mNumAdjacents; ++j, ++adjacents0)
        ...
        std::size_t DepthFirstSearch(std::size_t tInitial, std::vector<std::uint32_t>& visited,
            std::vector<std::size_t>& tStack, std::size_t* component) const
        {
            ...
            // result is the maximum value of std::size_t.
            ...
            while (top != invalid)
            {
                std::size_t t = tStack[top];
                visited[t] = 1;
                std::size_t i{};
                for (i = 0; i < 3; ++i)
                {
                    std::size_t tAdjacent = mAdjacents[t][i];
        ...
        void GetBoundaryPolygon(std::size_t t, std::size_t a,
            BoundaryEdgeMap& boundaryEdges, std::vector<std::size_t>& polygon) const
        {
            std::array<std::size_t, 3> tri = mTriangles[t];
            std::size_t i0 = (a + 1) % 3;
            std::size_t i1 = (i0 + 1) % 3;
            std::array<std::size_t, 2> vEdge = { tri[i0], tri[i1] };
...
#pragma warning(default : 28020)
        ...
        std::vector<std::size_t> mStorage;
        std::vector<std::array<std::size_t, 3>> mTriangles;
        std::vector<std::array<std::size_t, 3>> mAdjacents;
        std::size_t mMinTrianglesAtVertex;
        std::size_t mMaxTrianglesAtVertex;

    private:
        friend class UnitTestStaticVETManifoldMesh;
    };
}
