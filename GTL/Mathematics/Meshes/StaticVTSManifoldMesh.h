...
// StaticVTSManifoldMesh represents a vertex-face-tetrahedron manifold mesh
// for which tetrahedra are provided as a single batch and no mesh
// modification operations are going to be performed on the mesh.
// StaticVTSManifoldMesh outperforms DynamicVTSManifoldMesh, a class that
// allows tetrahedron insertions and removals at any time. The underlying
// C++ container classes lead to significant memory allocation and
// deallocation costs and are also expensive for find operations.
// StaticVTSManifoldMesh minimizes the memory management costs. Moreover,
// it allows for multithreading which is useful when the numbers of vertices
// and tetrahedra are large.
...
namespace gtl
{
    ...
    public:
        ...
        static std::array<std::array<std::size_t, 3>, 4> constexpr face =
        { {
        ...
        // block of memory has 20 * numTetrahedra elements of type std::size_t.
        class Vertex
        {
        public:
            Vertex()
            ...
            inline std::array<std::size_t, 5> const* GetAdjacents() const
            {
                return mAdjacents;
            }

        private:
            // Only StaticVTSManifoldMesh may write the members of this class.
            friend class StaticVTSManifoldMesh;

            void Initialize(std::size_t numAdjacents, std::size_t*& storage)
            {
                mNumAdjacents = 0;
                mAdjacents = reinterpret_cast<std::array<std::size_t, 5>*>(storage);
                storage += 5 * numAdjacents;
            }

            void Insert(std::size_t v1, std::size_t v2, std::size_t v3, std::size_t s, std::size_t location)
            {
                mAdjacents[mNumAdjacents++] = { v1, v2, v3, s, location };
            }

            // The number of tetrahedra sharing v0. The value starts at zero
            // and is incremented during the StaticVTSManifoldMesh::Populate
            // execution.
            ...
            std::array<std::size_t, 5>* mAdjacents;
        };
        ...
        {
            GTL_ARGUMENT_ASSERT(
                numVertices >= 4 && tetrahedra.size() > 0,
                "Invalid input.");

            mVertices.resize(numVertices);
            mStorage.resize(20 * tetrahedra.size(), invalid);
            std::fill(mStorage.begin(), mStorage.end(), invalid);
            mTetrahedra = tetrahedra;
            mAdjacents.resize(tetrahedra.size());
            std::fill(mAdjacents.begin(), mAdjacents.end(),
                std::array<std::size_t, 4>{ invalid, invalid, invalid, invalid });
            mMinTetrahedraAtVertex = 0;
            mMaxTetrahedraAtVertex = 0;

            std::vector<std::size_t> numTetrahedraAtVertex(numVertices, 0);
        ...
        inline std::size_t GetMinNumTetrahedraAtVertex() const
        {
            return mMinTetrahedraAtVertex;
        }

        inline std::size_t GetMaxNumTetrahedraAtVertex() const
        {
            return mMaxTetrahedraAtVertex;
        }

        // Determine whether or not the unordered face (v0,v1,v2) exists.
        bool FaceExists(std::size_t v0, std::size_t v1, std::size_t v2) const
        {
        ...
        bool GetAdjacentTetrahedra(std::size_t v0, std::size_t v1, std::size_t v2, std::size_t& adj0, std::size_t& adj1) const
        {
            if (v0 < mVertices.size() && v1 < mVertices.size() && v2 < mVertices.size() &&
                v0 != v1 && v0 != v2 && v1 != v2)
            {
    ...
    protected:
        ...
        // 5 components of type std::size_t, so the adjacent storage requires
        // 20 * numTetrahedra values of type std::size_t.
        void GetNumTetrahedraAtVertex(std::vector<std::size_t>& counts)
        {
        ...
        void InitializeStorage(std::vector<std::size_t> const& numTetrahedraAtVertex)
        {
        ...
        // 20 * numTetrahedraAtVertex[v] elements of type std::size_t. For the
        // entire mesh, we need 20 * numTetrahedra elements of type std::size_t.
        void PopulateVertices()
        {
            for (std::size_t t = 0; t < mTetrahedra.size(); ++t)
            {
        ...
        // by two tetrahedra.
        void UpdateAdjacencyForSharedFaces(std::size_t numThreads)
        {
            if (numThreads <= 1)
            {
        ...
        void UpdateAdjacencyForSharedFacesMultithreaded(std::size_t numThreads)
        {
        ...
        void UpdateAdjacencyForFace(std::size_t v0)
        {
            ...
            for (std::size_t j = 0; j < vertex0.mNumAdjacents; ++j, ++adjacents0)
            {
                ...
                if (adjacents1)
                {
                    ...
                    std::size_t tet0 = (*adjacents0)[3];
                    std::size_t loc0 = (*adjacents0)[4];
                    std::size_t adj1 = (*adjacents1)[3];
                    (*adjacents0)[4] = adj1;
        ...
        std::array<std::size_t, 5>* GetOrderedFace(std::size_t v0, std::size_t v1, std::size_t v2) const
        {
            auto& vertex0 = mVertices[v0];
            auto* adjacents0 = vertex0.mAdjacents;
            for (std::size_t j = 0; j < vertex0.mNumAdjacents; ++j, ++adjacents0)
            {
                std::array<std::size_t, 4> tetra =
                {
                    v0,
                    (*adjacents0)[0],
                    (*adjacents0)[1],
                ...
                std::array<std::size_t, 3> inFace = { v0, v1 ,v2 };

                for (std::size_t f = 0; f < 4; ++f)
                {
                    std::array<std::size_t, 3> compareFace =
                    {
                        tetra[face[f][0]],
                        tetra[face[f][1]],
                        tetra[face[f][2]]
        ...
        std::vector<std::array<std::size_t, 4>> mTetrahedra;
        std::vector<std::array<std::size_t, 4>> mAdjacents;
        std::size_t mMinTetrahedraAtVertex;
        std::size_t mMaxTetrahedraAtVertex;

    private:
        friend class UnitTestStaticVTSManifoldMesh;
    };
}
