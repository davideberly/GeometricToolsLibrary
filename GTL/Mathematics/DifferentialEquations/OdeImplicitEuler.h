...
namespace gtl
{
    template <typename T, std::size_t...> class OdeImplicitEuler;
    ...
    public:
        ...
        OdeImplicitEuler(T const& tDelta,
            ...
            std::size_t numNewtonIterations)
            :
            ...
            T& tOut, T& xOut) override
        {
            ...
            for (std::size_t j = 0; j < mNumNewtonIterations; ++j)
            {
    ...
    private:
        ...
        std::size_t mNumNewtonIterations;
    ...
    public:
            ...
            T& tOut, Vector<T>& xOut) override
        {
            ...
            {
                auto DG = (-this->mTDelta) * this->mDerivativeFunction(tNext, z);
                for (std::size_t k = 0; k < xIn.size(); ++k)
                {
                    DG(k, k) += C_<T>(1);
                }
                auto inverseDG = Inverse(DG, &determinant);
