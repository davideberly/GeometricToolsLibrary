...
namespace gtl
{
    ...
    public:
        ...
        RootsPolynomial(std::size_t maxBisections, std::size_t precision)
            :
            ...
            bool wantRootIntervals = false)
        {
            ...
            std::int32_t iMin = RemoveLeadingAndTrailingZeros(p, reduced);
        ...
        static std::int32_t constexpr nonzeroConstant = -1;
        static std::int32_t constexpr unknownRootStatus = 0;
        static std::int32_t constexpr hasOnlySimpleRoots = 1;
        static std::int32_t constexpr hasRepeatedRoots = 2;
        static std::int32_t constexpr zeroConstant = std::numeric_limits<std::int32_t>::max();

        inline std::int32_t GetRootStatus() const
        {
            return mRootStatus;
        }
    ...
    private:
        ...
        // std::numeric_limits<std::int32_t>::max() indicates the polynomial is
        ...
        template <typename T, IsFPType<T> = 0>
        static std::int32_t RemoveLeadingAndTrailingZeros(Polynomial1<T> const& p,
            Polynomial1<BSN>& reduced)
        {
            std::size_t degree = p.GetDegree();
            ...
            for (std::size_t i = iMin, j = 0; i <= iMax; ++i, ++j)
            {
                reduced[j] = p[i];
            }

            return static_cast<std::int32_t>(iMin);
        }
        ...
        static void GetRootBound(Polynomial1<BSN> const& p, BSN& bound)
        {
            ...
            std::size_t const degree = p.GetDegree();
            std::int32_t const minExponent = p[degree].GetExponent();
            std::int32_t maxExponent = p[0].GetExponent();
            for (std::size_t i = 1; i < degree; ++i)
            {
                std::int32_t exponent = p[i].GetExponent();
            ...
            std::vector<BSN>& pRoots, bool wantRootIntervals)
        {
            ...
            std::size_t const pDegree = p.GetDegree();
            std::size_t order = pDegree - 1;
            ...
            if (pDegree > 1)
            {
                ...
                --order;
                for (std::size_t i = 1, qDegree = 2; i < pDegree; ++i, ++qDegree, --order)
                {
                    q.SetDegree(qDegree);
                    q[0] = p[order];
                    for (std::size_t power = 1, j = order + 1; power <= qDegree; ++power, ++j)
                    {
                    ...
                    std::int32_t parity = 1 - 2 * (static_cast<std::int32_t>(qDegree & 1));
                    std::int32_t sign = q[qDegree].GetSign();
                    ...
                    if (dRoots.size() > 0)
                    {
                        ...
                        for (std::size_t j = 0; j + 2 <= dRoots.size(); ++j)
                        {
                            qMin = std::move(qMax);
                            qMax = q(dRoots[j + 1]);
            ...
            std::size_t j = pRoots.size();
            pRoots.resize(pRoots.size() + dRoots.size());
            for (std::size_t i = 0; i < dRoots.size(); ++i, ++j)
            {
        ...
        void ComputePascalsTriangle()
        {
            ...
            std::size_t constexpr initialMaxDegree = 16;
            ...
            SetPascal(0, 0, 1);
            for (std::size_t n = 1; n <= mMaxDegree; ++n)
            {
                SetPascal(n, 0, 1);
        ...
        void AddRowsToPascalsTriangle(std::size_t degree)
        {
            if (degree > mMaxDegree)
            {
                mPascal.resize((degree + 1) * (degree + 2) / 2);
                for (std::size_t n = mMaxDegree + 1; n <= degree; ++n)
                {
                    SetPascal(n, 0, 1);
                    for (std::size_t k = 1; k < n; ++k)
                    {
        ...
        inline void SetPascal(std::size_t n, std::size_t k, BSN const& value)
        {
            mPascal[n * (n + 1) / 2 + k] = value;
        }

        inline BSN const& GetPascal(std::size_t n, std::size_t k) const
        {
            return mPascal[n * (n + 1) / 2 + k];
        }

        inline BSN const& GetModulate(std::size_t order, std::size_t power)
        {
            return GetPascal(order + power, power);
        }

        std::size_t mMaxBisections, mPrecision;
        std::size_t mMaxDegree;
        std::vector<BSN> mPascal;
        std::int32_t mRootStatus;
