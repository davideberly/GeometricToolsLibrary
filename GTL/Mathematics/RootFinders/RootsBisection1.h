...
namespace gtl
{
    ...
    public:
        ...
        RootsBisection1(std::size_t maxIterations, std::size_t = 0)
            :
            ...
            T& tRoot, T& fRoot)
        {
            ...
            std::int32_t signFMin = (fMin > C_<T>(0) ? +1 : (fMin < C_<T>(0) ? -1 : 0));
            if (signFMin == 0)
            {
                mNumIterations = 0;
                tRoot = tMin;
            ...
            std::int32_t signFMax = (fMax > C_<T>(0) ? +1 : (fMax < C_<T>(0) ? -1 : 0));
            if (signFMax == 0)
            {
                mNumIterations = 0;
                tRoot = tMax;
            ...
            if (signFMin == signFMax)
            {
                ...
                mNumIterations = std::numeric_limits<std::size_t>::max();
                tRoot = C_<T>(0);
                fRoot = C_<T>(0);
                return false;
            }
            ...
            for (mNumIterations = 1; mNumIterations < mMaxIterations; ++mNumIterations)
            {
                ...
                std::int32_t signFRoot = (fRoot > C_<T>(0) ? +1 : (fRoot < C_<T>(0) ? -1 : 0));
                if (signFRoot != 0)
                {
    ...
    private:
        ...
        void RoundInitial(T const& inT0, T const& inT1, T& t0, T& t1)
        {
            if (mPrecision < std::numeric_limits<std::size_t>::max())
            {
                inT0.Convert(mPrecision, APRoundingMode::TO_NEAREST, t0);
                inT1.Convert(mPrecision, APRoundingMode::TO_NEAREST, t1);
            }
            else
            {
                t0 = inT0;
        ...
        T RoundAverage(T const& t0, T const& t1)
        {
            ...
            if (mPrecision < std::numeric_limits<std::size_t>::max())
            {
                ...
                average.Convert(mPrecision, APRoundingMode::TO_NEAREST, roundedAverage);
        ...
        std::size_t mMaxIterations, mPrecision;
