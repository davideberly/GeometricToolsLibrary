...
namespace gtl
{
    ...
    private:
            ...
            std::vector<Rational>& rRoots)
        {
            ...
            for (std::size_t i = 1; i + 1 <= degree; ++i)
            {
            ...
            Rational const& rXMax, bool useThreading, std::vector<Rational>& rRoots)
        {
            ...
            if (rRootsDerivative.size() > 0)
            {
                if (useThreading)
                {
                    ...
                    // [rXLower,rXUpper].
                    for (std::size_t i0 = 0, i1 = 1; i1 < rRootsDerivative.size(); i0 = i1++)
                    {
            ...
            Rational const& rXMin, Rational const& rXMax, Rational& rRoot)
        {
            ...
            std::int32_t signRPMin = rPMin.GetSign();
            if (signRPMin == 0)
            {
                rRoot = rXMin;
                return true;
            ...
            std::int32_t signRPMax = rPMax.GetSign();
            if (signRPMax == 0)
            {
            ...
            std::int32_t signTPMin = (tPMin > zero ? +1 : (tPMin < zero ? -1 : 0));
            if (signTPMin == 0)
            {
                rRoot = rXMin;
                return true;
            ...
            std::int32_t signTPMax = (tPMax > zero ? +1 : (tPMax < zero ? -1 : 0));
            if (signTPMax == 0)
            {
                // Do not return the root rXMax. The next interval will be
                // responsible for managing this root.
            ...
            std::size_t const maxIterations = 4096;
            ...
            for (std::size_t i = 0; i < maxIterations; ++i)
            {
                ...
                std::int32_t signTPAtRoot = (tPAtRoot > zero ? +1 : (tPAtRoot < zero ? -1 : 0));
                std::int32_t sign = signTPAtRoot * signTPMin;
                if (sign < 0)
                {
                    tXMax = tRoot;
                    tPMax = tPAtRoot;
    ...
    private:
        friend class UnitTestRootsGeneralPolynomial;
    };
}
