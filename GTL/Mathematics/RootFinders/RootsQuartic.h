...
namespace gtl
{
    ...
    public:
        ...
        // elements. The returned std::size_t is the number of valid roots in the
        // roots[] array.
        using Rational = BSRational<UIntegerAP32>;

        // Solve the general quartic g0 + g1*x + g2*x^2 + g3*x^3 + g4*x^4 = 0.
        static std::size_t Solve(bool useBisection,
            ...
            PolynomialRoot<T>* roots)
        {
            ...
            std::size_t numRoots = ComputeDepressedRoots(useBisection, rD0, rD1, rD2, rRoots.data());
            ...
            PolynomialRoot<T>* roots)
        {
            ...
            std::array<PolynomialRoot<Rational>, 4> rRoots{};
            std::size_t numRoots = ComputeDepressedRoots(useBisection, d0, d1, d2, rRoots.data());
            for (std::size_t i = 0; i < numRoots; ++i)
            {
        ...
        static std::size_t ComputeDepressedRoots(bool useBisection,
    ...
    private:
        ...
        static std::size_t HasZeroValuedRoots(bool useBisection,
            ...
            PolynomialRoot<T>* roots)
        {
            ...
            if (g1 == zero)
            {
                if (g2 == zero)
                {
                    ...
                    {
                        std::size_t numRoots = RootsLinear<T>::Solve(g3, g4, roots);
                ...
                {
                    std::size_t numRoots = RootsQuadratic<T>::Solve(useBisection, g2, g3, g4, roots);
            ...
            {
                std::size_t numRoots = RootsCubic<T>::Solve(useBisection, g1, g2, g3, g4, roots);
            ...
            PolynomialRoot<T>* roots)
        {
            ...
            if (m1 == zero)
            {
                if (m2 == zero)
                {
                    ...
                    {
                        std::size_t numRoots = RootsLinear<T>::Solve(m3, roots);
                        roots[numRoots++] = { zero, 3 };
                        std::sort(roots, roots + numRoots);
                        return numRoots;
                    }
                ...
                {
                    std::size_t numRoots = RootsQuadratic<T>::Solve(useBisection, m2, m3, roots);
                    roots[numRoots++] = { zero, 2 };
                    std::sort(roots, roots + numRoots);
                    return numRoots;
                }
            ...
            {
                std::size_t numRoots = RootsCubic<T>::Solve(useBisection, m1, m2, m3, roots);
                roots[numRoots++] = { zero, 1 };
                std::sort(roots, roots + numRoots);
                return numRoots;
            }
            ...
            PolynomialRoot<Rational>* rRoots)
        {
            ...
            if (signT > 0)
            {
                ...
                std::int32_t signSPsqrtT = rSPsqrtT.GetSign();
                std::int32_t signSMsqrtT = rSMsqrtT.GetSign();
                if (signSMsqrtT > 0)
                {
                    // Four real roots.
        ...
        static std::size_t ComputeDepressedRootsBisection(
            Rational const& rD0, Rational const& rD1, Rational const& rD2,
            PolynomialRoot<Rational>* rRoots)
        {
            std::int32_t signD0 = rD0.GetSign();
            std::int32_t signD1 = rD1.GetSign();
            std::int32_t signD2 = rD2.GetSign();
            if (signD0 == 0)
            {
                if (signD1 == 0)
                {
                ...
                else
                {
                    ...
                    std::size_t numRoots = RootsCubic<Rational>::ComputeDepressedRoots(true, rD1, rD2, rRoots);
            ...
            std::int32_t signDelta = rDelta.GetSign();
            if (signDelta == 0)
            {
        ...
        static std::size_t ComputeDepressedRootsClosedForm(
            Rational const& rD0, Rational const& rD1, Rational const& rD2,
            PolynomialRoot<Rational>* rRoots)
        {
            std::int32_t signD0 = rD0.GetSign();
            ...
            if (signD0 == 0)
            {
                ...
                else
                {
                    // Zero is a root of multiplicity 1. The cubic solver
                    // computes the other roots.
                    std::size_t numRoots = RootsCubic<Rational>::ComputeDepressedRoots(false, rD1, rD2, rRoots);
                    rRoots[numRoots++] = { Rational(0), 1 };
                    std::sort(rRoots, rRoots + numRoots);
                    return numRoots;
                }
