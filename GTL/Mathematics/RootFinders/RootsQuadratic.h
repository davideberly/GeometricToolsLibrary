...
namespace gtl
{
    ...
    public:
            ...
            PolynomialRoot<T>* roots)
        {
            ...
            std::size_t numRoots = ComputeDepressedRoots(useBisection, rD0, rRoots.data());
            ...
            PolynomialRoot<T>* roots)
        {
            ...
            std::array<PolynomialRoot<Rational>, 2> rRoots{};
            std::size_t numRoots = ComputeDepressedRoots(useBisection, d0, rRoots.data());
            for (std::size_t i = 0; i < numRoots; ++i)
            {
    ...
    private:
        ...
        static std::size_t HasZeroValuedRoots(T const& g1, T const& g2,
            PolynomialRoot<T>* roots)
        {
            T const zero = static_cast<T>(0);
            if (g1 == zero)
            ...
            {
                std::size_t numRoots = RootsLinear<T>::Solve(g1, g2, roots);
        ...
        static std::size_t HasZeroValuedRoots(T const& m1, PolynomialRoot<T>* roots)
        {
            T const zero = static_cast<T>(0);
            if (m1 == zero)
            {
            ...
            {
                std::size_t numRoots = RootsLinear<T>::Solve(m1, roots);
                roots[numRoots++] = { zero, 1 };
                std::sort(roots, roots + numRoots);
                return numRoots;
            }
