...
namespace gtl
{
    ...
    public:
        ...
        QuadricSurface(std::array<T, 10> const& q)
            ...
            mClassification(Classification::UNKNOWN),
            mA{
                { q[4], C_<T>(1, 2) * q[5], C_<T>(1, 2) * q[6] },
                { C_<T>(1, 2) * q[5], q[7], C_<T>(1, 2) * q[8] },
                { C_<T>(1, 2) * q[6], C_<T>(1, 2) * q[8], q[9] }},
            mB{ q[1], q[2], q[3] },
            mC(q[0])
        {
        }
    ...
    private:
        ...
        static void ComputeRootSigns(std::array<Rational, 4> const& rP,
            std::size_t& numPositive, std::size_t& numNegative, std::size_t& numZero)
        {
            // Collect the nonzero signs of the rP[0], rP[1] and rP[2].
            std::size_t constexpr degree = 3;
            std::array<std::int32_t, degree + 1> signs{};
            ...
            std::int32_t currentSign = signs[degree];
            numPositive = 0;
            for (std::size_t j = 0, i = degree - 1; j < degree; ++j, --i)
            {
        ...
        static Classification AllNonzero(Matrix3x3<Rational> const& A,
            Vector3<Rational> const& b, Rational const& c, std::size_t numPositiveRoots)
        {
            Rational r = Dot(b, GetInverse(A) * b) / C_<Rational>(4) - c;
        ...
        static Classification TwoNonzero(Matrix3x3<Rational> const& A,
            Vector3<Rational> const& b, Rational const& c, std::size_t numPositive, std::size_t numNegative)
        {
            ...
            Rational r = Dot(f, GetInverse(E) * f) / C_<Rational>(4) - c;
            ...
            Vector3<Rational>& w0, Vector3<Rational>& w1, Vector3<Rational>& w2)
        {
            ...
            {
                w0 = { -w2[2], C_<Rational>(0), w2[0] };
            }
            else
            {
                w0 = { C_<Rational>(0), w2[2], -w2[1] };
            }
            w1 = Cross(w2, w0);
        }

        static Classification OneNonzero(Matrix3x3<Rational> const& A,
            Vector3<Rational> const& b, Rational const& c, std::size_t numPositive)
        {
            Vector3<Rational> w0{}, w1{}, w2{};
            ComputeOrthogonalSetOneNonzero(A, w0, w1, w2);
            Rational d0 = Dot(w0, b);
            Rational d1 = Dot(w1, b);
            if (d0 != C_<Rational>(0) || d1 != C_<Rational>(0))
            {
            ...
            Rational f = Dot(w2, b);
            Rational r = f * f / (C_<Rational>(4) * E) - c;
    ...
    private:
        friend class UnitTestQuadricSurface;
    };
}
