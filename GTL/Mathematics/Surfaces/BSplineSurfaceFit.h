...
namespace gtl
{
    ...
    public:
        ...
        static void Execute(
            std::array<std::size_t, 2> const& degree,
            std::array<std::size_t, 2> const& numControls,
            std::array<std::size_t, 2> const& numSamples,
            std::vector<Vector<T, N>> const& samples, BSplineSurface<T, N>& spline)
        {
            ...
            for (std::size_t dim = 0; dim < 2; ++dim)
            {
                ...
                std::size_t last = input[dim].uniqueKnots.size() - 1;
            ...
            std::array<std::size_t, 2> numBands
            {
            ...
            for (std::size_t dim = 0; dim < 2; ++dim)
            {
                for (std::size_t i0 = 0; i0 < numControls[dim]; ++i0)
                {
                    ...
                    std::size_t i1Max = i0 + degree[dim];
                    if (i1Max >= numControls[dim])
                    {
                        i1Max = numControls[dim] - 1;
                    }

                    for (std::size_t i1 = i0; i1 <= i1Max; ++i1)
                    {
                        T value = C_<T>(0);
                        for (std::size_t i2 = 0; i2 < numSamples[dim]; ++i2)
                        {
                            T t = tMultiplier[dim] * static_cast<T>(i2);
            ...
            for (std::size_t dim = 0; dim < 2; dim++)
            {
                ...
                for (std::size_t i0 = 0; i0 < numControls[dim]; ++i0)
                {
                    for (std::size_t i1 = 0; i1 < numSamples[dim]; ++i1)
                    {
                        T t = tMultiplier[dim] * static_cast<T>(i1);
                        std::size_t imin = 0, imax = 0;
                        basisFunction[dim]->Evaluate(t, 0, imin, imax);
                        if (imin <= i0 && i0 <= imax)
                        {
            ...
            for (std::size_t i1 = 0; i1 < numControls[1]; ++i1)
            {
                for (std::size_t i0 = 0; i0 < numControls[0]; ++i0)
                {
                    Vector<T, N> sum{};
                    for (std::size_t j1 = 0; j1 < numSamples[1]; ++j1)
                    {
                        T x1Value = ATMat[1](i1, j1);
                        for (std::size_t j0 = 0; j0 < numSamples[0]; ++j0)
                        {
    ...
    private:
        friend class UnitTestBSplineSurfaceFit;
    };
}
