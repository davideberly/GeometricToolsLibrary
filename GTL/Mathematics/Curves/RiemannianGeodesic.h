...
namespace gtl
{
    ...
    public:
        ...
        RiemannianGeodesic(
            ...
            std::size_t numIntegralSamples,
            std::size_t numSearchSamples,
            std::size_t numSubdivisions,
            std::size_t numRefinements,
        ...
        RiemannianGeodesic(std::size_t dimension)
            :
        ...
        inline std::size_t GetNumIntegralSamples() const
        {
            return mNumIntegralSamples;
        }

        inline std::size_t GetNumSearchSamples() const
        {
            return mNumSearchSamples;
        }

        inline std::size_t GetNumSubdivisions() const
        {
            return mNumSubdivisions;
        }

        inline std::size_t GetNumRefinements() const
        {
        ...
        T ComputeSegmentLength(Vector<T> const& point0, Vector<T> const& point1)
        {
            ...
            std::size_t const imax = mNumIntegralSamples - 2;
        ...
        T ComputeTotalLength(std::size_t quantity, std::vector<Vector<T>> const& path)
        {
            GTL_ARGUMENT_ASSERT(
                quantity >= 2,
                "The path must have at least two points.");

            std::size_t const imax = quantity - 2;
        ...
        void ComputeGeodesic(Vector<T> const& end0, Vector<T> const& end1,
            std::size_t& quantity, std::vector<Vector<T>>& path)
        {
            GTL_ARGUMENT_ASSERT(
                mNumSubdivisions < 32,
                "The number of subdivisions has exceeded the maximum.");

            quantity = (static_cast<std::size_t>(1) << mNumSubdivisions) + 1;
            ...
            for (mSubdivision = 1; mSubdivision <= mNumSubdivisions; ++mSubdivision)
            {
                ...
                std::size_t newQuantity = 2 * mCurrentQuantity - 1;
                GTL_RUNTIME_ASSERT(
                ...
                for (std::size_t i = mCurrentQuantity - 1; i > 0; --i)
                {
                    path[2 * i] = path[i];
                }

                // Subdivide the polyline.
                for (std::size_t i = 0; i <= mCurrentQuantity - 2; ++i)
                {
                ...
                {
                    for (std::size_t i = 1; i <= mCurrentQuantity - 2; ++i)
                    {
        ...
        bool Refine(Vector<T> const& end0, Vector<T>& mid, Vector<T> const& end1)
        {
            ...
            std::size_t const jmax = 2 * mNumSearchSamples + 1;
            std::int32_t i = -static_cast<std::int32_t>(mNumSearchSamples);
            for (std::size_t j = 0; j <= jmax; ++j, ++i)
            {
        ...
        inline std::size_t GetSubdivision() const
        {
            return mSubdivision;
        }

        inline std::size_t GetRefinement() const
        {
            return mRefinement;
        }

        inline std::size_t GetCurrentQuantity() const
        {
        ...
        // segments are computed relative to the metric tensor.
        T ComputeTotalCurvature(std::size_t quantity, std::vector<Vector<T>> const& path)
        {
            GTL_ARGUMENT_ASSERT(
                quantity >= 2,
    ...
    protected:
        ...
        T ComputeIntegrand(Vector<T> const& pos, Vector<T> const& der)
        {
            ...
            for (std::size_t k = 0; k < mDimension; ++k)
            {
        ...
        {
            for (std::size_t derivative = 0; derivative < mDimension; ++derivative)
            {
                for (std::size_t i0 = 0; i0 < mDimension; ++i0)
                {
                    for (std::size_t i1 = 0; i1 < mDimension; ++i1)
                    {
        ...
        {
            for (std::size_t i2 = 0; i2 < mDimension; ++i2)
            {
                for (std::size_t i0 = 0; i0 < mDimension; ++i0)
                {
                    for (std::size_t i1 = 0; i1 < mDimension; ++i1)
                    {
                        T value = C_<T>(0);
                        for (std::size_t j = 0; j < mDimension; ++j)
                        {
        ...
        std::size_t mNumIntegralSamples; // default = 16
        std::size_t mNumSearchSamples;   // default = 32
        std::size_t mNumSubdivisions;    // default = 7
        std::size_t mNumRefinements;     // default = 8
        ...
        std::size_t mRefinement;
        std::size_t mCurrentQuantity;
    ...
    private:
        friend class UnitTestRiemannianGeodesic;
    };
}
