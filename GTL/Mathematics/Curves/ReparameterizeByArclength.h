...
namespace gtl
{
    ...
    {
    public:
        // TODO: The GTL interface for ParametricCurve<T,N> already contains
        // the support for computing arclength s from t. It has support for
        // the inversion (compute t from s), but the code here that uses the
        // hybrid Newton's method and bisection here will eventually replace
        // the GTL code and appear in the GTL distribution when it is shipped.
        ReparameterizeByArclength(std::shared_ptr<ParametricCurve<T, N>> const& curve)
            :
            mCurve(curve),
        ...
        struct Output
        {
            ...
            Output(T inT, T inF, std::size_t inIterations)
                :
                t(inT),
                f(inF),
        ...
        inline std::shared_ptr<ParametricCurve<T, N>> const& GetCurve() const
        {
            return mCurve;
        }
    ...
    private:
        ...
        static std::size_t constexpr maxIterations = 4096;
        ...
        std::shared_ptr<ParametricCurve<T, N>> mCurve;
        T const mTMin;
        T const mTMax;
