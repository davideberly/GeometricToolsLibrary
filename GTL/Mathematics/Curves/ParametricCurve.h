...
namespace gtl
{
    ...
    protected:
        ...
        ParametricCurve(std::size_t numSegments, T const* times)
            :
            mRombergOrder(defaultRombergOrder),
            mMaxBisections(defaultMaxBisections),
            mPrecision(defaultPrecision),
    ...
    public:
        ...
        inline std::size_t GetNumSegments() const
        {
        ...
        inline void SetRombergOrder(std::size_t order)
        {
            mRombergOrder = std::max(order, static_cast<std::size_t>(1));
        }

        inline std::size_t GetRombergOrder() const
        {
        ...
        {
            mMaxBisections = std::max(maxBisections, static_cast<std::size_t>(1));
        }
        ...
        inline void SetPrecision(std::size_t precision)
        {
            mPrecision = std::max(precision, static_cast<std::size_t>(1));
        }
        ...
        virtual void Evaluate(T const& t, std::size_t order, Vector<T, N>* jet) const = 0;

        // Return the evaluation as an array of N-tuples of T values.
        void Evaluate(T const& t, std::size_t order, T* values) const
        {
        ...
        T GetLength(T const& t0, T const& t1) const
        {
            ...
            if (mSegmentLength[0] == C_<T>(0))
            {
                ...
                for (std::size_t i = 0; i < mSegmentLength.size(); ++i)
                {
            ...
            if (index0 < index1)
            {
                ...
                for (std::size_t i = index0; i < isup; ++i)
                {
        ...
        void SubdivideByTime(std::size_t numPoints, Vector<T, N>* points) const
        {
        ...
        void SubdivideByLength(std::size_t numPoints, Vector<T, N>* points) const
        {
            T delta = GetTotalLength() / static_cast<T>(numPoints - 1);
            T length = C_<T>(0);
            for (std::size_t i = 0; i < numPoints; ++i)
            {
                T time = GetTime(length);
                points[i] = GetPosition(time);
                length += delta;
            }
        }
    ...
    protected:
        static std::size_t constexpr defaultRombergOrder = 8;
        static std::size_t constexpr defaultMaxBisections = 1024;
        static std::size_t constexpr defaultPrecision = 64;

        std::size_t mRombergOrder;
