...
namespace gtl
{
    ...
    public:
        ...
        // points.
        static void Execute(std::size_t degree, std::size_t numControls,
            std::vector<Vector<T, N>> const& samples, BSplineCurve<T, N>& spline)
        {
            ...
            std::size_t const numSamples = samples.size();
            GTL_ARGUMENT_ASSERT(
            ...
            std::size_t last = input.uniqueKnots.size() - 1;
            ...
            T t;
            std::size_t i0, i1, i2, imin = 0, imax = 0;

            // Construct the matrix A^T*A.
            std::size_t numBands = (numControls > degree + 1 ? degree + 1 : degree);
    ...
    private:
        friend class UnitTestBSplineCurveFit;
    };
}
