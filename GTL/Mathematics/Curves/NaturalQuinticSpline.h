...
namespace gtl
{
    ...
    public:
        ...
        NaturalQuinticSpline(bool isFree, std::size_t numPoints,
            ...
            mDelta{}
        {
            ...
            std::size_t numPm1 = numPoints - 1;
            mPolynomials.resize(numPm1);
            mDelta.resize(numPm1);
            for (std::size_t i0 = 0, i1 = 1; i1 < numPoints; i0 = i1++)
            {
            ...
            std::size_t const numBElements = 4 * numPm1;
            ...
            {
                std::size_t const numPm2 = numPoints - 2;
        ...
        NaturalQuinticSpline(std::size_t numPoints, Vector<T, N> const* f0,
        ...
        NaturalQuinticSpline(std::vector<Vector<T, N>> const& f0,
            ...
            Vector<T, N> const& derivative0, Vector<T, N> const& derivative1)
            :
            NaturalQuinticSpline(f0.size(), f0.data(), f1.data(),
                times.data(), derivative0, derivative1)
        {
        }
    ...
    private:
        ...
        void Solve(std::size_t ell20, std::size_t ell31, std::size_t numPoints,
        ...
        void RowReduce(std::size_t ell20, std::size_t ell31, std::size_t numPoints,
            Matrix4x4<T>& R, std::vector<Vector<T, N>>& B)
        {
            ...
            Matrix4x4<T> const& R, std::vector<Vector<T, N>> const& B)
        {
            ...
            std::size_t const numPm2 = mPolynomials.size() - 2;
            for (std::size_t k = 0, i1 = numPm2, i0 = i1 + 1; k <= numPm2; ++k, i0 = i1--)
            {
        ...
        void GetKeyInfo(T const& t, std::size_t& key, T& u) const
        {
            std::size_t const numSegments = this->GetNumSegments();
            if (t > this->mTime[0])
            {
                if (t < this->mTime[numSegments])
                {
                    for (std::size_t i = 0, ip1 = 1; i < numSegments; i = ip1++)
                    {
                        if (t < this->mTime[ip1])
                        {
                            key = i;
    ...
    private:
        friend class UnitTestNaturalQuinticSpline;
    };
}
