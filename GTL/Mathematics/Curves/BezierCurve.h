...
namespace gtl
{
    ...
    public:
        ...
        // domain is t in [0,1].
        BezierCurve(std::size_t degree, Vector<T, N> const* controls)
            :
            ...
            mChoose{ mNumControls, mNumControls }
        {
            ...
            mChoose(1, 0) = C_<T>(1);
            mChoose(1, 1) = C_<T>(1);
            for (std::size_t i = 2; i <= mDegree; ++i)
            {
                mChoose(i, 0) = C_<T>(1);
                mChoose(i, i) = C_<T>(1);
    ...
    protected:
        ...
        void ComputeControlPointDifferences() const
        {
            ...
            for (std::size_t i = 0; i < mNumControls - 1; ++i)
            {
            ...
            for (std::size_t i = 0; i < mNumControls - 2; ++i)
            {
            ...
            if (mDegree >= 3)
            {
                ...
                for (std::size_t i = 0; i < mNumControls - 3; ++i)
                {
        ...
        Vector<T, N> Compute(T const& t, T const& omt, std::size_t order) const
        {
            Vector<T, N> result = omt * mControls[order][0];

            T tpow = t;
            std::size_t isup = mDegree - order;
            for (std::size_t i = 1; i < isup; ++i)
            {
