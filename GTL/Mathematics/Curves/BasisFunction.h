...
namespace gtl
{
    ...
    public:
        struct UniqueKnot
        {
            ...
            UniqueKnot(T const& inT, std::size_t inMultiplicity)
                :
                t(inT),
        ...
        public:
            ...
            Input(std::size_t inNumControls, std::size_t inDegree)
                :
                ...
                uniqueKnots{}
            {
                ...
                std::size_t const numUniqueKnots = numControls - degree + 1;
                ...
                for (std::size_t i = 1; i <= numUniqueKnots - 2; ++i)
                {
        ...
        void Create(Input const& input)
        {
            ...
            std::size_t const numUniqueKnots = input.uniqueKnots.size();
            ...
            std::size_t mult0 = mUniqueKnots.front().multiplicity;
            GTL_RUNTIME_ASSERT(
                mult0 >= 1 && mult0 <= mDegree + 1,
                "Invalid first multiplicity.");

            std::size_t mult1 = mUniqueKnots.back().multiplicity;
            GTL_RUNTIME_ASSERT(
            ...
            for (std::size_t i = 1; i <= numUniqueKnots - 2; ++i)
            {
                std::size_t mult = mUniqueKnots[i].multiplicity;
                GTL_RUNTIME_ASSERT(
            ...
            std::size_t sum = 0;
            for (std::size_t i = 0, j = 0; i < numUniqueKnots; ++i)
            {
                T tCommon = mUniqueKnots[i].t;
                std::size_t mult = mUniqueKnots[i].multiplicity;
                for (std::size_t k = 0; k < mult; ++k, ++j)
                {
                    mKnots[j] = tCommon;
                }
            ...
            std::size_t numRows = mDegree + 1;
            std::size_t numCols = mNumControls + mDegree;
        ...
        inline std::size_t GetNumUniqueKnots() const
        {
        ...
        inline std::size_t GetNumKnots() const
        {
            return mKnots.size();
        }
        ...
        void Evaluate(T t, std::size_t order, std::size_t& minIndex, std::size_t& maxIndex) const
        {
            GTL_ARGUMENT_ASSERT(
                order <= 3,
                "Invalid order.");

            std::size_t i = GetIndex(t);
            mJet[0](i, 0) = C_<T>(1);
            ...
            for (std::size_t j = 1; j <= mDegree; j++)
            {
            ...
            for (std::size_t j = 2; j <= mDegree; ++j)
            {
                for (std::size_t k = i - j + 1; k < i; ++k)
                {
                    n0 = t - mKnots[k];
                    n1 = mKnots[k + j + 1] - t;
        ...
        T const& GetValue(std::size_t order, std::size_t i) const
        {
            GTL_ARGUMENT_ASSERT(
    ...
    private:
        ...
        std::vector<std::pair<T, std::size_t>> mKeys;
    ...
    private:
        friend class UnitTestBasisFunction;
    };
}
