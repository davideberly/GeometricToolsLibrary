...
namespace gtl
{
    ...
    public:
        ...
        // Member access.
        inline std::size_t GetNumKeyFrames() const
        {
            return mPoint.size();
        }
    ...
    protected:
        ...
        {
            std::size_t const nm1 = mPoint.size() - 1;
        ...
        {
            std::size_t const n = mPoint.size();
            for (std::size_t km1 = 0, k = 1, kp1 = 2; kp1 < n; km1 = k, k = kp1++)
            {
            ...
            if (mLambda.size() > 0)
            {
                for (std::size_t k = 1, kp1 = 2; kp1 < n; k = kp1++)
                {
        ...
        {
            for (std::size_t k = 0, kp1 = 1; kp1 < mPoint.size(); k = kp1++)
            {
        ...
        void GetKeyInfo(T const& t, std::size_t& key, T& u) const
        {
            ...
            std::size_t const numSegments = mA.size();
    ...
    private:
        friend class UnitTestTCBSplineCurve;
    };
}
