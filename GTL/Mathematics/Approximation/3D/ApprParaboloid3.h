...
namespace gtl
{
    ...
    {
    public:
        // A successful fit is indicated by return value of 'true'. Set
        // the rootMeanSquareError to a nonnull pointer if you want the
        // least-squares error.

        // Fit with z = u0*x^2 + u1*x*y + u2*y^2 + u3*x + u4*y + u5
        static bool Fit(std::vector<Vector3<T>> const& points,
            std::array<T, 6>& u, T* rootMeanSquareError = nullptr)
        {
            return Fit(static_cast<std::int32_t>(points.size()), points.data(),
                u, rootMeanSquareError);
        }

        static bool Fit(std::size_t numPoints, Vector3<T> const* points,
            std::array<T, 6>& u, T* rootMeanSquareError = nullptr)
        {
            GTL_ARGUMENT_ASSERT(
                numPoints >= 6,
            ...
            Matrix<T, 6, 6> A{};  // creates the zero matrix
            Vector<T, 6> B{};  // creates the zero vector
            ...
            Vector<T, 6> solution{};
            bool success = LinearSystem<T>().Solve(A, B, solution);
            if (success)
            {
                for (std::size_t i = 0; i < 6; ++i)
                {
        ...
        static bool FitRobust(std::vector<Vector3<T>> const& points,
            Vector3<T>& average, std::array<T, 6>& v, T* rootMeanSquareError = nullptr)
        {
            return FitRobust(points.size(), points.data(), average, v, rootMeanSquareError);
        }

        static bool FitRobust(std::size_t numPoints, Vector3<T> const* points,
            Vector3<T>& average, std::array<T, 6>& v, T* rootMeanSquareError = nullptr)
        {
            GTL_ARGUMENT_ASSERT(
                numPoints >= 6,
    ...
    private:
        friend class UnitTestApprParaboloid3;
    };
}
