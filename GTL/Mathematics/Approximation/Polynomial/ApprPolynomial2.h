...
// these. Another alternative is to transform the (x,y)-values to the
// rectangle [-1,1]^2 by subtracting the center of their axis-aligned bounding
// rectangle and then uniformly scaling (x,y,w) so that the rectangle tightly
// contains the (x,y)-values. 
//
...
namespace gtl
{
    ...
    public:
            ...
            Polynomial<T, 2>& polynomial)
        {
            if (validateInput)
            {
                ValidateInput(xDegree, yDegree, observations);
            }

            Matrix<T> xPower{}, yPower{};
            ComputePowers(xDegree, yDegree, observations,
                xPower, yPower);

            Matrix<T> A{};
            Vector<T> B{};
            ComputeLinearSystemComponents(xDegree, yDegree, observations,
                xPower, yPower, A, B);

            return SolveLinearSystem(xDegree, yDegree,
                A, B, polynomial);
        }
    ...
    private:
        static void ValidateInput(
            ...
            {
                GTL_ARGUMENT_ASSERT(
                    xDegree > 0 && yDegree > 0 && observations.size() > 0,
                    "Invalid input.");
        }
            ...
            Vector<T>& B)
        {
            ...
            std::size_t numCoefficients = xDegreeP1 * yDegreeP1;
            ...
            Polynomial<T, 2>& polynomial)
        {
            ...
            if (LinearSystem<T>::Solve(A, B, coefficient))
            {
                ...
                for (std::size_t r = 0, i = 0; r < yDegreeP1; ++r)
                {
