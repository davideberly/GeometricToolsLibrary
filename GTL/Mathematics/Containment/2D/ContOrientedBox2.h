...
namespace gtl
{
    ...
    public:
        ...
        // the average of the points. The box axes are the eigenvectors of
        // the covariance matrix.
        static void GetContainer(std::vector<Vector2<T>> const& points, OrientedBox2<T>& box)
        {
        ...
        // center and U0 and U1 are the orthonormal axes of the box. X is in
        // the box when |y_i| <= E_i for all i, where E_i are the extents of
        // the box.
        static bool InContainer(Vector2<T> const& point, OrientedBox2<T> const& box)
        {
        ...
        // The result is not guaranteed to be the minimum area box containing
        // the input boxes.
        static void MergeContainers(OrientedBox2<T> const& box0,
            OrientedBox2<T> const& box1, OrientedBox2<T>& merge)
        {
            ...
            // The corresponding endpoints on the axes are C+min[i]*D[i]
            // and C+max[i]*D[i]. The point C is not necessarily the midpoint
            // for any of the intervals. The actual box center will be
            // adjusted from C to a point C' that is the midpoint of each
            // interval,
    ...
    private:
        friend class UnitTestContOrientedBox2;
    };
}
