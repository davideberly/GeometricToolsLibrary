...
{
    template <typename T>
    class ContTetrahedron3
    {
    public:
        // Test for containment of a point by a tetrahedron.
        static bool InContainer(Vector3<T> const& point, Tetrahedron3<T> const& tetra)
        {
            // A loop over the faces is not used in order to avoid redundant
            // computations of edge directions. The difference vector is the
            // same for the first 3 faces but differs for the last face.
    ...
    private:
        friend class UnitTestContTetrahedron3;
    };
}
