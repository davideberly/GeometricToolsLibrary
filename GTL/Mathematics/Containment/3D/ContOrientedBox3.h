...
{
    template <typename T>
    class ContOrientedBox3
    {
    public:
        // Compute an oriented bounding box of the points. The box center is
        // the average of the points. The box axes are the eigenvectors of the
        // covariance matrix.
        static void GetContainer(std::vector<Vector3<T>> const& points, OrientedBox3<T>& box)
        {
        ...
        // Test for containment. Let X = C + y0*U0 + y1*U1 + y2*U2 where C is
        // the box center and U0, U1 and U2 are the orthonormal axes of the
        // box. X is in the box if |y_i| <= E_i for all i where E_i are the
        // extents of the box.
        static bool InContainer(Vector3<T> const& point, OrientedBox3<T> const& box)
        {
        ...
        // The result is not guaranteed to be the minimum volume box
        // containing the input boxes.
        static void MergeContainers(OrientedBox3<T> const& box0,
            OrientedBox3<T> const& box1, OrientedBox3<T>& merge)
        {
            ...
            // axis D[i] containing the current center C has a minimum
            // projected value min[i] and a maximum projected value max[i].
            // The corresponding end points on the axes are C+min[i]*D[i] and
            // C+max[i]*D[i]. The point C is not necessarily the midpoint for
            // any of the intervals. The actual box center will be adjusted
            // from C to a point C' that is the midpoint of each interval,
            ...
            // [min,max] is the axis-aligned box in the coordinate system of 
            // the merged box axes. Update the current box center to be the
            // center of the new box. Compute the extents based on the new
            // center.
            for (std::size_t j = 0; j < 3; ++j)
            {
    ...
    private:
        friend class UnitTestContOrientedBox3;
    };
}
