...
namespace gtl
{
    template <typename T, std::size_t...> class CholeskyDecomposition;
    ...
    private:
        friend class UnitTestCholeskyDecomposition;
    };

    // Implementation for size known only at run time. To solve A*X = B,
    //   std::size_t const N = user_specified_dimension
    ...
    public:
        ...
        CholeskyDecomposition(std::size_t inN)
            :
            N(inN)
        {
            GTL_LENGTH_ASSERT(
