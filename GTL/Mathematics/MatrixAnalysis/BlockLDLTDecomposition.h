...
namespace gtl
{
    template <typename T, std::size_t...> class BlockLDLTDecomposition;
    ...
    public:
        ...
        void Get(BlockMatrix const& M, std::size_t row, std::size_t col, T& value)
        {
        ...
        void Set(BlockMatrix& M, std::size_t row, std::size_t col, T const& value)
        {
            std::size_t b0 = col / BlockSize;
            std::size_t b1 = row / BlockSize;
            std::size_t i0 = col - BlockSize * b0;
        ...
        {
            for (std::size_t row = 0; row < NumBlocks; ++row)
            {
                for (std::size_t col = 0; col < NumBlocks; ++col)
                {
                    MakeZero(L[row][col]);
                    MakeZero(D[row][col]);
                }
            ...
            for (std::size_t j = 0; j < NumBlocks; ++j)
            {
                ...
                MakeIdentity(L[j][j]);
                for (std::size_t i = j + 1; i < NumBlocks; ++i)
                {
    ...
    private:
        friend class UnitTestBlockLDLTDecomposition;
    };
    ...
    public:
        ...
        BlockLDLTDecomposition(std::size_t blockSize, std::size_t numBlocks)
            :
            BlockSize(blockSize),
            NumBlocks(numBlocks),
        ...
        void Get(BlockMatrix const& M, std::size_t row, std::size_t col, T& value, bool verifySize = true)
        {
            if (verifySize)
            {
                GTL_ARGUMENT_ASSERT(
        ...
        void Set(BlockMatrix& M, std::size_t row, std::size_t col, T const& value, bool verifySize = true)
        {
            if (verifySize)
            {
                GTL_ARGUMENT_ASSERT(
            ...
            BlockVector const& B, BlockVector& X, bool verifySize = true)
        {
            if (verifySize)
            {
                std::size_t const LDsize = NumBlocks * NumBlocks;
                GTL_ARGUMENT_ASSERT(
                    L.size() == LDsize &&
                    D.size() == LDsize &&
                    B.size() == NumBlocks,
