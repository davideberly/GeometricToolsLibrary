...
// GetEigenvector(std::size_t) for the desired eigenvectors.
//
...
namespace gtl
{
    template <typename T, std::size_t...> class SymmetricEigensolver;
    ...
    public:
        ...
        T const& GetEigenvalue(std::size_t i) const
        {
        ...
        std::array<T, 2> const& GetEigenvector(std::size_t i) const
        {
            GTL_OUTOFRANGE_ASSERT(
                i < 2,
                "The index must not exceed the matrix size.");
    ...
    public:
        ...
        //
        // The function std::size_t return value is the number of iterations used
        // by the iterative algorithm. The return value is 0 for the
        // noniterative algorithm.
        std::size_t operator()(T const& a00, T const& a01, T const& a02, T const& a11,
        ...
        // Get a single eigenvector.
        std::array<T, 3> const& GetEigenvector(std::size_t i) const
        {
            GTL_ARGUMENT_ASSERT(
                i < 3,
                "The index must not exceed the matrix size.");
    ...
    private:
        ...
        std::size_t SolveIterative(T const& a00, T const& a01, T const& a02, T const& a11,
            T const& a12, T const& a22, bool aggressive)
        {
            ...
            // matrices.
            std::size_t const maxIteration = static_cast<std::size_t>(
        ...
        std::size_t SolveNoniterative(T a00, T a01, T a02, T a11, T a12, T a22)
        {
    ...
    public:
        ...
        // iterations allowed by the solver. The function std::size_t return value
        // is the number of iterations used by the iterative algorithm.
        std::size_t operator()(std::size_t size, T const* input, std::size_t maxIterations)
        {
            GTL_ARGUMENT_ASSERT(
            ...
            for (std::size_t i = 0; i < mMatrix.size(); ++i)
            {
                mMatrix[i] = input[i];
            }
            ...
            Tridiagonalize();
            mGivens.clear();
            for (std::size_t j = 0; j < maxIterations; ++j)
            {
                std::size_t imin = invalid, imax = invalid;
                for (std::size_t k = 2, i = mSize - 2; k <= mSize; ++k, --i)
                {
        ...
        // Get a single eigenvector.
        std::vector<T> const& GetEigenvector(std::size_t i) const
        {
            GTL_OUTOFRANGE_ASSERT(
                i < mSize,
                "The index must not exceed the matrix size.");
    ...
    private:
        ...
        {
            std::size_t r{}, c{};
            for (std::size_t i = 0, ip1 = 1; i < mSize - 2; ++i, ++ip1)
            {
            ...
            std::size_t k{}, ksup = mSize - 1, index = 0, delta = mSize + 1;
        ...
        // T(imax,imax).
        void DoQRImplicitShift(std::size_t imin, std::size_t imax)
        {
            ...
            T a02 = C_<T>(0);
            std::size_t i0 = imin - 1, i1 = imin, i2 = imin + 1;
        ...
        void RepackageEigenstuff()
        {
            ...
            std::vector<std::size_t> permutation(mSize);
            i = 0;
        ...
        void ComputeEigenvectors(std::vector<std::size_t> const& permutation)
        {
            ...
            // accumulation.
            std::size_t r{}, c{};
            for (std::size_t k = 3, i = mSize - 3, rmin = i + 1; k <= mSize; ++k, --i, --rmin)
            {
            ...
            std::vector<std::size_t> visited(mSize, 0);
            for (std::size_t i = 0; i < mSize; ++i)
            {
                if (visited[i] == 0 && permutation[i] != i)
                {
                    ...
                    std::size_t start = i, current = i, next{};
        ...
        struct GivensRotation
        {
            ...
            GivensRotation(std::size_t inIndex, T inCs, T inSn)
                :
                index(inIndex),
                cs(inCs),
                sn(inSn)
