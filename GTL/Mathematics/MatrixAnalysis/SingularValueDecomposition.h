...
namespace gtl
{
    ...
    public:
        ...
        SingularValueDecomposition(std::size_t numRows, std::size_t numCols, std::size_t maxIterations)
            :
            mNumRows(numRows),
            mNumCols(numCols),
            ...
            mRGivens{}
        {
            ...
            for (std::size_t col = 0; col < mLHouseholder.size(); ++col)
            {
                mLHouseholder[col].resize(mNumRows);
            }

            mRHouseholder.resize(mNumCols - 2);
            for (std::size_t row = 0; row < mRHouseholder.size(); ++row)
            {
        ...
        // occurs or std::numeric_limits<std::size_t>::max() when convergence
        // does not occur.
        std::size_t Solve(T const* input, T multiplier = C_<T>(8))
        {
            GTL_ARGUMENT_ASSERT(
            ...
            mRGivens.clear();
            mLGivens.clear();
            for (std::size_t iteration = 0; iteration < mMaxIterations; ++iteration)
            {
                ...
                for (std::size_t i = 0; i <= mNumCols - 2; ++i)
                {
        ...
        T GetSingularValue(std::size_t index) const
        {
            GTL_ARGUMENT_ASSERT(
                index < mNumCols,
        ...
        void GetUColumn(std::size_t index, T* uColumn) const
        {
            GTL_ARGUMENT_ASSERT(
        ...
        void GetVColumn(std::size_t index, T* vColumn) const
        {
            GTL_ARGUMENT_ASSERT(
                index < mNumCols && vColumn != nullptr,
                "Invalid index or null pointer for V-column.");

            for (std::size_t row = 0; row < mNumCols; ++row)
            {
        ...
        void GetSingularValues(T* singularValues) const
        {
            ...
            for (std::size_t index = 0; index < mNumCols; ++index)
            {
    ...
    private:
        ...
        // size numRows.
        static void ComputeHouseholderU(std::size_t numRows, std::size_t numCols,
            T const* A, std::size_t selectCol, T* v)
        {
        ...
        // numRows-by-numCols with numRows >= numCols and the vector v has
        // size numCols.
        static void ComputeHouseholderV(std::size_t numRows, std::size_t numCols,
            T const* A, std::size_t selectRow, T* v)
        {
            ...
            // reference these elements.
            (void)numRows;
            std::size_t selectRowP1 = selectRow + 1;
            std::size_t col{};
        ...
        static void DoHouseholderPremultiply(std::size_t numRows, std::size_t numCols,
            T const* v, std::size_t selectCol, T* A)
        {
            T vSqrLength = C_<T>(0);
            for (std::size_t row = selectCol; row < numRows; ++row)
            {
            ...
            for (std::size_t col = selectCol; col < numCols; ++col)
            {
                w[col] = C_<T>(0);
                for (std::size_t row = selectCol; row < numRows; ++row)
                {
        ...
        static void DoHouseholderPostmultiply(std::size_t numRows, std::size_t numCols,
            T const* v, std::size_t selectRow, T* A)
        {
            T vSqrLength = C_<T>(0);
            for (std::size_t col = selectRow; col < numCols; ++col)
            {
            ...
            for (std::size_t row = selectRow; row < numRows; ++row)
            {
                w[row] = C_<T>(0);
                for (std::size_t col = selectRow; col < numCols; ++col)
                {
        ...
        void Bidiagonalize()
        {
            ...
            for (std::size_t d = 0; d < mNumCols; ++d)
            {
                mDiagonal[d] = mMatrix[d + mNumCols * d];
            }
            for (std::size_t s = 0; s < mNumCols - 1; ++s)
            {
        ...
        void ComputeCutoffs(T multiplier, T& delta, T& threshold) const
        {
            ...
            for (std::size_t i = 0; i < mNumCols - 1; ++i)
            {
        ...
        bool DiagonalEntriesNonzero(std::size_t iMin, std::size_t iMax, T threshold)
        {
        ...
        void DoGolubKahanStep(std::size_t iMin, std::size_t iMax)
        {
            ...
            T a02 = C_<T>(0);
            for (std::size_t i0 = iMin - 1, i1 = iMin, i2 = iMin + 1; i1 <= iMax - 1; ++i0, ++i1, ++i2)
            {
        ...
        void ComputeUOrthogonal()
        {
            ...
            for (std::size_t d = 0; d < mNumRows; ++d)
            {
            ...
            for (std::size_t k = 0, col = mNumCols - 1; k <= mNumCols - 1; ++k, --col)
            {
            ...
            {
                std::size_t j0 = givens.index0;
                std::size_t j1 = givens.index1;
                for (std::size_t row = 0; row < mNumRows; ++row, j0 += mNumRows, j1 += mNumRows)
                {
                    T& q0 = mUMatrix[j0];
                    T& q1 = mUMatrix[j1];
        ...
        void ComputeVOrthogonal()
        {
            ...
            for (auto const& givens : mRGivens)
            {
                std::size_t j0 = givens.index0;
                std::size_t j1 = givens.index1;
                for (std::size_t col = 0; col < mNumCols; ++col, j0 += mNumCols, j1 += mNumCols)
                {
                    T& q0 = mVMatrix[j0];
                    T& q1 = mVMatrix[j1];
                    T prd0 = givens.cs * q0 - givens.sn * q1;
        ...
        void SortSingularValues()
        {
            ...
            std::vector<std::size_t> permute(mNumCols);
            for (std::size_t i = 0; i < mNumCols; ++i)
            {
        ...
        struct GivensRotation
        {
            ...
            GivensRotation(std::size_t inIndex0, std::size_t inIndex1, T inCs, T inSn)
                :
                index0(inIndex0),
                index1(inIndex1),
                cs(inCs),
        ...
        struct SingularInfo
        {
            ...
            T singular;
            std::size_t inversePermute;
        };

    private:
        friend class UnitTestSingularValueDecomposition;
    };
}
