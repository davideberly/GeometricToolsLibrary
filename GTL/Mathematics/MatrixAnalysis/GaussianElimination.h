...
namespace gtl
{
    ...
    public:
        ...
        static bool SolveSystem(std::size_t numRows, std::size_t numCols, T const* M,
        ...
        // inverseM is invalid.
        static bool GetInverse(std::size_t numRows, T const* M, T* inverseM,
        ...
        static bool GetDeterminant(std::size_t numRows, T const* M, T& determinant,
            bool isRowMajor = true)
        {
        ...
        static bool GetInverseAndDeterminant(std::size_t numRows, T const* M,
    ...
    private:
        static bool Execute(std::size_t numRows, std::size_t numCols, T const* M,
            T* inverseM, T* determinant, T const* B, T* X, bool isRowMajor)
        {
            ...
            std::size_t const sizeM = numRows * numRows;
            std::size_t const sizeX = numRows * numCols;
            ...
            std::function<T& (std::size_t, std::size_t)> locInverseM{};
            std::function<T& (std::size_t, std::size_t)> locX{};
            if (isRowMajor)
            {
                locInverseM = [numRows, inverseM](std::size_t row, std::size_t col) -> T&
                {
                ...
                if (wantSolution)
                {
                    locX = [numCols, X](std::size_t row, std::size_t col) -> T&
                    {
            ...
            else
            {
                ...
                if (wantSolution)
                {
                    locX = [numRows, X](std::size_t row, std::size_t col) -> T&
                    {
            ...
            std::vector<std::size_t> rowIndex(numRows), colIndex(numRows);
            std::vector<std::size_t> pivoted(numRows, 0);
            for (std::size_t i0 = 0; i0 < numRows; ++i0)
            {
                // Search the matrix, excluding pivoted rows, for the maximum
                // absolute entry.
                T maxValue = C_<T>(0);
                for (std::size_t i1 = 0; i1 < numRows; ++i1)
                {
                    if (!pivoted[i1])
                    {
                        for (std::size_t i2 = 0; i2 < numRows; ++i2)
                        {
                            if (!pivoted[i2])
                            {
                ...
                if (wantSolution)
                {
                    for (std::size_t i2 = 0; i2 < numCols; ++i2)
                    {
            ...
            if (wantInverse)
            {
                ...
                std::size_t i1 = numRows - 1;
                for (std::size_t j = 0; j < numRows; ++j, --i1)
                {
                ...
                for (std::size_t i = 0; i < storageInvM.size(); ++i)
                {
    ...
    private:
        friend class UnitTestGaussianElimination;
    };
    ...
    {
        std::size_t const N = M.GetNumRows();
        GTL_ARGUMENT_ASSERT(
            N > 0 && N == M.GetNumCols() &&
            N == B.size() && N == X.size(),
    ...
    {
        std::size_t const N = M.GetNumRows();
        std::size_t const K = B.GetNumCols();
        GTL_ARGUMENT_ASSERT(
            N > 0 && N == M.GetNumCols() &&
