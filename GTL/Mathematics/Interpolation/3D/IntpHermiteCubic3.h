...
namespace gtl
{
    ...
    public:
        ...
        IntpHermiteCubic3(std::array<std::array<std::array<Sample, 2>, 2>, 2> const& blocks)
            :
            c{}
        {
            Generate(blocks);
            ...
            T const& x, T const& y, T const& z) const
        {
            ...
            if (xOrder <= 3 && yOrder <= 3 && zOrder <= 3)
            {
                ...
                for (std::size_t i = 0; i < 4; ++i)
                {
                    T xValue = IntpHermiteCubic1<T>::P(i, xOrder, x);
                    for (std::size_t j = 0; j < 4; ++j)
                    {
                        T yValue = IntpHermiteCubic1<T>::P(j, yOrder, y);
                        T xyValue = xValue * yValue;
                        for (std::size_t k = 0; k < 4; ++k)
                        {
                            T zValue = IntpHermiteCubic1<T>::P(k, zOrder, z);
