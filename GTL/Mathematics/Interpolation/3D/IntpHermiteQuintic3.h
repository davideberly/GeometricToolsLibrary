...
namespace gtl
{
    ...
    public:
        ...
        IntpHermiteQuintic3(std::array<std::array<std::array<Sample, 2>, 2>, 2> const& blocks)
            :
            c{}
        {
            Generate(blocks);
            ...
            T const& x, T const& y, T const& z) const
        {
            ...
            if (xOrder <= 5 && yOrder <= 5 && zOrder <= 5)
            {
                ...
                for (std::size_t i = 0; i < 6; ++i)
                {
                    T xValue = IntpHermiteQuintic1<T>::P(i, xOrder, x);
                    for (std::size_t j = 0; j < 6; ++j)
                    {
                        T yValue = IntpHermiteQuintic1<T>::P(j, yOrder, y);
                        T xyValue = xValue * yValue;
                        for (std::size_t k = 0; k < 6; ++k)
                        {
                            T zValue = IntpHermiteQuintic1<T>::P(k, zOrder, z);
