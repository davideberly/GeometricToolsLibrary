...
namespace gtl
{
    // The rigid body state is stored in a separate structure so that the
    // Force and Torque functionals can be passed a single object to avoid a
    // large number of parameters that would otherwise have to be passed to
    // the functionals. This makes the Runge-Kutta ODE solver easier to read.
    // The RigidBody<T> class provides wrappers around the state accessors to
    // avoid exposing the private state member.
    template <typename T>
    ...
    public:
        ...
        void SetQOrientation(Quaternion<T> const& qOrientation, bool normalize = false)
        {
            ...
            RigidMotion<T>::Convert(qOrientation, mROrientation);
            if (IsMovable())
            {
                UpdateWorldInertialQuantities();
            }
        ...
        void SetROrientation(Matrix3x3<T> const& rOrientation)
        {
            ...
            RigidMotion<T>::Convert(rOrientation, mQOrientation);
            if (IsMovable())
            {
                UpdateWorldInertialQuantities();
            }
    ...
    private:
        friend class UnitTestRigidBody;
    };
    ...
    public:
        ...
        void Update(T const& t, T const& dt)
        {
            ...
            Quaternion<T> A1DQDT = C_<T>(1, 2) * W * GetQOrientation();
            ...
            Quaternion<T> A2DQDT = C_<T>(1, 2) * W * newState.GetQOrientation();
            ...
            W = newState.GetQAngularVelocity();
            Quaternion<T> A3DQDT = C_<T>(1, 2) * W * newState.GetQOrientation();
            ...
            W = newState.GetQAngularVelocity();
            Quaternion<T> A4DQDT = C_<T>(1, 2) * W * newState.GetQOrientation();
            Vector3<T> A4DPDT = Force(TpDT, newState);
            Vector3<T> A4DLDT = Torque(TpDT, newState);

            SetPosition(GetPosition() +
                sixthDT * (A1DXDT + C_<T>(2) * (A2DXDT + A3DXDT) + A4DXDT));

            SetQOrientation(GetQOrientation() +
                sixthDT * (A1DQDT + C_<T>(2) * (A2DQDT + A3DQDT) + A4DQDT), true);

            SetLinearMomentum(GetLinearMomentum() +
                sixthDT * (A1DPDT + C_<T>(2) * (A2DPDT + A3DPDT) + A4DPDT));

            SetAngularMomentum(GetAngularMomentum() +
                sixthDT * (A1DLDT + C_<T>(2) * (A2DLDT + A3DLDT) + A4DLDT));
        }
    ...
    public:
        ...
        virtual void ApplyImpulse()
        {
            ...
            if (T0 != vzero)
            {
                ...
                Vector3<T> sysInput{};
                sysInput[0] = -(C_<T>(1) + restitution) * Dot(N, velDiffNeg);
                sysInput[1] = C_<T>(0);
                sysInput[2] = C_<T>(0);
                Vector3<T> sysOutput{};
            ...
            else
            {
                ...
                T numer = -(C_<T>(1) + restitution) * Dot(N, velDiffNeg);
