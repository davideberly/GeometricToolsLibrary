...
// index array has numTriangles such triples. The Boolean value
...
namespace gtl
{
    ...
    {
    public:
        static void Execute(Vector3<T> const* vertices, std::size_t numTriangles,
            std::size_t const* indices, bool bodyCoords, T& mass, Vector3<T>& center,
            Matrix3x3<T>& inertia)
        {
            T const oneDiv6 = C_<T>(1, 6);
            T const oneDiv24 = C_<T>(1, 24);
            T const oneDiv60 = C_<T>(1, 60);
            T const oneDiv120 = C_<T>(1, 120);

            // order: 1, x, y, z, x^2, y^2, z^2, xy, yz, zx
            std::array<T, 10> integral{};
            integral.fill(C_<T>(0));

            std::size_t const* index = indices;
            for (std::size_t i = 0; i < numTriangles; ++i)
            {
                // Get vertices of triangle i.
                Vector3<T> const& v0 = vertices[*index++];
                Vector3<T> const& v1 = vertices[*index++];
                Vector3<T> const& v2 = vertices[*index++];

                // Get cross product of edges and normal vector.
                Vector3<T> V1mV0 = v1 - v0;
                Vector3<T> V2mV0 = v2 - v0;
                Vector3<T> N = Cross(V1mV0, V2mV0);
            ...
            // center of mass
            center = { integral[1], integral[2], integral[3] };
            center /= mass;
    ...
    private:
        friend class UnitTestPolyhedralMassProperties;
    };
}
