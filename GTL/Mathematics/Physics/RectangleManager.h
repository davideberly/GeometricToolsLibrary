...
namespace gtl
{
    ...
    public:
        // Construction.
        RectangleManager(std::vector<AlignedBox2<T>>& rectangles)
            :
        ...
        void Initialize()
        {
            ...
            std::size_t intrSize = mRectangles.size();
            ...
            for (std::size_t i = 0; i < endpSize; ++i)
            {
                ...
                if (endpoint.type == 0)  // an interval 'begin' value
                {
                    ...
                    for (auto activeIndex : active)
                    {
                        ...
                        AlignedBox2<T> const& r0 = mRectangles[activeIndex];
                        AlignedBox2<T> const& r1 = mRectangles[index];
        ...
        void SetRectangle(std::size_t i, AlignedBox2<T> const& rectangle)
        {
        ...
        inline void GetRectangle(std::size_t i, AlignedBox2<T>& rectangle) const
        {
    ...
    private:
        ...
        void InsertionSort(std::vector<Endpoint>& endpoint, std::vector<std::size_t>& lookup)
        {
            ...
            TIQuery<T, AlignedBox2<T>, AlignedBox2<T>> query{};
        ...
        std::vector<AlignedBox2<T>>& mRectangles;
        ...
        std::vector<std::size_t> mXLookup, mYLookup;

    private:
        friend class UnitTestRectangleManager;
    };
}
