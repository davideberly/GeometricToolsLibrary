...
namespace gtl
{
    ...
    {
    public:
        static T Integrate(std::size_t order, T const& a, T const& b,
            std::function<T(T)> const& integrand)
        {
            ...
            for (std::size_t i0 = 2, p0 = 1; i0 <= order; ++i0, p0 *= 2, h *= C_<T>(1, 2))
            {
                // Approximations via the trapezoid rule.
                T sum = C_<T>(0);
                for (std::size_t i1 = 1; i1 <= p0; ++i1)
                {
                ...
                T p2 = C_<T>(4);
                for (std::size_t i2 = 1; i2 < i0; ++i2, p2 *= C_<T>(4))
                {
