...
namespace gtl
{
    ...
    {
    public:
        static void ComputeRootsAndCoefficients(std::size_t degree, std::size_t maxBisections,
            std::size_t precision, std::vector<T>& roots, std::vector<T>& coefficients)
        {
            ...
            for (std::size_t i = 0; i <= finalPoly.GetDegree(); ++i)
            {
            ...
            for (std::size_t i = 0; i < ratRoots.size(); ++i)
            {
                roots[i] = ratRoots[i];
            }

            coefficients.resize(roots.size());
            std::size_t n = roots.size() - 1;
            std::vector<T> subroots(n);
            for (std::size_t i = 0; i < roots.size(); ++i)
            {
                T denominator = C_<T>(1);
                for (std::size_t j = 0, k = 0; j < roots.size(); ++j)
                {
                    if (j != i)
                    {
                        subroots[k++] = roots[j];
    ...
    private:
        ...
        inline static void Convert(SourceType const& source,
            RootsPolynomial::BSN& target, std::size_t = 0)
        {
        ...
        inline static void Convert(SourceType const& source,
            RootsPolynomial::BSN& target, std::size_t precision)
        {
            source.Convert(precision, APRoundingMode::TO_NEAREST, target);
        }
