...
namespace gtl
{
    ...
    public:
        ...
        // polynomial further.
        inline void SetDegree(std::size_t degree)
        {
            std::size_t oldSize = mCoefficient.size();  // oldDegree = oldSize - 1
            std::size_t newSize = degree + 1;  // newDegree = degree
        ...
        inline CType const& operator[](std::size_t i) const
        {
            return mCoefficient[i];
        }

        inline CType& operator[](std::size_t i)
        {
            return mCoefficient[i];
        }
        ...
        template <std::size_t _N = N, TraitSelector<_N == 1> = 0>
    ...
    private:
        friend class UnitTestPolynomial;
    };
    ...
    Polynomial<T, N> operator-(Polynomial<T, N> const& p)
    {
        ...
        for (std::size_t i = 0; i <= result.GetDegree(); ++i)
        {
            result[i] = -result[i];
        }
        return result;
    ...
    {
        std::size_t const p0Degree = p0.GetDegree();
        std::size_t const p1Degree = p1.GetDegree();
        if (p0Degree >= p1Degree)
        {
            Polynomial<T, N> result = p0;
            for (std::size_t i = 0; i <= p1Degree; ++i)
            {
                result[i] += p1[i];
            }
        ...
        else
        {
            ...
            for (std::size_t i = 0; i <= p0Degree; ++i)
            {
                result[i] += p0[i];
            }
            EliminateLeadingZeros(result);
    ...
    Polynomial<T, N> operator*(Polynomial<T, N> const& p0, Polynomial<T, N> const& p1)
    {
        ...
        for (std::size_t i0 = 0; i0 <= p0Degree; ++i0)
        {
            for (std::size_t i1 = 0; i1 <= p1Degree; ++i1)
            {
    ...
    template <typename T>
    Polynomial1<T> MultiplyByXToPower(std::size_t power, Polynomial1<T> const& p0)
    {
        if (power > 0)
        {
            std::size_t const p0Degree = p0.GetDegree();
            std::size_t const p1Degree = p0Degree + power;
            Polynomial1<T> p1(p1Degree);
            for (std::size_t i = 0, src = p0Degree, trg = p1Degree; i <= p0Degree; ++i, --src, --trg)
            {
                p1[trg] = p0[src];
            }
    ...
    Polynomial1<T> GetDerivative(Polynomial1<T> const& p)
    {
        ...
        if (degree > 0)
        {
            ...
            for (std::size_t i0 = 0, i1 = 1; i0 < degree; ++i0, ++i1)
            {
    ...
    Polynomial1<T> GetTranslation(Polynomial1<T> const& p, T const& x0)
    {
        ...
        for (std::size_t i = 1, j = degree - 1; i <= degree; ++i, --j)
        {
        ...
        Polynomial1<Rational>& remainder)          // R(x)
    {
        ...
        std::size_t const dDegree = denominator.GetDegree();
        Rational const& dLeading = denominator[dDegree];
        remainder = numerator;
        std::size_t rDegree = remainder.GetDegree();
        if (rDegree >= dDegree)
        {
        ...
        Polynomial1<Rational>& remainder)
    {
        ...
        Rational const zero = C_<Rational>(0);
        std::size_t dDegree = denominator.GetDegree();
        Rational const& dLeading = denominator[dDegree];
        remainder = numerator;
        std::size_t rDegree = remainder.GetDegree();
        if (rDegree >= dDegree)
        {
            ...
            while (rDegree >= dDegree && !IsZero(remainder))
            {
                ...
                for (std::size_t i = 0; i < rDegree; ++i)
                {
            ...
            for (std::size_t i = 0; i <= quotient.GetDegree(); ++i)
            {
                if (quotient[i] != zero)
                {
