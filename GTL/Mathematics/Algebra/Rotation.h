...
// Conversions among various representations of rotations. Rotation axes must
// be unit length. The angles are in radians. The Euler angles are in world
// coordinates.
...
namespace gtl
{
    ...
    {
    public:
        // Create rotations from various representations. The static Convert
        // functions can be used directly. The constructors and implicit
        // conversion operators are provided for convenience. For example,
        //
        //   // Convert a quaternion to a rotation matrix.
        //   Quaternion<T> quaternion(...);
        //   Matrix3x3<T> matrix = Rotation<T>(q);
        //
        //   // Convert an axis-angle object to Euler angles using the
        //   // specified ordering of coordinate-axis rotations.
        //   AxisAngle<T> axisAngle(...);
        //   EulerAngles<T> eulerAngles = Rotation<T>(axisAngle)(1,0,2);

        Rotation(Matrix3x3<T> const& matrix)
            :
        ...
        Rotation(Quaternion<T> const& quaternion)
            :
        ...
        Rotation(AxisAngle<T> const& axisAngle)
            :
        ...
        Rotation(EulerAngles<T> const& eulerAngles)
            :
        ...
        EulerAngles<T> const& operator()(std::size_t i0, std::size_t i1, std::size_t i2) const
        {
        ...
        static void Convert(Matrix3x3<T> const& r, EulerAngles<T>& e)
        {
                ...
                e.axis[1] != e.axis[2])
            {
                if (e.axis[0] != e.axis[2])
                {
                    ...
                    std::size_t parity = (((e.axis[2] | (e.axis[1] << 2)) >> e.axis[0]) & 1);
                ...
                else
                {
                    ...
                    std::size_t b0 = 3 - e.axis[1] - e.axis[2];
                    std::size_t parity = (((b0 | (e.axis[1] << 2)) >> e.axis[2]) & 1);
                    T const sgn = (parity & 1 ? C_<T>(1) : -C_<T>(1));
        ...
        static void Convert(EulerAngles<T> const& e, Matrix3x3<T>& r)
        {
                ...
                e.axis[1] != e.axis[2])
            {
                ...
                MakeUnit(e.axis[0], unit0);
                MakeUnit(e.axis[1], unit1);
                MakeUnit(e.axis[2], unit2);
    ...
    private:
        ...
        Type mType;
        mutable Matrix3x3<T> mMatrix;
        mutable Quaternion<T> mQuaternion;
        mutable AxisAngle<T> mAxisAngle;
