...
namespace gtl
{
    ...
    // the matrix elements.
    template <typename T, std::size_t...> class Matrix;

    template <typename T, std::size_t RowDimension, std::size_t ColDimension>
    struct MatrixTraits
    {
        using value_type = T;
        static std::size_t constexpr NumRows = RowDimension;
        static std::size_t constexpr NumCols = ColDimension;
    };
}
...
namespace gtl
{
    ...
    public:
        ...
        inline std::size_t constexpr GetNumRows() const noexcept
        {
            return NumRows;
        }

        inline std::size_t constexpr GetNumCols() const noexcept
        {
            return NumCols;
        }

        std::size_t GetIndex(std::size_t row, std::size_t col) const noexcept
        {
        ...
        inline T const& at(std::size_t row, std::size_t col) const
        {
        ...
        void SetRow(std::size_t row, Vector<T, NumCols> const& v)
        {
        ...
        void SetCol(std::size_t col, Vector<T, NumRows> const& v)
        {
            GTL_OUTOFRANGE_ASSERT(
                col < NumCols,
                "Invalid column.");
        ...
        Vector<T, NumCols> GetRow(std::size_t row) const
        {
            GTL_OUTOFRANGE_ASSERT(
                row < NumRows,
                "Invalid row.");
        ...
        Vector<T, NumRows> GetCol(std::size_t col) const
        {
            GTL_OUTOFRANGE_ASSERT(
                col < NumCols,
                "Invalid column.");
        ...
        static Matrix<T, NumRows, NumCols> Identity()
        {
            ...
            std::size_t constexpr numDiagonal = std::min(NumRows, NumCols);
            for (std::size_t d = 0; d < numDiagonal; ++d)
            {
                identity(d, d) = C_<T>(1);
            }
            return identity;
...
namespace gtl
{
    ...
    public:
        ...
        // initialized to 0.
        Matrix(std::size_t numRows = 0, std::size_t numCols = 0)
            :
            mNumRows(numRows),
            mNumCols(numCols),
        ...
        inline std::size_t const GetNumRows() const noexcept
        {
            return mNumRows;
        }

        inline std::size_t const GetNumCols() const noexcept
        {
            return mNumCols;
        }
        ...
        void SetRow(std::size_t row, Vector<T> const& v)
        {
            GTL_OUTOFRANGE_ASSERT(
        ...
        void SetCol(std::size_t col, Vector<T> const& v)
        {
            GTL_OUTOFRANGE_ASSERT(
        ...
        Vector<T> GetCol(std::size_t col) const
        {
            GTL_OUTOFRANGE_ASSERT(
                col < mNumCols,
                "Invalid column.");
        ...
        static Matrix<T> Zero(std::size_t numRows, std::size_t numCols)
        {
            Matrix<T> zero(numRows, numCols);
            return zero;
        }

        static Matrix<T> Identity(std::size_t numRows, std::size_t numCols)
        {
        ...
        static Matrix<T> Unit(std::size_t row, std::size_t col)
        {
            Matrix<T> unit{};
            MakeUnit(row, col, unit);
            return unit;
        }
...
namespace gtl
{
    ...
    template <typename T, std::size_t NumRows, std::size_t NumCols>
    void MakeUnit(std::size_t row, std::size_t col, Matrix<T, NumRows, NumCols>& M)
    {
        GTL_LENGTH_ASSERT(
    ...
    template <typename T, std::size_t NumRows, std::size_t NumCols>
    bool IsUnit(std::size_t rowQuery, std::size_t colQuery, Matrix<T, NumRows, NumCols> const& M)
    {
        GTL_LENGTH_ASSERT(
    ...
    // M0 * M1
    template <typename T, std::size_t NumRows, std::size_t NumCols, std::size_t NumCommon>
        ...
        Matrix<T, NumCommon, NumCols> const& M1)
    {
        ...
        for (std::size_t row = 0; row < NumRows; ++row)
        {
            for (std::size_t col = 0; col < NumCols; ++col)
            {
                for (std::size_t i = 0; i < NumCommon; ++i)
                {
...
namespace gtl
{
    ...
    bool operator==(Matrix<T> const& M0, Matrix<T> const& M1)
    {
        ...
        for (std::size_t row = 0; row < M0.GetNumRows(); ++row)
        {
            for (std::size_t col = 0; col < M0.GetNumCols(); ++col)
            {
                if (M0(row, col) != M1(row, col))
                {
                    return false;
    ...
    // standard Euclidean basis for matrices.
    template <typename T>
    void MakeUnit(std::size_t row, std::size_t col, Matrix<T>& M)
    {
        GTL_LENGTH_ASSERT(
    ...
    // element is 1 and all other elements are 0.
    template <typename T>
    bool IsUnit(std::size_t rowQuery, std::size_t colQuery, Matrix<T> const& M)
    {
        GTL_LENGTH_ASSERT(
            rowQuery < M.GetNumRows() && colQuery < M.GetNumCols(),
            "Invalid row or column.");

        for (std::size_t row = 0; row < M.GetNumRows(); ++row)
        {
            for (std::size_t col = 0; col < M.GetNumCols(); ++col)
            {
                if (row != rowQuery || col != colQuery)
                {
                    if (M(row, col) != C_<T>(0))
    ...
    Matrix<T> OuterProduct(Vector<T> const& v0, Vector<T> const& v1)
    {
        ...
        for (std::size_t row = 0; row < v0.size(); ++row)
        {
            for (std::size_t col = 0; col < v1.size(); ++col)
            {
                result(row, col) = v0[row] * v1[col];
            }
        }
    ...
    Matrix<T> MultiplyAB(Matrix<T> const& M0, Matrix<T> const& M1)
    {
        ...
        std::size_t const numCommon = M0.GetNumCols();
        for (std::size_t row = 0; row < result.GetNumRows(); ++row)
        {
            for (std::size_t col = 0; col < result.GetNumCols(); ++col)
            {
                for (std::size_t i = 0; i < numCommon; ++i)
                {
                    result(row, col) += M0(row, i) * M1(i, col);
    ...
    Matrix<T> MultiplyATB(Matrix<T> const& M0, Matrix<T> const& M1)
    {
        ...
        std::size_t const numCommon = M0.GetNumRows();
        for (std::size_t row = 0; row < result.GetNumRows(); ++row)
        {
            for (std::size_t col = 0; col < result.GetNumCols(); ++col)
            {
    ...
    Matrix<T> HLift(Matrix<T> const& M)
    {
        ...
        std::size_t const numRows = M.GetNumRows();
        Matrix<T> result(numRows + 1, numRows + 1);
        for (std::size_t row = 0; row < numRows; ++row)
        {
            for (std::size_t col = 0; col < numRows; ++col)
            {
                result(row, col) = M(row, col);
            }
    ...
    Matrix<T> HProject(Matrix<T> const& M)
    {
        ...
        std::size_t const numRowsM1 = M.GetNumRows() - 1;
        Matrix<T> result(numRowsM1, numRowsM1);
        for (std::size_t row = 0; row < numRowsM1; ++row)
        {
            for (std::size_t col = 0; col < numRowsM1; ++col)
            {
                result(row, col) = M(row, col);
            }
        }
