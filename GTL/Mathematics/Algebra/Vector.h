...
namespace gtl
{
    ...
    {
        using value_type = T;
        static std::size_t constexpr N = Dimension;
    };
}
...
namespace gtl
{
    ...
    public:
        ...
        // Container access.
        inline T const& at(std::size_t i) const
        {
        ...
        inline static Vector<T, N> Unit(std::size_t d)
        {
            Vector<T, N> v{};
            MakeUnit(d, v);
            return v;
...
namespace gtl
{
    ...
    public:
        ...
        // initialized to zero.
        Vector(std::size_t numElements = 0)
            :
        ...
        // Size and data access.
        inline std::size_t const size() const noexcept
        {
        ...
        inline static Vector<T> Zero(std::size_t numElements)
        {
            return Vector<T>(numElements);
        }

        inline static Vector<T> One()
        {
            Vector<T> v{};
            MakeOne(v);
            return v;
        ...
        inline static Vector<T> Unit(std::size_t d)
        {
            Vector<T> v{};
            MakeUnit(d, v);
            return v;
...
namespace gtl
{
    ...
    template <typename T, std::size_t N>
    void MakeUnit(std::size_t d, Vector<T, N>& v)
    {
    ...
    template <typename T, std::size_t N>
    bool IsUnit(std::size_t d, Vector<T, N> const& v)
    {
        GTL_OUTOFRANGE_ASSERT(
            d < N,
            "Invalid dimension.");
    ...
    T NormalizeRobust(Vector<T, N>& v)
    {
        ...
        if (cmax > C_<T>(0))
        {
            std::int32_t cmaxExponent{};
            (void)std::frexp(cmax, &cmaxExponent);
            T length = C_<T>(0);
            for (std::size_t i = 0; i < N; ++i)
            {
                std::int32_t vExponent{};
    ...
    template <typename T, std::size_t N, std::size_t NumArray>
    std::pair<Vector<T, N>, Vector<T, N>> ComputeExtremes(std::array<Vector<T, N>, NumArray> const& v)
    {
        static_assert(
            NumArray > 0,
        ...
        for (std::size_t j = 1; j < NumArray; ++j)
        {
            auto const& vec = v[j];
            for (std::size_t i = 0; i < N; ++i)
            {
    ...
    template <typename T, std::size_t N>
    Vector<T, N + 1> Lift(Vector<T, N> const& v, std::size_t inject, T const& value)
    {
        GTL_OUTOFRANGE_ASSERT(
            inject <= N,
            "Invalid index.");
    ...
    template <typename T, std::size_t N>
    Vector<T, N - 1> Project(Vector<T, N> const& v, std::size_t reject)
    {
        static_assert(
            N > 1,
            "Invalid dimension for a projection.");
        ...
        Vector<T, N - 1> result{};
        for (std::size_t i = 0, j = 0; i < N - 1; ++i, ++j)
        {
            if (j == reject)
            {
                ++j;
...
namespace gtl
{
    ...
    // convenience for creating the standard Euclidean basis for vectors.
    template <typename T>
    void MakeUnit(std::size_t d, Vector<T>& v)
    {
        GTL_LENGTH_ASSERT(
            d < v.size(),
            "Invalid dimension.");
    ...
    // and all other elements are 0.
    template <typename T>
    bool IsUnit(std::size_t d, Vector<T> const& v)
    {
        GTL_OUTOFRANGE_ASSERT(
            d < v.size(),
            "Invalid dimension.");
    ...
    // Compute the axis-aligned bounding box of the vectors.
    template <typename T, std::size_t NumArray>
    std::pair<Vector<T>, Vector<T>> ComputeExtremes(std::array<Vector<T>, NumArray> const& v)
    {
        static_assert(
            NumArray > 0,
            "The input must have at least one vector.");
        ...
        GTL_OUTOFRANGE_ASSERT(
            v[0].size() > 0,
            "The inputs must have at least one dimension.");
    ...
    Vector<T> HProject(Vector<T> const& v)
    {
        ...
        std::size_t const sizem1 = size - 1;
        Vector<T> result(sizem1);
        for (std::size_t i = 0; i< sizem1; ++i)
        {
            result[i] = v[i];
        }
        return result;
    ...
    // is a (n-inject)-tuple [nonexistent when inject=n].
    template <typename T>
    Vector<T> Lift(Vector<T> const& v, std::size_t inject, T const& value)
    {
        std::size_t const size = v.size();
        GTL_OUTOFRANGE_ASSERT(
            size > 0 && inject <= size,
    ...
    // is a (n-1-reject)-tuple [nonexistent when reject=n-1].
    template <typename T>
    Vector<T> Project(Vector<T> const& v, std::size_t reject)
    {
        std::size_t const size = v.size();
        GTL_OUTOFRANGE_ASSERT(
            size > 1 && reject < size,
        ...
        std::size_t const sizem1 = size - 1;
        Vector<T> result(sizem1);
        for (std::size_t i = 0, j = 0; i < sizem1; ++i, ++j)
        {
            if (j == reject)
            {
                ++j;
...
namespace gtl
{
    ...
    // false, the outputs v0 and v1 are invalid (both set to the zero vector).
    template <typename T>
    ...
    // set. The values bary[] are valid only when the return value is 'true'
    ...
    public:
        ...
        void operator()(std::size_t numPoints, Vector2<T> const* points, T const& inEpsilon)
        {
            GTL_ARGUMENT_ASSERT(
            ...
            // and max.
            std::array<std::size_t, 2> indexMin = { 0, 0 };
            std::array<std::size_t, 2> indexMax = { 0, 0 };
        ...
        std::array<std::size_t, 3> extreme;
        bool extremeCCW;
    };
}
...
namespace gtl
{
    ...
    // outputs v0, v1 and v2 are invalid (all set to the zero vector).
    template <typename T>
    bool ComputeOrthonormalBasis(std::size_t numInputs, Vector3<T>& v0,
        Vector3<T>& v1, Vector3<T>& v2)
    {
        GTL_ARGUMENT_ASSERT(
            1 <= numInputs && numInputs <= 2,
    ...
    // complement of a unit-length vector v2. The algorithm does not require
    // a call to std::sqrt. See
    // https://www.geometrictools.com/Documentation/FastOrthogonalComplement.pdf
    //
    template <typename T>
    void ComputeOrthogonalComplement(Vector3<T> const& v2, Vector3<T>& v0, Vector3<T>& v1)
    {
        T const zero = C_<T>(0);
        T const one = C_<T>(1);
        T temp0{}, temp1{}, temp2{};
    ...
    // a linearly independent set. The values bary[] are valid only when the
    // return value is 'true' but set to zero when the return value is
    // 'false'.
    template <typename T>
    ...
    public:
        ...
        void operator()(std::size_t numPoints, Vector3<T> const* points, T const& inEpsilon)
        {
            GTL_ARGUMENT_ASSERT(
                numPoints > 0 && points != nullptr && inEpsilon >= C_<T>(0),
                "Invalid number of points, points pointer or epsilon.");
            ...
            // Keep track of the indices into 'points' for the current min
            // and max.
            std::array<std::size_t, 3> indexMin = { 0, 0, 0 };
            std::array<std::size_t, 3> indexMax = { 0, 0, 0 };
        ...
        std::array<std::size_t, 4> extreme;
        bool extremeCCW;
    };
}
...
namespace gtl
{
    ...
    // v1, v2 and v3 are invalid (all set to the zero vector).
    template <typename T>
    bool ComputeOrthonormalBasis(std::size_t numInputs, Vector4<T>& v0,
    ...
    // Support for 3D operations applied to vectors as 4D homogeneous points.
    ...
    // v1 = (y0,y1,y2).
    template <typename T>
    Vector4<T> Cross(Vector4<T> const& v0, Vector4<T> const& v1)
    {
        Vector4<T> result{};
    ...
    // Compute the normalized cross product.
    template <typename T>
    Vector4<T> UnitCross(Vector4<T> const& v0, Vector4<T> const& v1)
    {
        Vector4<T> unitCross = Cross(v0, v1);
        Normalize(unitCross);
        return unitCross;
    }
    ...
    // v2 is (z0,z1,z2).
    template <typename T>
    T DotCross(Vector4<T> const& v0, Vector4<T> const& v1, Vector4<T> const& v2)
    {
    ...
    //
    template <typename T>
    void ComputeOrthogonalComplement(Vector4<T> const& v2, Vector4<T>& v0, Vector4<T>& v1)
    {
        T const zero = C_<T>(0);
        T const one = C_<T>(1);
        T temp0{}, temp1{}, temp2{};
