...
namespace gtl
{
    ...
    // corresponding to the quaternion q, the rotated vector v = R*u is
    // computed by this function.
    template <typename T>
    Vector3<T> Rotate(Quaternion<T> const& q, Vector3<T> const& u)
    {
        Vector3<T> v{ q[0], q[1], q[2] };
        Vector3<T> t = C_<T>(2) * Cross(v, u);
        Vector3<T> rotatedU = u + q[3] * t + Cross(v, t);
        return rotatedU;
    }
    ...
    // the rotated vector v = R*u is computed by this function.
    template <typename T>
