...
namespace gtl
{
    ...
    private:
        ...
        T SqrDistance(Vector<T, N> const& e, Vector<T, N> const& y, Vector<T, N>& x)
        {
            ...
            std::array<std::pair<T, std::size_t>, N> permute{};
            for (std::size_t i = 0; i < N; ++i)
            {
                permute[i].first = -e[i];
                permute[i].second = i;
            ...
            std::array<std::size_t, N> invPermute{};
            for (std::size_t i = 0; i < N; ++i)
            {
        ...
        T Bisector(std::size_t numComponents, Vector<T, N> const& e,
            Vector<T, N> const& y, Vector<T, N>& x)
        {
            ...
            std::size_t constexpr jmax = 2048;
    ...
    private:
        friend class UnitTestDistPointHyperellipsoid;
    };
}
