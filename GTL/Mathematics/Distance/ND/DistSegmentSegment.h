...
namespace gtl
{
    ...
    public:
            ...
            Vector<T, N> const& Q0, Vector<T, N> const& Q1)
        {
            ...
            if (a > C_<T>(0) && c > C_<T>(0))
            {
                ...
                std::array<std::int32_t, 2> classify{};
                ...
                else
                {
                    ...
                    // 2 (t=0), 3 (t=1).
                    std::array<std::size_t, 2> edge{ 0, 0 };
    ...
    private:
        ...
        static void ComputeIntersection(std::array<T, 2> const& sValue,
            std::array<std::int32_t, 2> const& classify, T const& b, T const& f00,
            T const& f10, std::array<std::size_t, 2>& edge,
            std::array<std::array<T, 2>, 2>& end)
        {
        ...
        static void ComputeMinimumParameters(std::array<std::size_t, 2> const& edge,
    ...
    private:
        friend class UnitTestDistSegmentSegment;
    };
}
