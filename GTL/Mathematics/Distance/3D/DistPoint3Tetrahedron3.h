...
namespace gtl
{
        ...
        Output operator()(Vector3<T> const& point, Tetrahedron3<T> const& tetrahedron)
        {
            ...
            for (std::size_t i = 0; i < 4; ++i)
            {
                if (Dot(planes[i].normal, point) >= planes[i].constant)
                {
                    ...
                    auto ptOutput = ptQuery(point, triangle);
                    if (output.sqrDistance == invalid ||
                        ptOutput.sqrDistance < output.sqrDistance)
                    {
                        output.sqrDistance = ptOutput.sqrDistance;
                        output.closest = ptOutput.closest;
                    }
                }
            }
            ...
            (void)ComputeBarycentrics(output.closest[1], tetrahedron.v[0],
    ...
    private:
        friend class UnitTestDistPoint3Tetrahedron3;
    };
}
