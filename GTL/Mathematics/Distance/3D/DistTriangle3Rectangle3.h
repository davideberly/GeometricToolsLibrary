...
namespace gtl
{
    ...
    public:
        ...
        Output operator()(Triangle3<T> const& triangle, Rectangle3<T> const& rectangle)
        {
            ...
            std::array<std::size_t, 4> j0{ 0, 0, 1, 1 };
            std::array<std::size_t, 4> j1{ 1, 1, 0, 0 };
            std::array<std::array<std::size_t, 2>, 4> const edges
            {{
            ...
            for (std::size_t i0 = 2, i1 = 0, i2 = 1; i1 < 3; i2 = i0, i0 = i1++)
            {
    ...
    private:
        friend class UnitTestDistTriangle3Rectangle3;
    };
}
