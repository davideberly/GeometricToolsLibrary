...
namespace gtl
{
            ...
            Critical* outCritical = nullptr)
        {
            ...
            if (NxM != vzero)
            {
                ...
                if (NxD != vzero)
                {
                    ...
                    // circle center.
                    PDFSection422(line, circle, D, NxM, NxD, output, critical);
                }
                else
                {
                    // The line origin is on the normal line through the circle
                    // center.
                    PDFSection421(line, circle, D, NxM, output, critical);
                }
            }
            else
            {
                ...
                if (NxD != vzero)
                {
                    ...
                    PDFSection412(line, circle, D, output, critical);
                }
                else
                {
                    // The line contains the circle center.
                    PDFSection411(line, circle, D, output, critical);
                }
            }
    ...
    private:
        ...
        void PDFSection411(Line3<T> const& line, Circle3<T> const& circle, Vector3<T> const& D,
            Output& output, Critical& critical)
        {
            ...
            output.linearClosest[0] = C;
            Vector3<T> U = GetOrthogonal(N, true);
            output.circularClosest[0] = C + r * U;
            Vector3<T> diff = output.linearClosest[0] - output.circularClosest[0];
            output.sqrDistance = Dot(diff, diff);
            output.distance = std::sqrt(output.sqrDistance);
            output.equidistant = true;
            critical.numPoints = 1;
            critical.linearPoint[0] = output.linearClosest[0];
            critical.circularPoint[0] = output.circularClosest[0];
            critical.parameter[0] = -Dot(M, D) / Dot(M, M);
            critical.distance[0] = output.distance;
        }
            ...
            Output& output, Critical& critical)
        {
            ...
            critical.numPoints = 1;
            critical.parameter[0] = -Dot(M, D) / Dot(M, M);
            Finalize(line, circle, D, output, critical);
        }
            ...
            Output& output, Critical& critical)
        {
            ...
            if (a1 > std::sqrt(a3))
            {
                ...
                T const twoThirds = C_<T>(2) / C_<T>(3);
            ...
            Output& output, Critical& critical)
        {
            ...
            {
                output.numClosestPairs = 1;
                output.distance = critical.distance[0];
                output.linearClosest[0] = critical.linearPoint[0];
                output.circularClosest[0] = critical.circularPoint[0];
            }
            else // critical.numPoints = 2
            {
                ...
                {
                    output.numClosestPairs = 1;
                    output.distance = critical.distance[1];
                    output.linearClosest[0] = critical.linearPoint[1];
                    output.circularClosest[0] = critical.circularPoint[1];
                }
                else // critical.distance[0] = critical.distance[1]
                {
                    ...
                    output.distance = critical.distance[0];
                    output.linearClosest[0] = critical.linearPoint[0];
                    output.linearClosest[1] = critical.linearPoint[1];
                    output.circularClosest[0] = critical.circularPoint[0];
                    output.circularClosest[1] = critical.circularPoint[1];
                }
            }
    ...
    private:
        friend class UnitTestDistLine3Circle3;
    };
}
