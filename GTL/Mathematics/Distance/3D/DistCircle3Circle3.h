...
namespace gtl
{
    ...
    public:
        ...
        Output operator()(Circle3<T> const& inCircle0, Circle3<T> const& inCircle1)
        {
            Output output{};
            ...
            {
                output.circle0Closest[i] = (output.circle0Closest[i] * rotate) / scale - translate;
                output.circle1Closest[i] = (output.circle1Closest[i] * rotate) / scale - translate;
            }

            if (swapped)
            {
                std::swap(output.circle0Closest, output.circle1Closest);
            }
            return output;
        }
    ...
    private:
        ...
        public:
            ...
            inline Polynomial1<T> const& operator[] (std::uint32_t i) const
            {
                return mPoly[i];
            }

            inline Polynomial1<T>& operator[] (std::uint32_t i)
            {
                return mPoly[i];
            }
            ...
            Matrix3x3<T>& rotate, Vector3<T>& translate, T& scale)
        {
            ...
            RigidMotion<T>::Convert(aa, rotate);
