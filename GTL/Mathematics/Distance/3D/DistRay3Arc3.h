...
//
// The ray-arc distance is formulated as a calculus problem. The domain
// of the function is
//   (t,angle) in [0,+infinity)x(minAngle,maxAngle)
...
// (t,maxAngle), or at a boundary point (0,angle).
...
{
    template <typename T>
    class DCPQuery<T, Ray3<T>, Arc3<T>>
    {
    public:
        ...
        Output operator()(Ray3<T> const& ray, Arc3<T> const& arc,
            Critical* outCritical = nullptr)
        {
            ...
            bool equidistant = output.equidistant;
            output = Output{};
            ...
            if (!equidistant)
            {
                ...
                candidates.push_back(Candidate(arc.end[0], ray));  // (t,minAngle)
                candidates.push_back(Candidate(arc.end[1], ray));  // (t,maxAngle)
                candidates.push_back(Candidate(ray.origin, arc));  // (0,angle)

                // Insert critical points only if they are on the ray.
            ...
            {
                Candidate candidate(arc.end[0], ray);
    ...
    private:
        friend class UnitTestDistRay3Arc3;
    };
}
