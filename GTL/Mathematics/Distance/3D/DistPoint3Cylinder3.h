...
namespace gtl
{
    ...
    private:
            ...
            T const& height, Output& output)
        {
            ...
            // cylinder.
            T halfHeight = C_<T>(1, 2) * height;
            if (output.closest[1][2] > halfHeight)
            {
                output.closest[1][2] = halfHeight;
                Vector3<T> diff = output.closest[1] - P;
                output.sqrDistance = Dot(diff, diff);
                output.distance = std::sqrt(output.sqrDistance);
            }
            else if (output.closest[1][2] < -halfHeight)
            {
                output.closest[1][2] = -halfHeight;
                Vector3<T> diff = output.closest[1] - P;
                output.sqrDistance = Dot(diff, diff);
    ...
    private:
        friend class UnitTestDistPoint3Cylinder3;
    };
}
