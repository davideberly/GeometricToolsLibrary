...
void WICFileIONative::Load(std::string const& filename, std::uint32_t& outFormat,
    std::size_t& outWidth, std::size_t& outHeight, std::vector<std::uint8_t>& outTexels)
{
    ...
    TextureCreator creator = [&outFormat, &outWidth, &outHeight, &outTexels, &bytesPerTexel](
        std::uint32_t format, std::size_t width, std::size_t height)
    {
        outFormat = format;
        outWidth = width;
        outHeight = height;
        outTexels.resize(width * height * bytesPerTexel[format]);
        return reinterpret_cast<std::uint8_t*>(outTexels.data());
    };
...
void WICFileIONative::Load(void* module, std::string const& rtype,
    std::int32_t resource, std::uint32_t& outFormat, std::size_t& outWidth, std::size_t& outHeight,
...
void WICFileIONative::Load(void* module, std::string const& rtype,
    std::int32_t resource, TextureCreator const& creator)
{
    // Start COM and create WIC.
    ComInitializer comInitializer;
    if (!comInitializer.IsInitialized())
...
void WICFileIONative::SaveToPNG(std::string const& filename, std::uint32_t inFormat,
    std::size_t inWidth, std::size_t inHeight, std::uint8_t const* inTexels)
{
    SaveTo(filename, inFormat, inWidth, inHeight, inTexels, -1.0f);
}

void WICFileIONative::SaveToJPEG(std::string const& filename, std::uint32_t inFormat,
    std::size_t inWidth, std::size_t inHeight, std::uint8_t const* inTexels,
    float imageQuality)
{
...
void WICFileIONative::DoLoad(void* factory, void* frameDecode, TextureCreator const& creator)
{
    ...
    std::uint32_t format = R8G8B8A8;
    for (std::int32_t i = 0; i < NUM_LOAD_FORMATS; ++i)
    {
    ...
    // Create the 2D image.
    std::uint8_t* texels = creator(format, width, height);
...
void WICFileIONative::SaveTo(std::string const& filename, std::uint32_t inFormat,
    std::size_t inWidth, std::size_t inHeight, std::uint8_t const* inTexels, float imageQuality)
{
    ...
    for (std::int32_t i = 0; i < NUM_SAVE_FORMATS; ++i)
    {
...
std::array<std::uint32_t, WICFileIONative::NUM_FORMATS> const
