...
#include <GTL/Mathematics/DifferentialEquations/OdeMidpoint.h>
#include "DrawLine.h"
...
UnitTestOdeMidpoint::UnitTestOdeMidpoint()
{
    UTInformation("Mathematics/DifferentialEquations/OdeMidpoint");
...
void UnitTestOdeMidpoint::Test1()
{
    ...
    OdeMidpoint<double, 1> solver(tDelta, F);
...
void UnitTestOdeMidpoint::Test2()
{
    ...
    OdeMidpoint<double, 2> solver(tDelta, F);
    ...
    std::ofstream outFile("Mathematics/DifferentialEquations/Output/PendulumMidpoint.txt");
    for (std::size_t i = 0; i < output.size(); ++i)
    {
...
#if defined(GTL_USE_MSWINDOWS)
    WICFileIONative::SaveToPNG("Mathematics/DifferentialEquations/Output/PendulumMidpoint.png",
        WICFileIONative::R8G8B8A8, numOutputs, numOutputs,
        reinterpret_cast<std::uint8_t*>(canvas.data()));
#endif
}

void UnitTestOdeMidpoint::TestDynamic()
{
    ...
    double const tDelta = 0.1;
    OdeMidpoint<double> solver(tDelta, F);
    ...
    // Write the approximation solution as text.
    std::ofstream outFile("Mathematics/DifferentialEquations/Output/PendulumMidpointD.txt");
    for (std::size_t i = 0; i < output.size(); ++i)
    {
        outFile << "i = " << i << ", " << output[i] << std::endl;
    }
...
#if defined(GTL_USE_MSWINDOWS)
    WICFileIONative::SaveToPNG("Mathematics/DifferentialEquations/Output/PendulumMidpointD.png",
        WICFileIONative::R8G8B8A8, numOutputs, numOutputs,
        reinterpret_cast<std::uint8_t*>(canvas.data()));
#endif
}
...
namespace gtl
{
    template class OdeMidpoint<float, 1>;
    template class OdeMidpoint<float, 2>;
    template class OdeMidpoint<float>;

    template class OdeMidpoint<double, 1>;
    template class OdeMidpoint<double, 2>;
    template class OdeMidpoint<double>;

#if defined(GTL_INSTANTIATE_RATIONAL)
    using Rational = BSRational<UIntegerAP32>;
    template class OdeMidpoint<Rational, 1>;
    template class OdeMidpoint<Rational, 2>;
    template class OdeMidpoint<Rational>;
#endif
}
