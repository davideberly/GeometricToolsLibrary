...
#include <GTL/Mathematics/DifferentialEquations/OdeRungeKutta4.h>
#include "DrawLine.h"
...
namespace gtl
{
    class UnitTestOdeRungeKutta4
    {
    public:
        UnitTestOdeRungeKutta4();
...
UnitTestOdeRungeKutta4::UnitTestOdeRungeKutta4()
{
    UTInformation("Mathematics/DifferentialEquations/OdeRungeKutta4");
...
void UnitTestOdeRungeKutta4::Test1()
{
    ...
    OdeRungeKutta4<double, 1> solver(tDelta, F);
...
void UnitTestOdeRungeKutta4::Test2()
{
    ...
    OdeRungeKutta4<double, 2> solver(tDelta, F);
    ...
    std::ofstream outFile("Mathematics/DifferentialEquations/PendulumRungeKutta4.txt");
    for (std::size_t i = 0; i < output.size(); ++i)
    {
...
#if defined(GTL_USE_MSWINDOWS)
    WICFileIONative::SaveToPNG("Mathematics/DifferentialEquations/PendulumRungeKutta4.png",
        WICFileIONative::R8G8B8A8, numOutputs, numOutputs,
        reinterpret_cast<std::uint8_t*>(canvas.data()));
#endif
}

void UnitTestOdeRungeKutta4::TestDynamic()
{
    ...
    double const tDelta = 0.1;
    OdeRungeKutta4<double> solver(tDelta, F);
    ...
    // Write the approximation solution as text.
    std::ofstream outFile("Mathematics/DifferentialEquations/PendulumRungeKutta4D.txt");
    for (std::size_t i = 0; i < output.size(); ++i)
    {
        outFile << "i = " << i << ", " << output[i] << std::endl;
    }
...
#if defined(GTL_USE_MSWINDOWS)
    WICFileIONative::SaveToPNG("Mathematics/DifferentialEquations/PendulumRungeKutta4D.png",
        WICFileIONative::R8G8B8A8, numOutputs, numOutputs,
        reinterpret_cast<std::uint8_t*>(canvas.data()));
#endif
}
...
namespace gtl
{
    template class OdeRungeKutta4<float, 1>;
    template class OdeRungeKutta4<float, 2>;
    template class OdeRungeKutta4<float>;

    template class OdeRungeKutta4<double, 1>;
    template class OdeRungeKutta4<double, 2>;
    template class OdeRungeKutta4<double>;

#if defined(GTL_INSTANTIATE_RATIONAL)
    using Rational = BSRational<UIntegerAP32>;
    template class OdeRungeKutta4<Rational, 1>;
    template class OdeRungeKutta4<Rational, 2>;
    template class OdeRungeKutta4<Rational>;
#endif
}
...
GTL_TEST_FUNCTION(OdeRungeKutta4)
