...
void UnitTestApprGreatArc3::Test()
{
    ...
    std::ofstream output("Mathematics/Approximation/3D/Input/ApprGreatArc3Points.binary", std::ios::binary);
    output.write(reinterpret_cast<char const*>(points.data()), points.size() * sizeof(Vector3<double>));
    output.close();

    output.open("Mathematics/Approximation/3D/Input/ApprGreatArc3Points.txt");
    ...
    output.close();
#else
    std::ifstream input("Mathematics/Approximation/3D/Input/ApprGreatArc3Points.binary", std::ios::binary);
