...
namespace gtl
{
    class UnitTestUIntegerFP32
    {
    public:
        UnitTestUIntegerFP32();
    ...
    private:
        ...
        using UInteger = UIntegerFP32<8>;
        using ALU = UIntegerALU32<UInteger>;
    };
}

UnitTestUIntegerFP32::UnitTestUIntegerFP32()
{
    UTInformation("Mathematics/Arithmetic/UIntegerFP32");
...
void UnitTestUIntegerFP32::TestShift()
{
    UInteger n0, n1, n2, actual;
...
void UnitTestUIntegerFP32::TestAdd()
{
    UInteger n0, n1, n2, n3, actual;
...
void UnitTestUIntegerFP32::TestSub()
{
    UInteger n0, n1, n2, n3, actual;
    std::uint64_t u0, u1, uactual;
    std::size_t shift;
...
void UnitTestUIntegerFP32::TestMul()
{
    UInteger n0, n1, n2, n3, n4, n5;
...
void UnitTestUIntegerFP32::TestRoundUp()
{
    std::uint64_t u0 = 0x0091BB5C22AE9EF7ull;
    std::uint64_t u1 = (u0 >> 3) + 1ull;
    UInteger n0(u0), n1(u1);
    std::size_t shift = UIntegerALU32<UInteger>::RoundUp(n0);
    ...
    n0 = UInteger(u0);
    n1 = UInteger(1u);
    shift = UIntegerALU32<UInteger>::RoundUp(n0);
    UTAssert(shift == 64, "Incorrect RoundUp.");
    UTAssert(n0 == n1, "Incorrect RoundUp.");
}

void UnitTestUIntegerFP32::TestComparison()
{
    ...
    n1 = UInteger(static_cast<std::uint64_t>(0));
    result = (n0 < n1);
    UTAssert(result == false, "Incorrect LessThan.");
    result = (n1 < n0);
    UTAssert(result == true, "Incorrect LessThan.");

    n0 = UInteger(static_cast<std::uint64_t>(0));
    result = (n0 < n1);
    UTAssert(result == false, "Incorrect LessThan.");
    result = (n1 < n0);
    UTAssert(result == false, "Incorrect LessThan.");
...
void UnitTestUIntegerFP32::TestCopyAndMove()
{
    std::uint64_t u0 = 0x1291BB5C22AE9EF7ull;
    UInteger n0(u0);
    UInteger n1 = n0;
...
void UnitTestUIntegerFP32::TestWriteAndRead()
{
    std::uint64_t u0 = 0x1291BB5C22AE9EF7ull;
    UInteger n0(u0);
...
namespace gtl
{
    template class UIntegerFP32<8>;

    template bool operator==(UIntegerFP32<8> const&, UIntegerFP32<8> const&);
    template bool operator!=(UIntegerFP32<8> const&, UIntegerFP32<8> const&);
    template bool operator<(UIntegerFP32<8> const&, UIntegerFP32<8> const&);
    template bool operator<=(UIntegerFP32<8> const&, UIntegerFP32<8> const&);
    template bool operator>(UIntegerFP32<8> const&, UIntegerFP32<8> const&);
...
GTL_TEST_FUNCTION(UIntegerFP32)
