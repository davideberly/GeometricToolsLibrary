...
namespace gtl
{
    ...
    public:
        ...
        MinHeap(std::size_t maxElements = 0)
            :
            mNumElements(0),
            mKeys{},
            mIndices{},
        ...
        // whatever the default constructor creates.
        void Reset(std::size_t maxElements)
        {
            mNumElements = 0;
            if (maxElements > 0)
            {
        ...
        inline std::size_t GetMaxElements() const
        {
            return mKeys.size();
        }
        ...
        std::size_t GetMinimum(std::size_t& handle, T& weight) const
        {
            if (mNumElements > 0)
            {
                // Get the minimum value from the root of the min-heap.
                std::size_t key = mKeys[0];
        ...
        // to Update.
        std::size_t Insert(std::size_t handle, T const& weight)
        {
            if (mNumElements < GetMaxElements())
            {
                // Store (handle, weight) in the last leaf node of the binary
                // tree.
                std::size_t insertIndex = mNumElements++;
                std::size_t insertKey = mKeys[insertIndex];
                ...
                // tree to a min-heap.
                std::size_t childIndex = insertIndex;
                while (childIndex > 0)
                {
                    std::size_t parentIndex = (childIndex - 1) / 2;
                    std::size_t parentKey = mKeys[parentIndex];
        ...
        std::size_t Remove(std::size_t& handle, T& weight)
        {
            if (mNumElements > 0)
            {
                // Get the minimum value from the root of the min-heap.
                std::size_t removeKey = mKeys[0];
                ...
                // min-heap.
                std::size_t lastIndex = --mNumElements;
                if (lastIndex == 0)
                {
                    ...
                    if (mNumElements == 0)
                    {
                        std::size_t const maxElements = GetMaxElements();
                        for (std::size_t i = 0; i < maxElements; ++i)
                        {
                            mKeys[i] = i;
                            mIndices[i] = i;
                ...
                std::size_t lastKey = mKeys[lastIndex];
                ...
                {
                    std::size_t childKey = mKeys[childIndex];
                    if (childIndex < lastIndex)
                    {
                        // Select the child with smallest value.
                        std::size_t otherChildIndex = childIndex + 1;
                        std::size_t otherChildKey = mKeys[otherChildIndex];
        ...
        bool Update(std::size_t updateKey, T const& updateWeight)
        {
            std::size_t updateIndex = mIndices[updateKey];
            if (updateKey < GetMaxElements() && updateIndex < GetNumElements())
            {
                ...
                if (oldWeight < updateWeight)
                {
                    ...
                    std::size_t parentIndex = mIndices[updateKey];
                    std::size_t childIndex = 2 * parentIndex + 1;
                ...
                else if (updateWeight < oldWeight)
                {
                    ...
                    // it toward the root.
                    std::size_t childIndex = updateIndex;
                    while (childIndex > 0)
                    {
                        std::size_t parentIndex = (childIndex - 1) / 2;
                        std::size_t parentKey = mKeys[parentIndex];
        ...
        bool IsValid() const
        {
            for (std::size_t childIndex = 1; childIndex < mNumElements; ++childIndex)
            {
                std::size_t childKey = mKeys[childIndex];
                std::size_t parentIndex = (childIndex - 1) / 2;
                std::size_t parentKey = mKeys[parentIndex];
        ...
        struct Node
        {
            ...
            Node(std::size_t inHandle, T const& inWeight)
                :
                handle(inHandle),
                weight(inWeight)
            {
        ...
        inline Node const& GetNode(std::size_t key) const
        {
            return mNodes[key];
        }

        inline std::size_t GetHandle(std::size_t key) const
        {
            return mNodes[key].handle;
        }

        inline T const& GetWeight(std::size_t key) const
        {
    ...
    private:
        ...
        std::vector<std::size_t> mKeys, mIndices;
