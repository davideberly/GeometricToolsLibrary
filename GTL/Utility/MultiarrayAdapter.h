...
namespace gtl
{
    template <typename T, bool OrderLtoR, std::size_t... Sizes>
    ...
    public:
        ...
        T const& operator()(std::array<std::size_t, sizeof...(Sizes)> const& coordinate) const
        {
            return mContainer[this->index(coordinate)];
        }

        T& operator()(std::array<std::size_t, sizeof...(Sizes)> const& coordinate)
        {
            return mContainer[this->index(coordinate)];
        }
    ...
    private:
        friend class UnitTestMultiarrayAdapter;
    };
}
...
namespace gtl
{
    ...
    public:
        ...
        // uninitialized.
        MultiarrayAdapter(std::vector<std::size_t> const& sizes, T* container)
            :
        ...
        void reset(std::vector<std::size_t> const& sizes, T* container)
        {
            GTL_ARGUMENT_ASSERT(
                container != nullptr,
                "The container must exist.");
        ...
        // Get the element corresponding to the n-dimensional coordinate.
        T const& operator()(std::vector<std::size_t> const& coordinate) const
        {
            return mContainer[this->index(coordinate)];
        }

        T& operator()(std::vector<std::size_t> const& coordinate)
        {
            return mContainer[this->index(coordinate)];
        }
