...
{
    GTL_ARGUMENT_ASSERT(
        texture != nullptr && engine != nullptr,
        "Invalid input.");
...
bool DX11Engine::SetFullscreen(DXGIOutput const& output, bool fullscreen)
{
    if (mSwapChain)
    {
        ...
        if (iter->second != fullscreen)
        {
            ...
            if (fullscreen)
            {
                ...
                {
                    GTL_RUNTIME_ERROR("Failed to go fullscreen.");
                }
            }
            else
            {
                ...
                {
                    GTL_RUNTIME_ERROR("Failed to go windowed.");
                }
            }
        }
        else
    ...
    GTL_RUNTIME_ERROR("This function requires a swap chain.");
}
...
bool DX11Engine::CreateBestMatchingDevice()
{
    ...
    GTL_RUNTIME_ERROR("Failed to create device");
}
    ...
    DX11VertexShader*& dxVShader, DX11GeometryShader*& dxGShader, DX11PixelShader*& dxPShader)
{
    ...
    GTL_RUNTIME_ASSERT(
        ...
        "Effect must have a vertex shader.");
    ...
    GTL_RUNTIME_ASSERT(
        effect->GetPixelShader(),
        "Effect must have a pixel shader.");
...
void DX11Engine::EnableCBuffers(Shader const* shader, DX11Shader* dxShader)
{
    ...
    for (auto const& cb : shader->GetData(index))
    {
        if (cb.object)
        {
            ...
            {
                GTL_RUNTIME_ERROR("Failed to bind constant buffer.");
            }
        }
        else
        {
            GTL_RUNTIME_ERROR(cb.name + " is null constant buffer.");
        }
    }
}
...
void DX11Engine::EnableTBuffers(Shader const* shader, DX11Shader* dxShader)
{
    ...
    for (auto const& tb : shader->GetData(index))
    {
        if (tb.object)
        {
            ...
            {
                GTL_RUNTIME_ERROR("Failed to bind texture buffer.");
            }
        }
        else
        {
            GTL_RUNTIME_ERROR(tb.name + " is null texture buffer.");
        }
    }
}
...
void DX11Engine::EnableSBuffers(Shader const* shader, DX11Shader* dxShader)
{
    ...
    for (auto const& sb : shader->GetData(index))
    {
        if (sb.object)
        {
            ...
            {
                GTL_RUNTIME_ERROR("Failed to bind structured buffer.");
            }
        }
        else
        {
            GTL_RUNTIME_ERROR(sb.name + " is null structured buffer.");
        }
    }
}
...
void DX11Engine::EnableRBuffers(Shader const* shader, DX11Shader* dxShader)
{
    ...
    for (auto const& rb : shader->GetData(index))
    {
        if (rb.object)
        {
            ...
            {
                GTL_RUNTIME_ERROR("Failed to bind byte-address buffer.");
            }
        }
        else
        {
            GTL_RUNTIME_ERROR(rb.name + " is null byte-address buffer.");
        }
    }
}
...
void DX11Engine::EnableTextures(Shader const* shader, DX11Shader* dxShader)
{
    ...
    for (auto const& tx : shader->GetData(index))
    {
        if (tx.object)
        {
            ...
            {
                GTL_RUNTIME_ERROR("Failed to bind texture.");
            }
        }
        else
        {
            GTL_RUNTIME_ERROR(tx.name + " is null texture.");
        }
    }
    ...
    if (type == GT_PIXEL_SHADER)
    {
        ...
        if (numBindPoints > 0)
        {
            ...
            {
                GTL_RUNTIME_ASSERT(
                    bindPoints[i] == bindPoints[i - 1] + 1,
                    "Bind points must be consecutive.");
            }
...
void DX11Engine::EnableTextureArrays(Shader const* shader, DX11Shader* dxShader)
{
    ...
    for (auto const& ta : shader->GetData(index))
    {
        if (ta.object)
        {
            ...
            {
                GTL_RUNTIME_ERROR("Failed to bind texture array.");
            }
        }
        else
        {
            GTL_RUNTIME_ERROR(ta.name + " is null texture array.");
        }
    }
}
...
void DX11Engine::EnableSamplers(Shader const* shader, DX11Shader* dxShader)
{
    ...
    for (auto const& ss : shader->GetData(index))
    {
        if (ss.object)
        {
            ...
            {
                GTL_RUNTIME_ERROR("Failed to bind sampler state.");
            }
        }
        else
        {
            GTL_RUNTIME_ERROR(ss.name + " is null sampler state.");
        }
    }
}
...
void DX11Engine::SetBlendState(std::shared_ptr<BlendState> const& state)
{
    if (state)
    {
        if (state != mActiveBlendState)
        {
            ...
            {
                GTL_RUNTIME_ERROR("Failed to bind blend state.");
            }
        }
    }
    else
    {
        GTL_RUNTIME_ERROR("Input state is null.");
    }
}
...
void DX11Engine::SetDepthStencilState(std::shared_ptr<DepthStencilState> const& state)
{
    if (state)
    {
        if (state != mActiveDepthStencilState)
        {
            ...
            {
                GTL_RUNTIME_ERROR("Failed to bind depth-stencil state.");
            }
        }
    }
    else
...
void DX11Engine::SetRasterizerState(std::shared_ptr<RasterizerState> const& state)
{
    if (state)
    {
        if (state != mActiveRasterizerState)
        {
            ...
            {
                GTL_RUNTIME_ERROR("Failed to bind rasterizer state.");
            }
        }
    }
    else
    ...
    uint32_t numXGroups, uint32_t numYGroups, uint32_t numZGroups)
{
    ...
    if (hlslProgram && numXGroups > 0 && numYGroups > 0 && numZGroups > 0)
    {
        ...
        {
            GTL_RUNTIME_ERROR("Invalid input parameter.");
        }
    }
}
