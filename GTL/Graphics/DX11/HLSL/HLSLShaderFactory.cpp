    ...
    uint32_t compileFlags)
{
    ...
    GTL_ARGUMENT_ASSERT(
        type == "vs_" || type == "gs_" || type == "ps_" || type == "cs_",
        "Invalid target " + target
    );

    ID3DBlob* compiledCode = CompileShader(name, source, entry, target, compileFlags, defines);
    if (!compiledCode)
    {
    ...
    ProgramDefines const& defines)
{
    ...
    // Repackage the 'defines' for D3DCompile.
    ...
    HRESULT hr = D3DCompile(source.c_str(), source.length(), name.c_str(),
        ...
        (compileFlags | D3DCOMPILE_PACK_MATRIX_ROW_MAJOR), 0, &compiledCode, &errors);
    ...
    if (SUCCEEDED(hr))
    {
        if (errors)
        {
            ...
            GTL_RUNTIME_ERROR(
                "D3DCompile warning: " + std::string(message));
        }
    }
    else  // FAILED(hr)
    {
        if (errors)
        {
            ...
            GTL_RUNTIME_ERROR(
                "D3DCompile error: " + std::string(message));
        }
        else
        {
...
bool HLSLShaderFactory::GetCBuffers(ID3DShaderReflection* reflector, HLSLReflection& shader)
{
    ...
    for (UINT i = 0; i < numCBuffers; ++i)
    {
        ...
        {
            GTL_RUNTIME_ERROR(
                "Interface pointers are not yet supported in GTL.");
        }
    }
    return true;
}
