    ...
    std::string const&, std::string const& gsSource)
{
    ...
    {
        GTL_RUNTIME_ERROR("A program must have a vertex shader and a pixel shader.");
    }
    ...
    if (programHandle == 0)
    {
        GTL_RUNTIME_ERROR("Program creation failed.");
    }
    ...
    std::string const&, std::string const& csSource)
{
    if (csSource == "")
    {
        GTL_RUNTIME_ERROR("A program must have a compute shader.");
    }
...
GLuint GLSLProgramFactory::Compile(GLenum shaderType, std::string const& source)
{
    ...
    if (handle > 0)
    {
        ...
        if (logLength > 0)
        {
            ...
            GTL_RUNTIME_ERROR("Compile failed:\n" + message);
        }
        else
        {
            GTL_RUNTIME_ERROR("Invalid info log length.");
        }
    }
    else
    {
        GTL_RUNTIME_ERROR("Cannot create shader.");
    }
}
