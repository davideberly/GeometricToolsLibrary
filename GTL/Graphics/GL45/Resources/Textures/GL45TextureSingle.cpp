...
bool GL45TextureSingle::Update(uint32_t level)
{
    auto texture = GetTexture();
    if (texture->GetUsage() != Resource::Usage::DYNAMIC_UPDATE)
    {
        GTL_RUNTIME_ERROR("Texture usage must be DYNAMIC_UPDATE.");
    }

    return DoCopyCpuToGpu(level);
}

bool GL45TextureSingle::CopyCpuToGpu(uint32_t level)
{
...
bool GL45TextureSingle::CopyGpuToCpu(uint32_t level)
{
    ...
    if (level >= numLevels)
    {
        GTL_RUNTIME_ERROR("Level for Texture is out of range");
    }

    auto pixBuffer = mLevelPixelPackBuffer[level];
    if (0 == pixBuffer)
    {
        GTL_RUNTIME_ERROR("Staging buffer not defined for level " + std::to_string(level));
    }
    ...
    {
        GTL_RUNTIME_ERROR("No target data for texture level " + std::to_string(level));
    }
...
bool GL45TextureSingle::DoCopyCpuToGpu(uint32_t level)
{
    auto texture = GetTexture();

    if (CanAutoGenerateMipmaps() && (level > 0))
    {
        GTL_RUNTIME_ERROR("Cannot update automatically generated mipmaps in GPU");
    }
    ...
    if ((nullptr == data) || (0 == numBytes))
    {
        GTL_RUNTIME_ERROR("No source data for texture level " + std::to_string(level));
    }
