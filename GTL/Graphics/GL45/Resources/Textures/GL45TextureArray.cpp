...
bool GL45TextureArray::Update(uint32_t item, uint32_t level)
{
    auto texture = GetTexture();
    GTL_ARGUMENT_ASSERT(
        texture->GetUsage() != Resource::Usage::DYNAMIC_UPDATE,
...
bool GL45TextureArray::CopyGpuToCpu(uint32_t item, uint32_t level)
{
    ...
    if (item >= numItems)
    {
        GTL_RUNTIME_ERROR("Item for texture is out of range");
    }
...
bool GL45TextureArray::DoCopyCpuToGpu(uint32_t item, uint32_t level)
{
    ...
    if (item >= numItems)
    {
        GTL_RUNTIME_ERROR("Item for texture array is out of range");
    }
    ...
    if (level >= numLevels)
    {
        GTL_RUNTIME_ERROR("Level for texture array is out of range");
    }
    ...
    if ((nullptr == data) || (0 == numBytes))
    {
        GTL_RUNTIME_ERROR("No source data for texture array level " + std::to_string(level));
    }
