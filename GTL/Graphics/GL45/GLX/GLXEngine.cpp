    ...
    mIsComputeWindow(false)
{
    ...
    if (!mDisplay)
    {
        GTL_RUNTIME_ERROR("XOpenDisplay failed.");
        return;
    }
    ...
    if (!success)
    {
        GTL_RUNTIME_ERROR("glXQueryExtension failed, errorBase = "
    ...
    if (!visualInfo)
    {
        GTL_RUNTIME_ERROR("glXChooseVisual failed.");
        return;
    }
    ...
    if (!mImmediate)
    {
        GTL_RUNTIME_ERROR("glXCreateContext failed.");
        return;
    }
...
bool GLXEngine::Initialize(int32_t requiredMajor, int32_t requiredMinor, bool useDepth24Stencil8, bool saveDriverInfo)
{
    ...
    {
        GTL_RUNTIME_ERROR("glXMakeCurrent failed.");
