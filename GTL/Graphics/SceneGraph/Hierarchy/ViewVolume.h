...
#define GTL_VALIDATE_COORDINATE_FRAME_ONCE
...
namespace gtl
{
    ...
    public:
        ...
        //   X = P + y0*R + y1*U + y2*D,
        // where y0 = Dot(R,X-P), y1 = Dot(U,X-P), and y2 = Dot(D,X-P). If
        // Y = (y0,y1,y2,1), then Y = V*X, where V is the view matrix.
    ...
    protected:
        ...
        // The view matrix V for the coordinate frame. The other matrix is
        // the inverse view matrix V^{-1}.
        ...
        // GTL_VALIDATE_COORDINATE_FRAME_ONCE).  It allows you to trap
