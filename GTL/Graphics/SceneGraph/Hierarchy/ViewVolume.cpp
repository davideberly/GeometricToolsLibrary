    ...
    Vector4<float> const& uVector, Vector4<float> const& rVector)
{
    ...
    {
#if defined(GTL_VALIDATE_COORDINATE_FRAME_ONCE)
        ...
        // to renormalize.
        std::vector<Vector4<float>> v = { mDVector, mUVector, mRVector };
        (void)Orthonormalize<float, 4>(v);
        mDVector = v[0];
        mUVector = v[1];
        mRVector = v[2];
    }
...
{
    float halfAngleRadians = 0.5f * upFovDegrees * C_DEG_TO_RAD<float>;
...
bool ViewVolume::GetFrustum(float& upFovDegrees, float& aspectRatio, float& dMin, float& dMax) const
{
        ...
        && mFrustum[VF_UMIN] == -mFrustum[VF_UMAX])
    {
        ...
        upFovDegrees = 2.0f * std::atan(tmp) * C_RAD_TO_DEG<float>;
