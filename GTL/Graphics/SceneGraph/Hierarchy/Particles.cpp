...
void Particles::GenerateParticles(std::shared_ptr<Camera> const& camera)
{
    ...
    Matrix4x4<float> inverse = worldTransform.GetInverseH();
...
uint32_t Particles::IsValid(VertexFormat const& vformat) const
{
    ...
    if (index < 0)
    {
#if defined(GTL_THROW_ON_PARTICLES_INVALID)
        GTL_RUNTIME_ERROR("Vertex format does not have VASemantic::POSITION.");
#else
    ...
    {
#if defined(GTL_THROW_ON_PARTICLES_INVALID)
        GTL_RUNTIME_ERROR("Invalid position type.");
#else
        return std::numeric_limits<uint32_t>::max();
#endif
    }
    ...
    {
#if defined(GTL_THROW_ON_PARTICLES_INVALID)
        GTL_RUNTIME_ERROR("Position offset must be 0.");
#else
        return std::numeric_limits<uint32_t>::max();
#endif
    }
    ...
    {
#if defined(GTL_THROW_ON_PARTICLES_INVALID)
        GTL_RUNTIME_ERROR("Vertex format does not have VASemantic::TEXCOORD.");
#else
        return std::numeric_limits<uint32_t>::max();
#endif
    }
    ...
    {
#if defined(GTL_THROW_ON_PARTICLES_INVALID)
        GTL_RUNTIME_ERROR("Invalid texture coordinate type.");
#else
        return std::numeric_limits<uint32_t>::max();
#endif
    }
    ...
    {
#if defined(GTL_THROW_ON_PARTICLES_INVALID)
        GTL_RUNTIME_ERROR("Texture coordinate must immediately follow position.");
#else
        return std::numeric_limits<uint32_t>::max();
#endif
    }
