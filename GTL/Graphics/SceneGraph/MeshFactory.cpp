    ...
    uint32_t numYSamples, float xExtent, float yExtent)
{
    ...
    Vector3<float> normal{ 0.0f, 0.0f, 1.0f };
    Vector3<float> tangent{ 1.0f, 0.0f, 0.0f };
    Vector3<float> bitangent{ 0.0f, 1.0f, 0.0f };  // = Cross(normal,tangent)
    ...
    float xExtent, float yExtent)
{
    ...
    Vector3<float> normal{ 0.0f, 0.0f, 1.0f };
    Vector3<float> tangent{ 1.0f, 0.0f, 0.0f };
    Vector3<float> bitangent{ 0.0f, 1.0f, 0.0f };  // = Cross(normal,bitangent)
    Vector2<float> tcoord{};
    position[2] = 0.0f;
    for (uint32_t i1 = 0, i = 0; i1 < numSamples; ++i1)
    {
        tcoord[1] = i1 * inv;
        position[1] = tcoord[1] * yExtent;
        for (uint32_t i0 = 0; i0 + i1 < numSamples; ++i0, ++i)
        {
            tcoord[0] = i0 * inv;
            position[0] = tcoord[0] * xExtent;
    ...
    uint32_t numRadialSamples, float radius)
{
    ...
    SetNormal(0, normal);
    SetTangent(0, tangent);
    SetBitangent(0, bitangent);
    SetTCoord(0, tcoord);

    for (uint32_t r = 0; r < numRadialSamples; ++r)
    {
        float angle = invRS * r * C_TWO_PI<float>;
...
std::shared_ptr<Visual> MeshFactory::CreateBox(float xExtent, float yExtent, float zExtent)
{
    ...
    Vector3<float> position{}, normal{}, tangent{}, bitangent{};
    Vector2<float> tcoord{};
    ...
    for (uint32_t z = 0, v = 0; z < 2; ++z)
    {
        ...
        normal[2] = zSign * diag[2];
        for (uint32_t y = 0; y < 2; ++y)
        {
            float fy = static_cast<float>(y);
            float ySign = 2.0f * fy - 1.0f;
            position[1] = ySign * yExtent;
            normal[1] = ySign * diag[1];
            tcoord[1] = (1.0f - fy) * omfz + (0.75f - 0.5f * fy) * fz;
            for (uint32_t x = 0; x < 2; ++x, ++v)
            {
                float fx = static_cast<float>(x);
                float xSign = 2.0f * fx - 1.0f;
                position[0] = xSign * xExtent;
                normal[0] = xSign * diag[0];
                tcoord[0] = fx * omfz + (0.25f + 0.5f * fx) * fz;

                ComputeOrthonormalBasis(1, normal, tangent, bitangent);

                SetPosition(v, position);
                SetNormal(v, normal);
                SetTangent(v, tangent);
                SetBitangent(v, bitangent);
                SetTCoord(v, tcoord);
            }
        }
    }
    ...
    uint32_t numRadialSamples, float radius, float height)
{
    ...
    position = Position(i);
    position[2] = -hDiv2;
    SetPosition(i, position);  // south pole
    i = numVertices - 1;
    position = Position(i);
    position[2] = +hDiv2;
    SetPosition(i, position);  // north pole
    ...
    for (i = 0; i < numVertices - 2; ++i)
    {
        ...
        float adjust = radius / std::sqrt(position[0] * position[0] + position[1] * position[1]);
        position[0] *= adjust;
        position[1] *= adjust;
        SetPosition(i, position);
    }
    ...
    uint32_t numRadialSamples, float radius)
{
    ...
    for (uint32_t z = 1; z < zsm1; ++z)
    {
        ...
        for (uint32_t r = 0; r <= numRadialSamples; ++r, ++i)
        {
            ...
            ComputeOrthonormalBasis(1, normal, tangent, bitangent);
            tcoord = { radialFraction, 0.5f * (zFraction + 1.0f) };
    ...
    float outerRadius, float innerRadius)
{
    ...
    for (uint32_t c = 0; c < numCircleSamples; ++c)
    {
        ...
        float theta = circleFraction * C_TWO_PI<float>;
        ...
        for (uint32_t r = 0; r <= numRadialSamples; ++r, ++i)
        {
            ...
            float phi = radialFraction * C_TWO_PI<float>;
            float cosPhi = std::cos(phi);
            float sinPhi = std::sin(phi);
            normal = cosPhi * radial + sinPhi * Vector3<float>::Unit(2);
...
std::shared_ptr<Visual> MeshFactory::CreateTetrahedron()
{
    ...
    {
        position = Position(i);
        normal = (mOutside ? position : -position);
        SetNormal(i, normal);
        ComputeOrthonormalBasis(1, normal, tangent, bitangent);
        SetTangent(i, tangent);
        SetBitangent(i, bitangent);
        SetPlatonicTCoord(i, position);
    }
...
void MeshFactory::SetPlatonicTCoord(uint32_t i, Vector3<float> const& pos)
{
    ...
    {
        tcd[0] = 0.5f * (1.0f + std::atan2(pos[1], pos[0]) * C_INV_PI<float>);
    }
    else
    {
        tcd[0] = 0.5f;
    ...
    tcd[1] = std::acos(pos[2]) * C_INV_PI<float>;
