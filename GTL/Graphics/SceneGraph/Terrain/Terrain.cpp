    ...
    mCamera(camera)
{
    ...
    GTL_ARGUMENT_ASSERT(
        mSize == 3 || mSize == 5 || mSize == 9 || mSize == 17 || mSize == 33 || mSize == 65 || mSize == 129,
        "Invalid page size.");

    GTL_ARGUMENT_ASSERT(
        minElevation <= maxElevation,
        "Invalid ordering of elevation extremes.");
    ...
    GTL_ARGUMENT_ASSERT(
        offset == 0,
        "VertexFormat offset must be 0.");

    GTL_ARGUMENT_ASSERT(mCamera != nullptr, "Camera must exist.");
...
{
    GTL_ARGUMENT_ASSERT(
        row < mNumRows && col < mNumCols,
        "Invalid input to GetPage.");
...
{
    GTL_ARGUMENT_ASSERT(
        row < mNumRows && col < mNumCols && heights.size() >= mSize * mSize,
