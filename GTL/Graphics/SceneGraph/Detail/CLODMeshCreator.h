...
namespace gtl
{
    ...
    public:
            ...
            std::vector<CLODCollapseRecord>& records)
        {
            ...
            mHeap.Reset(mIndices.size());
            mSortedEdges.reserve(mIndices.size());
            mSortedEdges.clear();
            ...
            {
                mHeap.Update(element.second.heapKey, ComputeMetric(element.first));
            }
            ...
            while (mHeap.GetNumElements() > 0)
            {
                ...
                else
                {
                    ...
                    mHeap.Update(emIter->second.heapKey, std::numeric_limits<float>::max());
                }
            }
    ...
    private:
        ...
        public:
            Edge()
                :
                adjTriangles{},
                heapKey(std::numeric_limits<size_t>::max())
            {
            }
        ...
        void InsertTriangle(TriangleKey<true> const& tKey, Triangle t)
        {
            ...
            {
                EdgeKey<false>(tKey[0], tKey[1]),
                EdgeKey<false>(tKey[1], tKey[2]),
                EdgeKey<false>(tKey[2], tKey[0])
            };

            // Insert each edge into its endpoints' adjacency lists.
            for (size_t i0 = 2, i1 = 0; i1 < 3; i0 = i1++)
            {
                mVertices[tKey[i1]].adjEdges.insert(eKey[i0]);
                mVertices[tKey[i1]].adjEdges.insert(eKey[i1]);
                mVertices[tKey[i1]].adjTriangles.insert(tKey);
            }
            ...
            for (size_t i = 0; i < 3; ++i)
            {
                ...
                if (emIter == mEdges.end())
                {
                    ...
                    edge.heapKey = mHeap.Insert(mSortedEdges.size(), std::numeric_limits<float>::max());
                    mSortedEdges.push_back(eKey[i]);
                }
                else
                {
        ...
        void RemoveTriangle(TriangleKey<true> const& tKey)
        {
            ...
            for (size_t i = 0; i < 3; ++i)
            {
                mVertices[tKey[i]].adjTriangles.erase(tKey);
            }
            ...
            for (size_t i0 = 2, i1 = 0; i1 < 3; i0 = i1++)
            {
                ...
                if (emIter->second.adjTriangles.empty())
                {
                    ...
                    // from the heap.
                    mHeap.Update(emIter->second.heapKey, -1.0f);
                    size_t unusedHandle{};
                    float metric{};
                    mHeap.Remove(unusedHandle, metric);
                    GTL_RUNTIME_ASSERT(
                        metric == -1.0f,
                        "The metric should be -1.");

                    // Delete the edge from its endpoints' adjacency lists.
                    mVertices[tKey[i0]].adjEdges.erase(eKey[i0]);
                    mVertices[tKey[i1]].adjEdges.erase(eKey[i0]);
        ...
        float ComputeMetric(EdgeKey<false> const& eKey) const
        {
            ...
            {
                // Length contribution.
                Vector3<float> end0 = mVertexAtoms[eKey[0]].GetPosition();
                Vector3<float> end1 = mVertexAtoms[eKey[1]].GetPosition();
                ...
                Vector3<float> pos00 = mVertexAtoms[tKey[0]].GetPosition();
                Vector3<float> pos01 = mVertexAtoms[tKey[1]].GetPosition();
                Vector3<float> pos02 = mVertexAtoms[tKey[2]].GetPosition();
                Vector3<float> normal0 = Cross(pos01 - pos00, pos02 - pos00);

                tKey = *emIter->second.adjTriangles.rbegin();
                Vector3<float> pos10 = mVertexAtoms[tKey[0]].GetPosition();
                Vector3<float> pos11 = mVertexAtoms[tKey[1]].GetPosition();
                Vector3<float> pos12 = mVertexAtoms[tKey[2]].GetPosition();
        ...
        int32_t CanCollapse(EdgeKey<false> const& eKey)
        {
            ...
            if (mVertices[eKey[0]].collapsible)
            {
                indexKeep = 1;
                indexThrow = 0;
            }
            else if (mVertices[eKey[1]].collapsible)
            {
                indexKeep = 0;
                indexThrow = 1;
            }
            ...
            // over.
            int32_t vKeep = eKey[indexKeep];
            int32_t vThrow = eKey[indexThrow];
        ...
        void Collapse(EdgeKey<false> const& eKey, int32_t indexThrow)
        {
            ...
            for (auto const& updateEdgeKey : needUpdate)
            {
                auto emIter = mEdges.find(updateEdgeKey);
                size_t heapKey = emIter->second.heapKey;
                mHeap.Update(heapKey, ComputeMetric(heapKey));
            }
        }
        ...
        // The vertex-edge-triangle graph. The mSortedEdges container stores
        // the EdgeKey<false> objects in the order they are encountered during
        // triangle insertion.
        VertexArray mVertices;
        EdgeMap mEdges;
        std::vector<EdgeKey<false>> mSortedEdges;
