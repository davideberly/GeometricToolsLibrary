...
KeyframeController::KeyframeController(int32_t numCommonTimes, int32_t numTranslations,
    int32_t numRotations, int32_t numScales, AffineTransform<float> const& localTransform)
    :
...
Vector3<float> KeyframeController::GetTranslate(float normTime, int32_t i0, int32_t i1)
{
    Vector3<float> trn = mTranslations[i0] + normTime * (mTranslations[i1] - mTranslations[i0]);
    return trn;
}

Matrix3x3<float> KeyframeController::GetRotate(float normTime, int32_t i0, int32_t i1)
{
    Quaternion<float> q = Slerp(normTime, mRotations[i0], mRotations[i1]);
    Matrix3x3<float> rot = Rotation<float>(q);
    return rot;
}
