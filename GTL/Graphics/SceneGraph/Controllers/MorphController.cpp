...
{
    GTL_ARGUMENT_ASSERT(
        numTargets > 0 && numVertices > 0 && numTimes > 0,
...
{
    GTL_ARGUMENT_ASSERT(
        target < mNumTargets && vertices.size() >= mNumVertices,
...
{
    GTL_ARGUMENT_ASSERT(
        times.size() >= mNumTimes,
        "Input times array is too small.");
...
{
    GTL_ARGUMENT_ASSERT(
        key < mNumTimes && weights.size() >= mNumTargets,
...
void MorphController::SetObject(ControlledObject* object)
{
    ...
    auto const& vbuffer = visual->GetVertexBuffer();
    GTL_ARGUMENT_ASSERT(
        vbuffer->GetNumElements() == mNumVertices,
