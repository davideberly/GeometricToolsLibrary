...
void Picker::ExecuteRecursive(std::shared_ptr<Spatial> const& object)
{
    ...
    if (visual)
    {
        if (visual->worldBound.TestIntersection(HProject(mOrigin), HProject(mDirection), mTMin, mTMax))
        {
            ...
            Matrix4x4<float> const& invWorldMatrix = visual->worldTransform.GetInverseH();
            Line3<float> line;
            Vector4<float> temp;
    ...
    uint32_t i0, uint32_t i1, std::vector<PickRecord>& output) const
{
    ...
    for (uint32_t i = i0; i <= i1; ++i)
    {
        ...
        if (result.intersect && mTMin <= result.parameter[0] && result.parameter[0] <= mTMax)
        {
            PickRecord record;
            ...
            record.bary[0] = result.barycentric[0][0];
            record.bary[1] = result.barycentric[0][1];
            record.bary[2] = result.barycentric[0][2];
            record.linePoint = HLift(result.point[0], 1.0f);
    ...
    uint32_t vstride, IndexBuffer* ibuffer, Line3<float> const& line)
{
    ...
    for (uint32_t i = firstSegment; i < firstSegment + numSegments; ++i)
    {
        ...
        // Get the vertex positions.
        auto const& p0 = *reinterpret_cast<Vector3<float> const*>(positions + static_cast<size_t>(v0) * vstride);
        auto const& p1 = *reinterpret_cast<Vector3<float> const*>(positions + static_cast<size_t>(v1) * vstride);
    ...
    uint32_t vstride, IndexBuffer* ibuffer, Line3<float> const& line)
{
    ...
    for (uint32_t i = firstPoint; i < firstPoint + numPoints; ++i)
    {
        ...
        auto const& p = *reinterpret_cast<Vector3<float> const*>(positions + static_cast<size_t>(v) * vstride);
