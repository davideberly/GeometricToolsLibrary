...
IndexBuffer::IndexBuffer(IPType type, uint32_t numPrimitives, size_t indexSize, bool createStorage)
    :
    Buffer(msIndexCounter[BitHacks::Log2OfPowerOfTwo(static_cast<uint32_t>(type))](numPrimitives), indexSize, createStorage),
    mPrimitiveType(type),
...
IndexBuffer::IndexBuffer(IPType type, uint32_t numPrimitives)
    :
    Buffer(msIndexCounter[BitHacks::Log2OfPowerOfTwo(static_cast<uint32_t>(type))](numPrimitives), 0, false),
    mPrimitiveType(type),
    mNumPrimitives(numPrimitives),
    mNumActivePrimitives(numPrimitives),
    mFirstPrimitive(0)
...
{
    uint32_t i = BitHacks::Log2OfPowerOfTwo(static_cast<uint32_t>(mPrimitiveType));
...
uint32_t IndexBuffer::GetFirstIndex() const
{
    ...
    int32_t i = BitHacks::Log2OfPowerOfTwo(static_cast<uint32_t>(mPrimitiveType));
    ...
    if (ValidPrimitiveType(IP_HAS_TRIANGLES))
    {
        if (mData && i < mNumPrimitives)
        {
            if (mElementSize == sizeof(uint32_t))
            {
                ...
                {
                    GTL_RUNTIME_ERROR(
                        "IP_TRIMESH_ADJ not yet supported.");
                }
                else if (mPrimitiveType == IP_TRISTRIP_ADJ)
                {
                    GTL_RUNTIME_ERROR(
                        "IP_TRISTRIP_ADJ not yet supported.");
                }
            }
            else
            {
